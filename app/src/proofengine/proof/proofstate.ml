...
let proofstate_cxt (Proofstate { cxt }) = cxt

let proofstate_tree (Proofstate { tree }) = tree

let proofstate_givens (Proofstate { givens }) = givens

let proofstate_goal (Proofstate { goal }) = goal

let proofstate_target (Proofstate { target }) = target

let proofstate_root (Proofstate { root }) = root

let withcxt (Proofstate s) cxt = Proofstate { s with cxt }

let withtree (Proofstate s) tree = Proofstate { s with tree }

let withgivens (Proofstate s) givens = Proofstate { s with givens }

let withgoal (Proofstate s) goal = Proofstate { s with goal }

let withtarget (Proofstate s) target = Proofstate { s with target }

let withroot (Proofstate s) root = Proofstate { s with root }
...
let string_of_proofstate all
    (Proofstate { cxt; tree; givens; goal; target; root }) =
  ...
  implode
    [
      ...
      ",\ntarget = ";
      string_of_option string_of_fmtpath target;
      " = ";
      ...
      string_of_option
        (fun ((rp, _) as r) ->
          string_of_pair
            (string_of_subtree (Some (rootPath rp)))
            string_of_fmtpath "," r)
        root;
      "}";
    ]

let prunestate goal (Proofstate { cxt; tree } as state) =
  let goal', tree' = truncateprooftree cxt goal tree in
  withtree
    (withtarget (withgoal state (Some goal')) (Some goal'))
...
let proofstep a1 a2 a3 =
  match (a1, a2, a3) with
  | cxt, subtree, (Proofstate { tree; goal = Some gpath } as state) ->
...
let nextGoal a1 a2 =
  match (a1, a2) with
  | skip, (Proofstate { tree; goal = Some gpath } as state) ->
      withtarget (withgoal state (nextusefulgoal skip tree gpath)) (Some gpath)
  | skip, state -> state

let isproven (Proofstate { tree; cxt }) =
  (not (hasTip tree))
  && not
       (List.exists isUnknown
          (nj_fold
             (uncurry2 (sortedmerge earliervar))
             (provisovars <.> provisoactual <* provisos (rewritecxt cxt))
             []))

let rewriteproofstate (Proofstate { cxt; tree; givens } as state) =
...
let addautorule ((sense, tac) as rule) =
  autorulelist := rule :: ((fun (_, tac') -> tac <> tac') <| !autorulelist)
