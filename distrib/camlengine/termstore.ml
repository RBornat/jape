...
let ( hashterm,
  ...
  let module Local = struct
    ...
    and hashBinding bindings env pat =
      hash_3
        (hash_3 hash_ts hash_ts hash_ts)
        (hash_list (hash_2 hashterm Hashtbl.hash))
        hashterm (bindings, env, pat)
    ...
    let cacheelement e =
      try E.find elementtable e
      with Not_found ->
        E.add elementtable e e;
        e
    ...
    *)
    ...
    and registerApp (f, a) =
      if !termhashing then
        let h = hashApp f a in
        cacheterm (App (Some h, f, a))
      else App (None, f, a)
    ...
    and registerTup (s, ts (* as sts *)) =
      ...
      | _ ->
          if !termhashing then
            let h = hashTup s ts in
            cacheterm (Tup (Some h, s, ts))
          else Tup (None, s, ts)

    and registerLiteral l =
      if !termhashing then
        let h = hashLiteral l in
        cacheterm (Literal (Some h, l))
      else Literal (None, l)

    and registerFixapp (ss, ts) =
      if !termhashing then
        let h = hashFixapp ss ts in
        cacheterm (Fixapp (Some h, ss, ts))
      else Fixapp (None, ss, ts)

    and registerSubst (r, t, vts) =
      if !termhashing then
        let h = hashSubst r t vts in
        cacheterm (Subst (Some h, r, t, vts))
    ...
    and registerCollection (c, els) =
      if !termhashing then
        let h = hashCollection c els in
        cacheterm (Collection (Some h, c, els))
      else Collection (None, c, els)

    and registerElement (r, t) =
      if !termhashing then
        let h = hashElement r t in
        cacheelement (Element (Some h, r, t))
      else Element (None, r, t)

    and registerSegvar (ms, v) =
      if !termhashing then
        let h = hashSegvar ms v in
        cacheelement (Segvar (Some h, ms, v))
      else Segvar (None, ms, v)

    let resettermstore () =
      T.clear termtable;
      E.clear elementtable
  ...
  ( Local.hashterm,
    Local.hashelement,
    Local.registerId,
    Local.registerUnknown,
    Local.registerApp,
    Local.registerTup,
    Local.registerLiteral,
    Local.registerFixapp,
    Local.registerSubst,
    Local.registerBinding,
    Local.registerCollection,
    Local.registerElement,
    Local.registerSegvar,
    Local.resettermstore )
