...
TACTIC "ç elim forward" IS
	WHEN	(LETHYP2 _P (_Pç_Q) 
					(Noarg (CUTIN "ç elim" (WITHHYPSEL (hyp (_Pç_Q))) (WITHHYPSEL (hyp _P))) "ç elim")
				)
				(LETHYP2 _R (_Pç_Q)
					("ç elim forward fail"
				...
				(LETHYP (_Pç_Q) 
					(WHEN (LETGOAL _R (Noarg (CUTIN "ç elim" (WITHHYPSEL hyp) fstep) "ç elim"))
								("ç elim forward fail" (" You only selected %t.", _Pç_Q))
					)
				)
				(LETLHS (_Pç_Q)
					(WHEN (LETGOAL _R (Noarg (ForwardCut 0 "ç elim") "ç elim"))
