...
module Fmt :
  Fmt with type treelayout = Treelayout.treelayout and type term = Termtype.term =
struct
  open Listfuns
  open Optionfuns
  open Sml
  ...
  let nf_string =
    string_of_triple string_of_bool enQuote
      (string_of_option string_of_intlist)
      ","

  let rec string_of_treeformat (TreeFormat pair) =
    "TreeFormat"
    ^ string_of_pair string_of_treeformat_kind string_of_treeformat_filter ","
        pair

  and string_of_treeformat_kind = function
    | HideRootFormat -> "HideRootFormat"
    | HideCutFormat -> "HideCutFormat"
    | SimpleFormat -> "SimpleFormat"

  and string_of_treeformat_filter = function
    | DefaultFormat -> "DefaultFormat"
    | RotatingFormat stuff ->
        "RotatingFormat"
        ^ string_of_pair string_of_int
            (bracketed_string_of_list nf_string ",")
            "," stuff

  (* if tf1 is hidden, so is the merge *)
  let rec treeformatmerge (TreeFormat (tfk1, tff1), TreeFormat (tfk2, tff2)) =
    let rec tfkmerge = function SimpleFormat, _ -> tfk2 | _, _ -> tfk1 in
    let rec tffmerge = function
      ...
      | RotatingFormat (i1, nfs1), RotatingFormat (i2, nfs2) ->
          let res =
            ...
            | _ ->
                ...
                let rec redundant nf = List.exists (fun nf' -> nf = nf') nfs1 in
  ...
  let rec layouts_of_format (TreeFormat (tfk, tff) (* as f *)) =
    let rec layout = function
      | (true, s, isopt) :: nfs -> CompressedLayout (ly s isopt) :: layout nfs
  ...
  let rec string_of_fmtpath (FmtPath p) =
    "FmtPath " ^ bracketed_string_of_list string_of_int "," p
end

(* -------------------------- prooftrees for display, after formatting -------------------------- *)

module type VisFmt = sig
  (* VisPaths, at the present, are still simple lists of non-negative integers ... *)
...
module VisFmt : VisFmt = struct
  ...
  let string_of_vispath (VisPath p) =
    "VisPath " ^ bracketed_string_of_list string_of_int "," p
end
