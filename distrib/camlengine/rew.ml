...
(* in 09/2020, this file hadn't been touched since the camlp4 translation from SML-NJ. 
   It's still hardly been cosmeticised.
 *)
...
let raw2rew_ (vars, uVIDs, badres, psig) =
  mkrewinf (sortunique earliervar vars, sortunique (<) uVIDs, sortunique (<) badres, psig)

(* these functions designed to be folded *)
let rawinfTerm n (t, stuff) =
  let rec f (t, (vars, uVIDs, badres, psig as z)) =
    match t with
    | Id _                  -> Some (t :: vars, uVIDs, badres, psig)
    | Unknown (_, u, _)     -> Some (t :: vars, u :: uVIDs, badres, psig)
    | Subst (_, r, _P, vts) -> let ff = foldterm f in
...
let rawinfSeq n (Seq (st, _Hs, _Gs), stuff) =
  let _RIT = rawinfTerm n in 
  _RIT (_Hs, _RIT (_Gs, stuff))

let rawinfProviso n (p, stuff) =
  let _RIT = rawinfTerm n in
...
let rew_worthwhile subst cxt ri =
 (subst && (match rewinf_psig ri, getprovisosig cxt with
            | Some si, ci -> si <> ci
            | _           -> false
           ) 
  || List.exists (bool_of_opt <.> (fun uVID -> (varmap cxt <@> uVID))) (rewinf_uVIDs ri)
 ) 
 || List.exists (bool_of_opt <.> (fun i -> (resmap cxt <@> i))) (rewinf_badres ri)

(* this is almost f &~ (_Some <.> yes), but types get in the way ... *)
(* do they? I think it's f x &~~ (_Some <.> yes) RB 14/09/20 *)
let rew_ f x yes =
  match f x with
  | Some x' -> Some (yes x')
  | _       -> None

(* this is _Some <.> anyway f *)
let rew_yes f t =
  match f t with
  | None -> Some t
  | some -> some

let rew_2 f x g y yes =
  (option_rewrite2 f g &~ (_Some <.> yes)) (x, y)

let rew_Pair _R = option_rewrite2 _R _R

(* there must be a better way ... *)
let rec rew_3 f x g y h z yes =
  (option_rewrite3 f g h &~ (_Some <.> yes)) (x, y, z)

let rew_binding outer inner (h, (bs, ss, us), env, pat) =
  rew_3 (option_rewritelist outer) bs 
        (option_rewritelist inner) ss
...
let rec rew_Term subst cxt t =
  let rec _S t =
    match t with
    | Id (_, v, _)      -> None
    | Unknown (_, v, _) -> (match (varmap cxt <@> v) with
                            | Some t' -> rew_yes _S t'
                            | None -> None
                           )
...
let def (r, t) = rew_ _RT t (fun t -> [registerElement (r, t)]) in
let rec _RE =
function
...
| Element (_, (ResUnknown i as r), t) ->
    ...
    (match (resmap cxt <@> i) with
     | Some (r, t) -> rew_yes (rew_elements subst cxt) [registerElement (r, t)]
     | None        -> def (r, t)
    )
...
and rew_resnum_and_term subst cxt (r, t) = 
  let def () = rew_ (rew_Term subst cxt) t (fun t -> (r,t)) in
  match r with
  | ResUnknown i -> (match (resmap cxt <@> i) with
                    | Some (r, t) -> rew_resnum_and_term subst cxt (r, t)
                    | None        -> def ()
                   )
  | _            -> def ()
