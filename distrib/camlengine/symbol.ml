...
type synfixrec = { appF : int; substF : int; substS : bool }
...
let rec mkalt cts def =
  ...
  List.iter (fun (c, t) -> Hashtbl.add aa c t) cts;
  lookup

let optree : (int, symbol) searchtree ref = ref (emptysearchtree mkalt)

let idprefixtree : (int, idclass) searchtree ref = ref (emptysearchtree mkalt)

let idfixedtree : (int, idclass) searchtree ref = ref (emptysearchtree mkalt)
...
let rec get_oplist () =
  ...
  | None ->
      ...
      sortunique ( < )
        ( ops
        @ List.map utf8_of_ucode !decIDheads
        @ List.map utf8_of_ucode !decIDtails )
      ...
      if !symboldebug then
        consolereport
          [
            ...
            " ";
            string_of_bool isprefix;
            " ";
            enQuote s;
          ];
      tree := addtotree (fun (x, y) -> x = y) !tree (c :: cs, class__, isprefix)

let rec preclassopt = function
...
let prio sym =
  ...
  with Not_found ->
    raise
      (Catastrophe_ [ "Symbol.prio looking up "; debugstring_of_symbol sym ])

let assoc sym =
  try Hashtbl.find !assoctable sym
  with Not_found ->
    raise
      (Catastrophe_ [ "Symbol.assoc looking up "; debugstring_of_symbol sym ])
...
let rec register_op s sym =
  ...
  if List.exists (not <.> ispunct) cs then
    bang
      ( "notallpunct "
      ^ bracketed_string_of_list (enQuote <.> utf8_of_ucode) ";" cs );
  if !symboldebug then
    consolereport [ "register_op "; enQuote s; " "; debugstring_of_symbol sym ];
  optree := addtotree (fun (x, y) -> x = y) !optree (cs, sym, false)

let rec deregister_op s sym =
  if !symboldebug then
    consolereport
      [ "deregister_op "; enQuote s; " "; debugstring_of_symbol sym ];
  optree :=
    deletefromtree (fun (x, y) -> x = y) !optree (utf8_explode s, sym, false)
...
let delete symbol =
  let doit () =
    ...
    ( if
      isop oldstring
      &&
      match searchfsm (rootfsm optree) (utf8_explode oldstring) with
      | Found _ -> true
      | NotFound _ -> false
    then
      ...
      with DeleteFromTree_ ->
        let string_of_csrb =
          string_of_triple
            (bracketed_string_of_list (fun i -> "0x" ^ hexstring_of_int i) ",")
        ...
        consolereport
          [
            "DeleteFromTree_\n\t";
            string_of_pair
              (bracketed_string_of_list string_of_csrb "; ")
              string_of_csrb "\n\t"
              (summarisetree !optree, (utf8_explode oldstring, symbol, false));
          ] );
    if hidden symbol then Hashtbl.remove !reversemapping symbol;
    Hashtbl.remove !symboltable oldstring;
    Hashtbl.remove !priotable symbol;
    Hashtbl.remove !assoctable symbol
  in
  match symbol with ID (_, None) -> () | UNKNOWN _ -> () | _ -> doit ()
...
let enter string n a symbol =
  let rec doit () =
    ...
    if hidden symbol then Hashtbl.add !reversemapping symbol string;
...
let rec resetSymbols () =
  ...
  List.iter
    (fun s -> enter s None None (SHYID s))
    [
      "ABSTRACTION";
      "AND";
      "ARE";
      ...
      "DISTINCT";
      "DISPROOF";
      (* keep this for a year or so. RB 15.xi.2002 *)
      "END";
      "ENTRY";
      "FONTS";
      "FORCEDEF";
...
let set_syntax_tables syntabs =
  symboltable := syntabs.symT;
  reversemapping := syntabs.revT;
  priotable := syntabs.prioT;
  assoctable := syntabs.assocT;
  decIDheads := syntabs.decIDhs;
  decIDtails := syntabs.decIDts;
  idprefixtree := syntabs.idprefT;
  idfixedtree := syntabs.idfixT;
  optree := syntabs.opT;
  oplist := syntabs.ops;
...
let carefullyEnter s n a t =
  let bang_redef other ss =
    raise
      (ParseError_
         ( ss
         @ [
             "redefine the syntactic role of ";
             enQuote s;
             " from ";
             debugstring_of_symbol other;
             " to ";
             debugstring_of_symbol t;
           ] ))
  in
  let doit () =
    match !syntaxes with
    | (sname, _, tbls) :: _ ->
        ...
        ( try
            ...
            | Some other, t ->
                bang_redef other
                  [ "After PUSHSYNTAX "; enQuote sname; " attempt to " ]
          with exn ->
  ...
  | None -> doit ()
  | Some (ID _) -> doit ()
  | Some other -> if other = t then () else bang_redef other [ "Attempt to " ]
...
let rec autoID class__ prefix =
  ...
  | None -> (
      ...
      | None ->
          let _ = declareIdPrefix class__ prefix in
          prefix
...
let rec isnumber s = not (List.exists (not <.> isdigit) (utf8_explode s))
...
let rec scanwhile pp rcs con =
  ...
  if c <> uEOF && pp c then (
    next ();
    scanwhile pp (c :: rcs) con )
  else con (utf8_implode (List.rev rcs))

let rec scanwatch f =
  if !symboldebug then ( fun v ->
    let c = f v in
...
let rec scanop fsm rcs =
    ...
    consolereport
      [
        ...
        bracketed_string_of_list
          (fun c -> enCharQuote (utf8_of_ucode c))
          ";" rcs;
      ];
  ...
  | NotFound rcs' ->
      if rcs' = [] then (
        let c = char () in
        next ();
        checkbadID (utf8_of_ucode c) )
      else checkbadID (utf8_implode (List.rev rcs'))
...
let rec scanreport s =
  if !symboldebug then (
    consolereport [ debugstring_of_symbol s ];
    s )
  else s
...
let rec scan () =
  ...
  | space when space = Char.code ' ' ->
      next ();
      scan ()
  | tab when tab = Char.code '\t' ->
      next ();
      scan ()
  | nl when nl = Char.code '\n' ->
      ...
      next ();
      scan ()
  | bra when bra = Char.code '(' ->
      next ();
      scanreport (BRA "(")
  | ket when ket = Char.code ')' ->
  ...
  | dquote when dquote = Char.code '"' ->
      next ();
      scanreport (scanString !linenum [])
  | c ->
      if c = metachar then (
        (* unused
        ...
        if isIDhead (char ()) then
          ...
          raise
            (ParseError_ [ "ID expected following "; utf8_of_ucode metachar ]) )
      else if isIDhead c then
        scanreport (scanid (checkidclass (fun sc -> ID sc) true))
...
and scanComment (n, k) =
  ...
  | eof when eof = uEOF ->
      raise
        (ParseError_
           [
             "End of file inside level ";
             string_of_int n;
             " comment starting on line ";
             string_of_int k;
           ])
  | nl when nl = Char.code '\n' ->
      incr linenum;
...
let rec scansymb () =
  if !symboldebug then
    consolereport
      [
        "scansymb -- peekedsymb is ";
        bracketed_string_of_list debugstring_of_symbol ";" !peekedsymb;
      ];
...
let rec putbacksymb s =
  peekedsymb := !symb :: !peekedsymb;
  symb := s
...
let rec currnovelsymb () =
  ...
  if canstartnovelsymb !symb then
    ...
    *)
    ...
    raise
      (ParseError_
         [ string_of_symbol !symb; " can't start a new identifier or operator" ])
    ...
    raise
      (ParseError_
         [
           ...
           ", found ";
           debugstring_of_symbol (currsymb ());
         ])

let ignore s = if currsymb () = s then scansymb () else ()
...
let rec mustseparate = function
  ...
  | a1, a2 ->
      ...
      let rec msp c =
        bool_of_opt
          (fsmpos (rootfsm optree) (utf8_explode a1) &~~ fun t -> fsmpos t [ c ])
      in
      let ms = function
        | SPACE, _ -> false
        | _, SPACE -> false
...
let _ =
  try resetSymbols () with
  | Catastrophe_ ss ->
      consolereport [ "resetSymbols raised Catastrophe_ \""; implode ss; "\"" ]
  | exn -> consolereport [ "resetSymbols raised "; Printexc.to_string exn ]
...
let popSyntax () =
  ...
  | (name, fixes, tbls) :: sys ->
      ...
      appfix := fixes.appF;
      substfix := fixes.substF;
      substsense := fixes.substS;
      set_syntax_tables tbls;
      syntaxes := sys;
      ...
      *)
      let checksym str sym =
        ...
        | _ ->
            let newsym =
              try
                let s = pushlex "" (stream_of_utf8string str) in
                let r = !symb in
                next ();
                let c = char () in
                poplex s;
