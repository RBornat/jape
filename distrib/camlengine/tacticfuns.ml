...
let rec askNbc m bs c = Alert.askCancel (Alert.defaultseverity bs) m bs c 0
...
let rec getselectedhypotheses () =
  !selections &~~ fun (path, _, hs, _, _, _) ->
...
let ( getsidedselectedtext,
      getunsidedselectedtext,
      getsingleargsel,
      getsels,
      getargs ) =
  let rec getsides el sopt =
    try _The (term_of_element el &~~ explodebinapp)
    with _The_ ->
      raise
        (Catastrophe_
           [
             "getsides given ";
             string_of_element el;
             "; ";
      ...
      | Some s -> (
          ...
          | Left ->
              ...
              List.rev
                ...
                :: List.tl revss )
          | Right ->
              implode [ string_of_term left; " "; opp; " "; List.hd ss ]
              :: List.tl ss )
      | None -> ss
    in
    (p, el, ss')
  ...
  *)
  and getunsidedselectedtext () =
    getsidedselectedtext () &~~ fun (path, concsels, hypsels, givensels) ->
    let rec pairup = function
      | [] -> []
      | [ csel ] -> [ csel ]
      | ((_, (_, None), _) as csel) :: csels -> csel :: pairup csels
      | ((p, (el, Some side), ss) as csel) :: csels ->
          let rec otherside = function
            | _, (el', Some side'), _ -> sameresource (el, el') && side <> side'
            | _ -> false
          in
          let rec join ss ss' =
            let revss = List.rev ss in
            ...
            @ implode [ List.hd revss; " "; opp; " "; List.hd ss' ]
              :: List.tl ss'
          in
          if List.exists otherside csels then
            ...
            ( p,
              (el, None),
              match side with Left -> join ss ss' | Right -> join ss' ss )
            :: pairup csels'
  ...
  and getsingleargsel () =
    ...
    | _ -> None
  and getsels =
    (* selections (not givensels) are before :: sel :: middle :: sel :: ... *)
    function
    | x :: y :: zs -> y :: getsels zs
    | _ -> []
  and getargs str =
    ...
    | Some (_, [], [], []) ->
        setReason [ str; " failed: no text selections" ];
        None
    | None ->
        setReason [ str; " failed: no selections" ];
        None
    | Some (_, cs, hs, gs) ->
        let rec parseit s =
          try term_of_string s
          with ParseError_ ss ->
            raise
              (ParseError_
                 ...
                 @ [ ")" ] ))
        in
        Some (parseit <* List.concat (gs :: (getsels <.> thrd <* cs @ hs)))
  in
...
let rec _AQ env term =
  let rec _A = function
    | App (_, Id (_, v, _), a) ->
        if string_of_vid v = "QUOTE" then Some (_QU env a) else None
...
and _QU env term =
  let rec _Q = function
    | App (_, Id (_, v, _), a) ->
        if string_of_vid v = "ANTIQUOTE" then Some (_AQ env a) else None
    | _ -> None
  in
  mapterm _Q term
...
let rec striparg t =
  match nameopt_of_term t with Some n -> Argstring n | None -> Argterm t

let rec evalstr2 env arg =
  match env <@> arg with None -> Argstring arg | Some t -> striparg t
...
let rec extendwithselection env tac =
  ...
  | Some arg, TermTac (name, args) ->
      if extensibletac env tac then
        try TermTac (name, args @ [ quoteterm (term_of_string arg) ])
        with ParseError_ ss -> raise (ParseError_ (selparsefail arg ss))
      else tac
  | Some arg, WithSubstSelTac t -> WithSubstSelTac (extendwithselection env t)
  | Some arg, WithConcSelTac t -> WithConcSelTac (extendwithselection env t)
  | Some arg, WithHypSelTac t -> WithHypSelTac (extendwithselection env t)
  | Some arg, WithFormSelTac t -> WithFormSelTac (extendwithselection env t)
  | _ ->
      (* WithSelectionsTac -- clearly not, it includes WithArgSel
...
let askTime'sUp state =
  if time'sUp () then
    ...
    let bs =
      (string_of_int tried, tried)
      :: (string_of_int (tried * 2), tried * 2)
      ::
      ( if tried / 2 > 0 then
        [
          (string_of_int (tried / 2), tried / 2);
    ...
    if n = 0 then raise (Time'sUp_ (Some state))
    else if n = -1 then raise (Time'sUp_ None)
    else (
...
let _TermToParamTerm =
  (function
    | Ordinaryparam vc -> registerId vc | _ -> raise MatchinTermToParamTerm)
  <.> _TermToParam

let rec make_remark name args =
  let args = match args with [] -> [ "" ] | _ -> string_of_term <* args in
  match thingnamed name with
...
let freshenv patterns cxt env =
  ...
  (* consolereport ["after evaluation patterns are ";
                 bracketed_string_of_list string_of_term ";" patterns]; *)
  let us =
    nj_fold (uncurry2 (sortedmerge earliervar)) (termvars <* patterns) []
  in
  let rec f = function
...
let rec getcxt (Proofstate { cxt }) = cxt

let rec getconjecture (Proofstate { goal; tree }) =
  let seq, rewinf, _ = Prooftree.Tree.Fmttree.getTip tree (getGoalPath goal) in
  (seq, rewinf)

let rec getapplyinfo name args cxt =
  try
    match freshThingtoapply true name cxt args Proofstore.proved with
    | Some (cxt', env, principals, thing) -> (cxt', (env, principals, thing))
  ...
  with Fresh_ ss ->
    raise
      (Tacastrophe_
         ...
         :: " to arguments "
         :: bracketed_string_of_list string_of_term ", " args
         :: ": " :: ss ))
...
let rec hiddencontexts = function
  | [] -> (false, false)
  | _ -> (!autoAdditiveLeft, !autoAdditiveRight)

let rec expandstuff name (env, principals, thing) (Proofstate { givens }) =
...
let apply, resolve, applyorresolve =
    ...
    if !Applyrule.applydebug > 0 then
      consolereport
        [
          ...
          " ";
          string_of_option
            (string_of_pair string_of_cxt string_of_prooftree ",")
            resopt;
        ];
    let r =
      match resopt with
      ...
      | None ->
          prefixtoReason [ step_label how; " is not applicable: " ];
          None
    in
    if !Applyrule.applydebug > 0 then
      consolereport
  ...
  let rec apply checker filter taker selhyps selconcs stuff reason cxt state =
    ...
    makestep stuff' state
      (Applyrule.apply checker filter taker selhyps selconcs stuff' reason cxt
         (getconjecture state))
  and resolve checker filter taker selhyps selconcs stuff reason cxt state =
     ...
     *)
    let kind, hiddens, how, env, (lefts, rights), antes, conseq, provisos =
      stuff
    in
    ...
    let how' =
      ...
      | _ ->
          raise
            (Catastrophe_ [ "how in resolve is "; string_of_prooftree_step how ])
    in
  ...
  and applyorresolve checker filter taker selhyps selconcs stuff reason cxt
      state =
    ...
    |~~ fun _ ->
    if !resolvepossible (* user can cancel this *) then
...
let rec doSUBGOAL path (Proofstate { tree } as state) =
  try
    let tip = followPath tree path in
    if isTip tip then Some (withgoal state (Some path))
    else (
      setReason
        [ "That subgoal has been solved (SUBGOAL) "; string_of_fmtpath path ];
      None )
  with _ ->
    setReason [ "No such subtree (SUBGOAL) "; string_of_fmtpath path ];
    None

let rec doCOMPLETE f (Proofstate { cxt; goal; tree = parent; root } as state) =
  try
    let seq = getTip parent goal in
    let tree' = mkTip cxt (rewriteseq cxt seq) Treeformat.Fmt.neutralformat in
    let wholegoal, wholeproof = makewhole cxt root parent (getGoalPath goal) in
    let state' =
      withroot
        (withtree
    ...
    match
      try f state' with
      | Catastrophe_ ss -> raise (Catastrophe_ ("in argument of PROVE: " :: ss))
      | Tacastrophe_ ss -> raise (Tacastrophe_ ("in argument of PROVE: " :: ss))
  ...
  with _ ->
    raise
      (Tacastrophe_
         [
           mess;
           " must be a tuple of integers; you gave ";
           string_of_term ints;
           " = ";
           string_of_term (debracket (eval ints));
         ])

(* With the modern prooftree, LAYOUT is a simple assignment to a tip *)

let rec doLAYOUT layout eval action t (Proofstate { tree; goal } as state) =
  let parseSTR t =
    ...
    | _ ->
        raise
          (Tacastrophe_
             [ "label string expected in LAYOUT, found "; string_of_term t ])
  in
              ...
              ( ( match debracket t with
                ...
                | _ ->
                    raise
                      (Tacastrophe_
                         [
                           "in format in LAYOUT \"%s\" matches argument ";
                           string_of_term t;
                         ]) )
              :: rs )
  ...
  let rec parsefmt fmt =
    ...
    | Literal (_, String s) -> s
    | Tup (_, ",", t :: ts) ->
        parsetuplefmt (UTF.utf8_explode (parseSTR t)) ts []
    | _ ->
        raise
          (Tacastrophe_
             [
               "format in LAYOUT must be string or non-empty tuple of strings; \
                you gave ";
               string_of_term fmt';
             ])
  in
...
let rec doWITHCONCSEL try__ =
  ...
  | Some (_, c), (matching, checker, ruler, filter, taker, selhyps, selconcs) ->
      (matching, checker, ruler, filter, taker, selhyps, c :: selconcs)
  | _ -> try__

let rec doWITHHYPSEL try__ =
  match (getselectedhypotheses (), try__) with
  | Some (_, hs), (matching, checker, ruler, filter, taker, selhyps, selconcs)
    ->
      (matching, checker, ruler, filter, taker, hs @ selhyps, selconcs)
  | _ -> try__

let rec doMATCH (matching, checker, ruler, filter, taker, selhyps, selconcs) =
...
let rec doUNIQUE (matching, checker, ruler, filter, taker, selhyps, selconcs) =
  (matching, checker, ruler, filter, takeonlyone, selhyps, selconcs)

let rec doANY (matching, checker, ruler, filter, taker, selhyps, selconcs) =
  (matching, checker, ruler, filter, takefirst, selhyps, selconcs)

let rec doRESOLVE (matching, checker, ruler, filter, taker, selhyps, selconcs) =
...
*)
let rec doREPLAY (matching, checker, ruler, filter, taker, selhyps, selconcs) =
...
let rec doDO f state =
  ...
  if isproven state then Some state
    ...
    match
      try f state with
      | Time'sUp_ res -> raise (Time'sUp_ res)
...
let rec doSEQ f ts st =
  match ts with [] -> Some st | t :: ts -> (f t &~ doSEQ f ts) st
...
let doCUTIN f (Proofstate { tree; goal; cxt } as state) =
  if !cutindebug then
    consolereport
      [
        "CUTIN tree = ";
        ...
        "\nat goal = ";
        string_of_option string_of_fmtpath goal;
        " = ";
  ...
  try
    ...
    let (Proofstate { tree = tree''; cxt = cxt'' } as state'') =
      ...
      match
        apply unifyvarious nofilter takeonlyone [] []
          (expandstuff cutrule info state)
          (string_of_name cutrule) cxt'
          (withtree state'
    ...
    let cuthypel =
      match
        listsub sameresource (explodeCollection hs') (explodeCollection hs)
      with
      | [ c ] -> c
    ...
    let cutconcel =
      match
        listsub sameresource (explodeCollection cs') (explodeCollection cs)
      with
      | [ c ] -> c
    ...
    let l, r =
      ...
      | Element (_, rc, _), Element (_, rh, _) ->
          if (not (isProperResnum rc)) || not (isProperResnum rh) then bang ()
          else
    ...
    in
    let cuthypterm =
      try _The (term_of_element cuthypel)
      with None_ ->
        raise
          (Catastrophe_
             [
               "tacticfuns.doCUTIN can't make term of ";
               debugstring_of_element string_of_term cuthypel;
             ])
    in
    let provisos' =
      (function
        | true, false, v -> Provisotype.NotinProviso (v, cuthypterm)
        | b, r, v ->
            showAlert
              [
                "doCUTIN sees fresh name ";
                string_of_triple string_of_bool string_of_bool string_of_term
                  "," (b, r, v);
              ];
    ...
    optf
      (fun s ->
        let s' = withgoal s goal in
        if startAtTip then nextGoal false s' else s')
      (f
         (withgoal
            (withtree (withcxt state'' cxt''') wholeproof')
            (Some (subgoalPath wholeproof' (parentPath wholeproof' path) [ -l ]))))
    (* that's minus ell, not minus one *)
  with FollowPath_ stuff ->
    showAlert
      [
...
let rec _CanApply triv name state =
  if triv then []
    ...
    try
      let cxt, ((_, _, thing) as stuff) = getapplyinfo name [] (getcxt state) in
...
let rec autoStep a1 a2 a3 =
  ...
  | triv, rulenames, (state : proofstate) -> (
      let rec _Applicable (name, cs) =
        ...
        | [] -> cs
        | hypmatches ->
            nj_fold (fun (match__, cs) -> (name, match__) :: cs) hypmatches cs
      in
      ...
      | cs -> (
          ...
          match
            askChoice
              ("Choose a rule from these applicable rules:", showRule <* cs)
          with
          | None -> None
          | Some n ->
...
let rec forceUnify ts (Proofstate { cxt } as state) =
  match ts with
  | t1 :: t2 :: ts -> (
      let rec showit t =
        if t = rewrite cxt t then string_of_term t
          ...
          ( ( string_of_term t
            ^ " (which, because of earlier unifications, is equivalent to " )
          ^ string_of_term (rewrite cxt t) )
          ^ ")"
      in
      let rec bad reasons =
        setReason
          ("can't unify " :: showit t1 :: " with " :: showit t2 :: reasons);
        None
      in
      badunify := None;
      badmatch := None;
      ...
      try
        ...
        | None ->
            badunify := Some (t1, t2);
            bad []
      with Verifyproviso_ p ->
        badproviso := Some ((t1, t2), p);
        bad [ " because proviso "; string_of_proviso p; " is violated" ] )
  | _ -> Some state

let rec doDropUnify target sources (Proofstate { cxt } as state) =
  if !tactictracing then consolereport [ "** start doDropUnify" ];
  let rec bad reasons =
    setReason
      ( "can't drop "
      :: bracketed_string_of_list string_of_element "," sources
      :: " into " :: string_of_element target :: reasons );
    None
  in
  try
    match
      dropunify (target, sources) cxt
...
let rec associativelaw operator thing =
    ...
    try
      ...
      if !_FINDdebug then
        consolereport
          [
            "assoc ";
            bracketed_string_of_list string_of_paraparam ", " params;
            " (";
            string_of_term term;
            ") => lf=";
            bracketed_string_of_list string_of_term ", " lf;
            "; rf=";
            bracketed_string_of_list string_of_term ", " rf;
          ];
      (List.length lf = 3 && lf = rf) && all (formulageneralisable params) rf
    with Matchinassociativelawstuff_ -> false
  in
  let rec ok params seq =
    match seq with
    | Seq (_, Collection (_, _, []), Collection (_, _, [ Element (_, _, _C) ]))
      ->
        assoc params _C
    | Seq
        ( _,
          Collection (_, _, [ Segvar _ ]),
  ...
  if !_FINDdebug then
    consolereport
      [
        "associativelaw (";
        string_of_term operator;
        ") (";
        string_of_thing thing;
        ")";
      ];
  match thing with
...
module Assoccache = Cache.F (struct
  type dom = vid
  ...
  let eval operator =
    if !_FINDdebug then
      consolereport
        [
          "allrelevantthings()=";
          bracketed_string_of_list parseablestring_of_name ", "
            (relevant <| thingnames ());
        ];
    List.exists
      (associativelaw (registerId (operator, conOperatorClass)))
      (allrelevantthings ())
...
let rec _LeftFold f ts =
  let rec _Fold a1 a2 =
    match (a1, a2) with r, [] -> r | r, t :: ts -> _Fold (f r t) ts
  in
  match ts with
  | [] ->
      registerId
...
let rec rator t =
  match debracket t with Id _ -> Some t | App (_, l, _) -> rator l | _ -> None
...
let rec assocInfo = function
  | Id (_, c, _) as f ->
      let curried, lassoc =
        try _The (lookupassoc (string_of_vid c))
        with None_ -> raise MatchinAssocInfo
      in
      if !_FINDdebug then
        consolereport
          [
            "assocInfo (";
            debugstring_of_term f;
            ") = (";
            string_of_bool curried;
            ", ";
...
let rec _AssocFlatten pat t =
  ...
  | _ ->
      if !_FINDdebug then
        consolereport
          [ string_of_term pat; " isn't a recognisable operator / application" ];
      None
...
let _ARITHMETIC _C cxt opn invopn (e1 : term) (e2 : term) (e3 : term)
    (neg : term) =
  ...
  let rec result (e, neg) n =
    ...
    match _UnifyWithExplanation "EVALUATE _ARITHMETIC" (e, mkLit n) cxt with
  ...
  | ArithNumber v1, ArithNumber v2, ArithVariable e3 -> result e3 (opn (v1, v2))
...
let rec _DECIDE (turnstile : string) (cxt : Cxttype.cxt) (_HS : term)
    (_CS : term) (oracle : string) (args : string list) =
...
exception MatchinEvaluate

exception MatchinTtoV

(* spurious *)

let rec explodeEval _C =
...
let rec _Evaluate cxt seq =
  ...
  | Seq (turnstile, _HS, Collection (_, _, [ Element (_, _, _C) ])) -> (
      ...
      | "ASSOCEQ", [ source; target ] -> (
          ...
          match
            _UnifyWithExplanation "EVALUATE ASSOCEQ"
              (flat source, flat target)
              cxt
          with
      ...
      | "DECIDE", conclusion :: oracle :: args ->
          _DECIDE turnstile cxt _HS conclusion (string_of_term oracle)
            (string_of_term <* args)
      | _ ->
          setReason
  ...
  | Seq
      ...
      | "DECIDEBAG", oracle :: args ->
          _DECIDE turnstile cxt _HS
            (registerCollection (idclass, _CS))
...
let doEVAL args = function
  | Proofstate { cxt; goal = Some goal; tree } as state -> (
...
let rec _FIRSTSUBTERM f t =
  let rec _S a1 a2 =
    ...
    | t :: front, rear -> (
        match f t with None -> _S front (appendsubterms t rear) | r -> r )
  in
  _S [ t ] []
...
let rec tranformals formals =
  try _The <.> nameopt_of_term <.> paramvar <* formals
  with _The_ ->
    raise
      (Catastrophe_
         [
...
let rec mkenvfrommap params argmap =
  let rec mk ((v, t), e) =
    try e ++ (_The (nameopt_of_term v) |-> t)
    with _The_ ->
...
let rec unfix_ConcsOf _ConcsOf n =
  if !_FOLDdebug then
    consolereport
      [
        ...
        string_of_name n;
        " (=> ";
        string_of_option (string_of_thing <.> fst) (thingnamed n);
        ")";
      ];
  ...
  | Some ((Theorem (_, _, Seq (_, _, _CS)) as t), _) -> (
      ...
      | Collection (_, _, [ Element (_, _, _C) ]) -> [ (n, t, _C) ]
      | _ -> [] )
  | Some (Tactic (_, spec), _) -> _ConclusionsofTactic (emptyenv ()) (spec, [])
  | Some (Macro (_, spec), _) ->
      setReason [ "Only a macro is named "; string_of_name n; " (in _ConcsOf)" ];
      []
...
and _ConclusionsofTactic env (spec, l) =
    ...
    consolereport
      [
        ...
        string_of_pair string_of_tactic
          (bracketed_string_of_list
             (string_of_triple parseablestring_of_name string_of_thing
                string_of_term ",")
             ",")
          ", " (spec, l);
      ];
  ...
  | TheoryAltTac ns ->
      nj_fold (fun (n, l) -> _ConclusionsofthingNamed (evalname env n, l)) ns l
  | _ ->
      setReason [ "Bad FOLD/UNFOLD argument" ];
      l
...
and _ConclusionsofthingNamed (n, l) =
  let ts = _ConcsOf n in
  ts @ l

let rec resetcaches () =
  resetconclusioncache ();
  resetassociativecache ()
...
let rec strsep t =
  match debracket t with Literal (_, String s) -> s | _ -> string_of_term t
      ...
      listf fs ts
        ( ( match debracket t with
          ...
          | Tup (_, ",", [ Tup (_, _, ts); sep1; sep2 ]) ->
              sentencestring_of_list string_of_term (strsep sep1) (strsep sep2)
                ts
          | _ -> string_of_term t )
        :: r )
  | 0x25 :: 0x73 :: fs, t :: ts, r -> (* %s *) listf fs ts (message t :: r)
  | 0x25 :: 0x74 :: fs, t :: ts, r ->
      (* %t *) listf fs ts (string_of_term t :: r)
...
let rec doJAPE tryf display env ts
    (Proofstate { cxt; goal; target; tree; givens; root } as state) =
  ...
  let rec _Tickmenu on menu label state =
    try
      ...
      in
      tickmenuitem true (_V menu) (_V label) on;
      Some state
    with BadMatch_ ->
      setReason ([ "Badly-formed JAPE tactic: " ] @ (string_of_term <* ts));
      None
  in
  (* function is Term to Integer List *)
  let rec _TtoIL = function
    | Literal (_, Number n) -> [ n ]
    | Tup (_, ",", ts) -> ((fun (x, y) -> x @ y), []) </ (_TtoIL <* ts)
    | _ -> raise MatchinTtoIL_
  in
  let bang () =
    setReason
      [
        "Badly-formed tactic: JAPE("; string_of_list string_of_term "," ts; ")";
      ];
    None
  in
  match ts with
  ...
  | t :: _ -> (
      ...
      | Id (_, v, _), ts -> (
          ...
          | "write", [ t ] ->
              ...
              Some state
          | "showreason", [] ->
              showAlert ("Reason is: " :: explain "");
              Some state
          | "cachereset", _ ->
              (* This should be done automatically when a theory-search button
                 changes its state (says Bernard); or we should get rid of caches (says Richard)
              *)
              resetcaches ();
              Some state
          | "tick", [ menu; label ] -> _Tickmenu true menu label state
          | "untick", [ menu; label ] -> _Tickmenu false menu label state
          | "tactic", [ dest; tactic ] -> (
              try
                ...
                addthing (name, Tactic (params, transTactic tactic), InLimbo);
                (* cleanup(); (*DO ME SOON*)*)
                resetcaches ();
                Some state
              with MatchinJAPEparam_ ->
                setReason [ "Badly-formed parameters for JAPE(set)" ];
                None )
          | "GROUND", [ t ] ->
              let vs = termvars (adhoceval t) in
              if List.exists isUnknown vs then None else Some state
          | "UNKNOWN", [ t ] -> (
              match adhoceval t with Unknown _ -> Some state | _ -> None )
          | _ -> bang () )
      | _ -> bang () )

let rec doFLATTEN env f (Proofstate { cxt; goal; tree } as state) =
  ...
  | Seq (st, _HS, Collection (_, collc, [ Element (_, resn, _C) ])) as seq -> (
      ...
      | Some _C' ->
          ...
          *)
          if _C = _C' then
            (* eqterms is modulo bracketing, so is not to be used here! *)
            Some state
          else
            proofstep cxt
              (mkJoin cxt "FLATTEN"
                 (UnRule ("FLATTEN", []))
                 [ f ] Treeformat.Fmt.neutralformat seq
                 [
                   mkTip cxt
                     (rewriteseq cxt
                        (Seq
              ...
              state
      | None ->
          setReason [ "Cannot FLATTEN "; string_of_term f ];
          None )
  | _ ->
      setReason [ "FLATTEN with multiple-conclusion sequent" ];
      None

(**********************************************************************)

let rec trace string_of_termarg name args (Proofstate { cxt; goal; tree }) =
  if !tactictracing then
    consolereport
      [
        parseablestring_of_name name;
        " ";
        string_of_termarg cxt args;
        ( match goal with
        | None -> ""
        | Some path ->
            ( " [["
            ^ string_of_seq (rewriteseq cxt (sequent (followPath tree path))) )
            ^ "]]" );
      ]

let tracewithargs =
  trace (fun cxt -> string_of_list (string_of_termarg <.> rewrite cxt) " ")
...
let rec applyBasic name ((env, _, thing) as stuff)
  ...
  if currentlyProving name then (
    setReason
      [
        "But "; parseablestring_of_name name; " is what you're trying to prove!";
      ];
    None )
  else if (not (applyAnyway thing)) && lacksProof name then (
    setReason [ parseablestring_of_name name; " is unproved" ];
    None )
  else
    ruler checker filter taker selhyps selconcs
      (expandstuff name stuff state)
      reason cxt state
...
let tryApplyOrSubst dispatch getit mke con trace display try__ contn name args
    (Proofstate { cxt; tree; givens; goal; target; root } as state) =
  checkTimer state;
  ...
  | argmap, _, _ ->
      (* must it be a rule? *)
      let rec showit () =
        let _, wholetree = makewhole cxt root tree (rootPath tree) in
        ...
        let _ =
          ( showProof display wholetarget wholegoal cxt wholetree !autoselect
            : displaystate )
        in
        ()
      in
...
let rec newpath tacstr eval (Proofstate { tree }) p =
...
let rec dispatchTactic display try__ env contn tactic
  ...
  | SeqTac ts ->
      contn (doSEQ (dispatchTactic display try__ env nullcontn) ts state)
  | AltTac ts ->
      findfirst (fun t -> dispatchTactic display try__ env contn t state) ts
  | TheoryAltTac ns ->
      dispatchTactic display try__ env contn
        (AltTac ((fun n -> TermTac (n, [])) <* ns))
        state
  | RepTac t ->
      contn (doDO (dispatchTactic display try__ env nullcontn t) state)
  | IfTac t ->
      contn (doREPEAT1 (dispatchTactic display try__ env nullcontn t) state)
  | LayoutTac (t, l) ->
      doLAYOUT l (eval env) (dispatchTactic display try__ env contn) t state
  | CompTac t -> doCOMPLETE (dispatchTactic display try__ env contn t) state
  | EvalTac ts -> contn (doEVAL ts state)
  | FoldHypTac (name, stuff) ->
      doFOLDHYP name display try__ env contn (evalname env name, stuff) state
  | UnfoldHypTac (name, stuff) ->
      doUNFOLDHYP name display try__ env contn (evalname env name, stuff) state
  | UnfoldTac (name, stuff) ->
      doUNFOLD name display try__ env contn (evalname env name, stuff) state
  | FoldTac (name, stuff) ->
      doFOLD name display try__ env contn (evalname env name, stuff) state
  | AssignTac spec -> contn (doASSIGN env spec state)
  | UnifyTac terms -> contn (forceUnify (eval env <* terms) state)
  | UnifyArgsTac -> (
      getargs "UNIFYARGS" &~~ function
      | [ t ] -> (
          match Japeserver.askUnify (string_of_term t) with
  ...
  | AdHocTac ts ->
      contn
        (doJAPE
           (dispatchTactic display try__ env nullcontn)
           display env ts state)
  | CutinTac t ->
  ...
  | BindRHSTac _ -> contn (stripoption (doBIND tactic display try__ env state))
  | BindGoalTac _ -> contn (stripoption (doBIND tactic display try__ env state))
  | BindGoalPathTac _ ->
      contn (stripoption (doBIND tactic display try__ env state))
  ...
  | BindOpenSubGoalsTac _ ->
      contn (stripoption (doBIND tactic display try__ env state))
  | BindConcTac _ -> contn (stripoption (doBIND tactic display try__ env state))
  | BindHypTac _ -> contn (stripoption (doBIND tactic display try__ env state))
  | BindHyp2Tac _ -> contn (stripoption (doBIND tactic display try__ env state))
  | BindHypsTac _ -> contn (stripoption (doBIND tactic display try__ env state))
  | BindArgTac _ -> contn (stripoption (doBIND tactic display try__ env state))
  | BindArgTextTac _ ->
      contn (stripoption (doBIND tactic display try__ env state))
  | BindSubstTac _ ->
      contn (stripoption (doBIND tactic display try__ env state))
  ...
  | BindOccursTac _ ->
      contn (stripoption (doBIND tactic display try__ env state))
  | BadUnifyTac _ -> contn (stripoption (doBIND tactic display try__ env state))
  | BadMatchTac _ -> contn (stripoption (doBIND tactic display try__ env state))
  | BadProvisoTac _ ->
      contn (stripoption (doBIND tactic display try__ env state))
        ...
        with ParseError_ ss ->
          raise
            (Tacastrophe_
               ...
               :: string_of_term t' :: ", which isn't a valid tactic, because "
               :: ss ))
      in
      match env <@> str with
                  ...
                  raise
                    (Tacastrophe_
                       [
                         ...
                         " -- you can't make tactics 'on the fly' that way";
                       ]) )
          | [] -> (
  ...
  | SubstTac (name, vts) ->
      trySubst display try__ contn (evalname env name) (evalvt env <* vts) state
  ...
  | WithSubstSelTac t ->
      if withabletac tactic env t then
        doWITHSUBSTSEL display try__ state (fun try__ ->
            dispatchTactic display try__ env contn t)
  ...
  | WithArgSelTac t ->
      ...
      dispatchTactic display try__ env contn (extendwithselection env t) state
  | WithConcSelTac t ->
      if withabletac tactic env t then
  ...
  | WithFormSelTac t ->
      if withabletac tactic env t then
        dispatchTactic display
          (doWITHHYPSEL (doWITHCONCSEL try__))
          env contn t state
      else dispatchTactic display try__ env contn t state
  | WithSelectionsTac t ->
      if withabletac tactic env t then
        dispatchTactic display
          (doWITHHYPSEL (doWITHCONCSEL try__))
          env contn
          (extendwithselection env t)
          state
      else dispatchTactic display try__ env contn t state
  | MatchTac t -> dispatchTactic display (doMATCH try__) env contn t state
  | SameProvisosTac t ->
  ...
  | UniqueTac t -> dispatchTactic display (doUNIQUE try__) env contn t state
  | TakeAnyTac t -> dispatchTactic display (doANY try__) env contn t state
  | ResolveTac t -> dispatchTactic display (doRESOLVE try__) env contn t state
  | ReplayTac t ->
      let noticetimewas =
        let r = !noticetime in
        noticetime := false;
      ...
      let r = dispatchTactic display (doREPLAY try__) env contn t state in
      noticetime := noticetimewas;
      triesleft := triesleftwas;
      r
  | ContnTac (t1, t2) ->
      dispatchTactic display try__ env
        (function
          | Some s -> dispatchTactic display try__ env contn t2 s | None -> None)
        t1 state
  | ExplainTac m ->
      setReason [ message (rewrite cxt (eval env m)) ];
      Some state
  | CommentTac m ->
      setComment [ message (rewrite cxt (eval env m)) ];
      Some state
  | AlertTac (m, ps) ->
      contn
        (doALERT
           ...
           m ps state)
  | ShowHowToTac s ->
      Alert.showHowTo (message (eval env s));
      contn (Some state)
  | NextgoalTac -> contn (Some (nextGoal true state))
  | SetgoalTac p ->
      contn
        (Some (withgoal state (Some (newpath "GOALPATH" (eval env) state p))))
...
and tryApply display =
  tryApplyOrSubst dispatchTactic getapplyinfo mkenv
    ...
    tracewithargs display

and trySubst display =
  tryApplyOrSubst dispatchTactic (getsubstinfo true) mkenvfrommap
    (fun v -> SubstTac v)
    tracewithmap display

and tryGiven display
    (matching, checker, ruler, filter, taker, selhyps, selconcs (* as try__ *))
    i (Proofstate { cxt; givens } as state) =
  let i =
    try int_of_term i with _ -> raise (Tacastrophe_ [ "not an integer" ])
  in
  let given =
    try Listfuns.guardednth givens i
  ...
  let stuff =
    ( "given",
      hiddencontexts givens,
      Prooftree.Tree.Given (name, i, false),
      Mappingfuns.empty,
      principals,
      [],
      given,
...
and doMAPTERMS display try__ contn name args
    (Proofstate { cxt; goal; tree } as state) =
  match rewriteseq cxt (getTip tree goal) with
  ...
  | _ ->
      setReason [ "MAPTERMS with multiple-conclusion sequent" ];
      None
...
and doWITHSUBSTSEL display try__
    (Proofstate { cxt; goal; tree; givens; target; root }) tacfun =
  ...
  | Some (path, concsels, hypsels, givensel), Some g -> (
      let rec doit ishyp selel selss =
        try
          ...
          in
          let alteredstate =
            Proofstate { cxt = cxt'; tree = tree'; givens; goal; target; root }
          in
          tacfun
            ( match try__ with
            | matching, checker, ruler, filter, taker, selhyps, selconcs ->
                if ishyp then
                  (matching, checker, ruler, filter, taker, [ newel ], selconcs)
                else
                  (matching, checker, ruler, filter, taker, selhyps, [ newel ])
            )
            alteredstate
        with Selection_ ss ->
          setReason ("WITHSUBSTSEL failed: " :: ss);
          None
      in
      ...
      | [], [] ->
          setReason [ "WITHSUBSTSEL applied without a proof text selection" ];
          None
      | _ ->
          setReason
  ...
  | None, _ ->
      ...
      None
  | _, None -> raise (Catastrophe_ [ "WITHSUBSTSEL applied without a goal" ])
...
and doUNFOLD name display try__ env contn (tactic, laws)
  ...
  | Seq (_, _HS, Collection (_, collc, [ Element (_, resn, _C) ])) ->
      ...
      findfirst
          ...
          if !_FOLDdebug then
            consolereport
              [
                ...
                string_of_term term;
                "\" - now to try ";
                parseablestring_of_name tactic;
                " and then ";
                parseablestring_of_name name;
              ];
        ...
        lhss
  | _ ->
      setReason [ "UNFOLD with multiple-conclusion sequent" ];
      None
...
and doFOLD name display try__ env contn (tactic, laws)
  ...
  | Seq (_, _HS, Collection (_, collc, [ Element (_, resn, _C) ])) ->
      ...
      findfirst
        ...
        rhss
  | _ ->
      setReason [ "FOLD with multiple-conclusion sequent" ];
      None

and doFOLDHYP name display try__ env contn (tactic, patterns)
    (Proofstate { cxt; goal; tree; givens; target; root } (* as state *)) =
  let _, cxt, env, patterns = freshenv patterns cxt env in
  let patterns = eval env <* patterns in
  match rewriteseq cxt (getTip tree goal) with
  | Seq (_, _HS, Collection (_, collc, [ Element (_, resn, _C) ])) -> (
      let rec _HypMatches = function
        | Element (_, _, _Ht) as _H -> (
            match explodeApp false _Ht with
            | _, [ Tup (_, ",", [ _; rhs ]) ] ->
                findfirst
                  (fun pat ->
                    match (unifyterms (pat, _Ht) &~ checkprovisos) cxt with
                    | None -> None
                    | Some _ ->
                        _FIRSTSUBTERM
                          (fun subterm ->
                            match
                              (unifyterms (subterm, rhs) &~ checkprovisos) cxt
                            with
      ...
      | Some (cxt, term) ->
          tryApply display try__ contn tactic [ term ]
            (Proofstate { cxt; goal; target; tree; givens; root }) )
  | _ ->
      setReason [ "FOLDHYP with multiple-conclusion sequent" ];
      None
  ...
  | Seq (_, _HS, Collection (_, collc, [ Element (_, resn, _C) ])) -> (
                  ...
                  (fun pat ->
                    ...
                    | Some _ ->
                        _FIRSTSUBTERM
                          (fun subterm ->
                            match
                              (unifyterms (subterm, lhs) &~ checkprovisos) cxt
                            with
                            | Some cxt -> Some (cxt, subterm)
                            | None -> None)
                          _C)
      ...
      | Some (cxt, term) ->
          tryApply display try__ contn tactic [ term ]
            (Proofstate { cxt; goal; target; tree; givens; root }) )
  | _ ->
      setReason [ "UNFOLDHYP with multiple-conclusion sequent" ];
      None

and doBIND tac display try__ env (Proofstate { cxt; goal; tree } as state) =
  let matching = fst_of_7 try__ in
  let rec newenv cxt env newformals =
    nj_fold
      (fun (formal, env) ->
        env ++ (formal |-> rewrite cxt (_The (env <@> formal))))
      newformals env
  in
  ...
  let checkmatching cxt cxt' expr =
    ...
    || matchedtarget cxt cxt'
         (nj_fold
            (function Unknown (_, u, _), uvs -> u :: uvs | _, uvs -> uvs)
            (termvars expr) [])
  in

  let rec bindunify bad badp = function
    | [] -> _Some
    | ((_, expr) as pe) :: pes -> (
        fun cxt ->
          badunify := None;
          ...
          | Some cxt' -> (
              try
                ...
                if checkmatching cxt cxt' expr then bindunify bad badp pes cxt'
                else (
                  badmatch := Some pe;
                  bad [ " because the match changed the formula" ] )
              with Verifyproviso_ p ->
                badproviso := Some (pe, p);
                badp p ) )
  in
  ...
  let bind s cxt env pes =
    ...
    | None -> None
    | Some cxt' ->
        (* Notice that the consequences of the unification do not leak
  ...
  let rec bindThenDispatch s cxt env pes tac =
    bind s cxt env pes &~~ fun (cxt', env') ->
    Some (dispatchTactic display try__ env' nullcontn tac (withcxt state cxt'))
  in
  ...
  let rec doublesel spec ishyp s =
    ...
    | Some (_, _, [], _), true ->
        setReason [ s; " failed -- no selected hypothesis text" ];
        None
    | Some (_, _, _, _), true ->
        setReason [ s; " failed -- too much selected hypothesis text" ];
        None
    | None, _ ->
        setReason [ s; " failed -- no selected text" ];
        None
  in
  let rec findsel spec ishyptactic tacname =
    let rec doit selpath selel ss =
      ...
      | [ b; middle; a ] ->
          ...
          let rec pe ss =
            raise
              (ParseError_
                 ( [ tacname; ": the term ("; newtext; ") can't be parsed." ]
                 @ ss ))
          in
          (* unused
             let midterm =
             ...
             in *)
          let newterm =
            try term_of_string newtext with ParseError_ ss -> pe ss
          in
          let oldterm =
            try term_of_string (b ^ middle ^ a)
            with ParseError_ ss ->
              raise
                (Catastrophe_
                   ( [
                       tacname; " -- can't parse original term "; oldtext; " -- ";
                     ]
                   @ ss ))
          in
          if eqterms (oldterm, newterm) then
            (* Succeed, but don't change the state *)
            Some (Some state)
          else
            checkBIND tacname
    ...
    | Some (path, [], [ (_, hel, hss) ], _), false ->
        ...
        None
    | _ ->
        setReason [ tacname; ": requires selection in exactly one formula" ];
        None
  in
  ...
  let bindOccurs (pat1, term, pat2, tac) =
    ...
    let pat1 = List.hd pats in
    (* easiest way to shut up the complaint about matching [pat1] above *)
    ...
    | None ->
        setReason
          [
            "LETOCCURS failed to find pattern";
            string_of_term pat1;
            " in ";
            string_of_term expr;
          ];
  ...
  let rec term_of_seqside colln =
    try
      let ts = _The <.> term_of_element <* explodeCollection colln in
      (* I wish I didn't have to know how termparse does it, but I do ... *)
      match ts with
      | [ t ] ->
          t (* If I don't write this I can never match a single-term lhs/rhs *)
  ...
  let rec opensubgoals () =
    let tiprhss =
      pathPrefix tree (getGoalPath goal) <.> fst <| allTipConcs tree
    in
    (* the tips relevant to us, with their paths *)
    let tipterms =
      (fun (path, rhss) ->
        (path, _The <* (bool_of_opt <| (term_of_element <* rhss))))
      <* tiprhss
    in
    List.concat
      ((fun (path, terms) -> (fun term -> (path, term)) <* terms) <* tipterms)
  in
  ...
  | BindConcTac spec ->
      let topt =
        getselectedconclusion () &~~ function
  ...
  | BindHyp2Tac (pat1, pat2, tac) -> (
      ...
      | Some (_, [ Element (_, _, h1); Element (_, _, h2) ]) ->
          ( bind "LETHYP2" cxt env [ (pat1, h1); (pat2, h2) ] |~~ fun _ ->
            bind "LETHYP2" cxt env [ (pat1, h2); (pat2, h1) ] )
          &~~ fun (cxt', env') -> bindThenDispatch "LETHYP2" cxt' env' [] tac
      | _ ->
          setReason [ "LETHYP2 with not exactly two selected hypotheses" ];
          None )
      ...
      | Some (_, (_ :: _ as hs)) ->
          checkBIND "LETHYPS"
            ( cxt,
              ...
              Some
                (registerTup
                   (",", rewrite cxt <.> _The <.> term_of_element <* hs)),
              spec )
      | _ ->
          setReason [ "LETHYPS with no selected hypothesis/es" ];
          None )
  ...
  *)
  | BindTuplistTac (pat1, pat2, tup, tac) -> (
      ...
      | Tup (_, ",", t :: ts) ->
          bind "LETLISTMATCH" cxt env
            [ (pat1, t); (pat2, registerTup (",", ts)) ]
          &~~ fun (cxt', env') ->
          bindThenDispatch "LETLISTMATCH" cxt' env' [] tac
      | Tup (_, ",", []) ->
          setReason [ "LETLISTMATCH given empty tuple" ];
          None
      | _ as t ->
          setReason [ "LETLISTMATCH can't match non-tuple "; string_of_term t ];
          None )
  | BindArgTac spec -> (
  ...
  | BindSubstTac spec -> (
      let rec doit ss =
        ...
        with Selection_ ss ->
          setReason ("LETSUBSTSEL failed: " :: ss);
          None
      in
      match getunsidedselectedtext () with
      ...
      | Some (_, [], [], _) ->
          ...
          None
      | Some (_, _, _, _) ->
          setReason [ "LETSUBSTSEL failed: too many proof text selections" ];
          None
      | None ->
          setReason [ "LETSUBSTSEL failed: no selections" ];
          None )
  ...
  | BindMultiArgTac spec -> (
      getargs "LETMULTIARG" &~~ function
      | [ t ] ->
          setReason [ "LETMULTIARG failed: only one text selection" ];
          None
      | ts ->
          checkBIND "LETMULTIARG" (cxt, env, Some (registerTup (",", ts)), spec)
      )
  | BindLHSTac spec -> (
      (* LETLHS/RHS don't insist on a tip, nor a single element *)
      match rewriteseq cxt (sequent (followPath tree (getGoalPath goal))) with
      | Seq (_, hs, _) ->
  ...
  | BindGoalTac spec -> (
      ...
      match
        try Some (rewriteseq cxt (getTip tree goal)) with findTip_ -> None
      with
      | Some (Seq (_, _HS, Collection (_, collc, [ Element (_, resn, _C) ]))) ->
          checkBIND "LETGOAL" (cxt, env, Some _C, spec)
      | Some _ ->
          setReason [ "LETGOAL with non-single conclusion" ];
          None
      | None ->
          setReason [ "LETGOAL at non-tip position" ];
          None )
      ...
      | [ (FmtPath ns, conc) ] ->
          checkBIND "LETOPENSUBGOAL"
            ( cxt,
              ...
              Some (rewrite cxt conc),
              (pat, tac) )
      | [] ->
          setReason [ "LETOPENSUBGOAL with no unproved conclusions" ];
          None
      | _ ->
          setReason [ "LETOPENSUBGOAL with more than one unproved conclusion" ];
          None )
  | BindOpenSubGoalsTac spec -> (
      ...
      | [] ->
          ...
          None
      | [ _ ] ->
          setReason [ "LETOPENSUBGOALS with only one unproved conclusion" ];
          None
      | tips ->
          checkBIND "LETOPENSUBGOALS"
            ( cxt,
              env,
              Some (registerTup (",", rewrite cxt <.> snd <* tips)),
              spec ) )
...
let rec runTactic display env try__ tac (Proofstate { goal } as state) =
  noticetime := true;
...
let rec applyLiteralTac display env try__ text state =
  runTactic display env try__
    (firstextend env (transTactic (tactic_of_string text)))
    state
...
let rec errorcatcher f mess text state =
  try
    match f state with
    | None ->
        showAlert (explain (text ()));
        None
    | res ->
        if time'sUp () then showAlert (explain (text ()));
        res
  with
  | Tacastrophe_ ss ->
      showAlert (("Error in tactic: " :: ss) @ [ " "; mess () ]);
      None
  | Catastrophe_ ss -> raise (Catastrophe_ (ss @ [ " "; mess () ]))
  | ParseError_ ss ->
      showAlert (("Parse error: " :: ss) @ [ " "; mess () ]);
      None
  | AlterProof_ ss ->
      showAlert (("AlterProof_ error: " :: ss) @ [ " "; mess () ]);
      None
  | StopTactic_ -> None
  | Time'sUp_ res -> res
  | exn -> raise exn
...
let rec autoTactics display env rules
    (Proofstate { tree; goal = oldgoal } as state) =
  let rec tryone matching tac goal
      (Proofstate { cxt; tree; givens; root } (* as state *)) =
    let rec bad ss =
    ...
    try
      match
        runTactic display env
          ...
          tac
          (Proofstate
             { cxt; tree; givens; root; goal = Some goal; target = Some goal })
      with
      | None -> None
  ...
  | Some (Proofstate { tree = tree' } as state') ->
      (* round again *)
      autoTactics display env rules (nextGoal false (withgoal state' oldgoal))
  | None -> state
