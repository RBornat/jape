...
let rec rawinfresnum (r, ((vars, uVIDs, badres, psig) as raw)) =
  match r with ResUnknown i -> (vars, uVIDs, i :: badres, psig) | _ -> raw

let rec rawinfSeq n (Seq (st, _Hs, _Gs), stuff) =
  let _RIT = rawinfTerm n in
  _RIT (_Hs, _RIT (_Gs, stuff))
...
let rec rawinfElements n (es, stuff) =
  let rec _RE (e, stuff) =
    match e with
    | Segvar (_, ops, v) -> rawinfTerm n (v, nj_fold (rawinfTerm n) ops stuff)
...
let rec rew_worthwhile subst cxt ri =
  ( ( subst
    ...
    | _ -> false )
  || List.exists
       (bool_of_opt <.> fun uVID -> varmap cxt <@> uVID)
       (rewinf_uVIDs ri) )
  || List.exists (bool_of_opt <.> fun i -> resmap cxt <@> i) (rewinf_badres ri)

(* this is almost f &~ (Some o yes), but types get in the way ... *)

let rec rew_ f x yes = match f x with Some x' -> Some (yes x') | _ -> None

(* this is Some o anyway f *)

let rec rew_yes f t = match f t with None -> Some t | some -> some

let rec rew_2 f x g y yes = (option_rewrite2 f g &~ (_Some <.> yes)) (x, y)

let rec rew_Pair _R = option_rewrite2 _R _R
...
let rec rew_binding outer inner (h, (bs, ss, us), env, pat) =
  rew_3 (option_rewritelist outer) bs (option_rewritelist inner) ss
    (option_rewritelist outer) us (fun stuff ->
      registerBinding (stuff, env, pat))
...
and rew_Subst subst cxt (h, r, _P, m) =
  let simp = simplifySubst (facts (provisos cxt) cxt) in
  match
    rew_2 (rew_Term subst cxt) _P (rew_substmap subst cxt) m (fun (_P, m) ->
        if subst then
          match simp m _P with None -> registerSubst (r, _P, m) | Some t -> t
...
let rec rew_Seq subst cxt (Seq (st, _H, _G)) =
  rew_2 (rew_Term subst cxt) _H (rew_Term subst cxt) _G (fun (_H', _G') ->
      Seq (st, _H', _G'))
...
let rec rew_Proviso subst cxt p =
  ...
  | DistinctProviso vs ->
      rew_ (option_rewritelist _RT) vs (fun v -> DistinctProviso v)
  | NotoneofProviso (vs, pat, _C) ->
      rew_3 (option_rewritelist _RT) vs _RT pat _RT _C (fun v ->
          NotoneofProviso v)
...
let rec rew_cxt cxt =
  (* there is no point rewriting the substitutions inside the provisos
     ...
     * functions.
  *)
  let rec needsit subst cxt = function
    | Some inf -> rew_worthwhile subst cxt inf
    | None -> true
  in
  let rec doprovisos subst cxt =
    ...
    if needsit subst cxt pinf then
      let rec newrewinf n ps =
        Some
          (raw2rew_
             (nj_fold (rawinfProviso n) (provisoactual <* ps) nullrawinf))
      in
      ...
      let rec updatesig () =
        ...
        let rec g () = yes cxt ps in
        (* doesn't change ps, so doesn't cause incprovisosig *)
        rew_update f g pinf cxt
      in
      let rec rew_visproviso subst cxt vp =
        rew_
          (rew_Proviso subst cxt <.> provisoactual)
          vp (provisoresetactual vp)
      in
    ...
    | Exterior ((ss, s), inf, fvinf) ->
        if needsit subst cxt inf then
          let rec yes (ss, s) =
            ...
            let fvinf =
              ...
              | avs, [] ->
                  ...
                  let ps =
                    optionfilter
                      ( (function NotinProviso p -> Some p | _ -> None)
                      <.> provisoactual )
                      (provisos cxt)
                  in
                  ...
                  Some { avs; fvs; vmap = mkmap m; bhfvs; bcfvs }
              | _ -> None
            in
            Some
          ...
          let rec updatesig () =
            ...
            in
            let rec g _ = yes (ss, s) in
            rew_update f g inf cxt
          in
          match
            rew_2
              (option_rewritelist (rew_Seq subst cxt))
              ss (rew_Seq subst cxt) s idf
          with
          | Some r -> yes r
          | None -> updatesig ()
        else None
  ...
  if !rewritedebug then
    consolereport
      [
        "rew_cxt "; string_of_cxt cxt; " => "; string_of_option string_of_cxt r;
      ];
  r
...
let rec rewinfCxt cxt =
  ...
  | (_, Some pinf), Exterior (_, Some binf, _) -> rewinf_merge (pinf, binf)
  | (_, Some pinf), _ -> pinf
