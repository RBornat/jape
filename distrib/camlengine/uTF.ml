...
let hexstring_of_char c =
  enCharQuote ("x" ^ fixedwidth_hexstring_of_int 2 (Char.code c))
...
let utf8_next next =
  try
    let n0 = Char.code (next 0) in
    try
      let twobyte f =
        let n1 = f (next 1) in
        ((n0 land 0x1f) lsl 6) lor n1
      in
      let threebyte f =
        let n1 = f (next 1) in
        try
          let n2 = check_80bf (next 2) in
          ((((n0 land 0x0f) lsl 6) lor n1) lsl 6) lor n2
        with
        | Stream.Failure ->
            raise (MalformedUTF_ [ hexstring_of_char (Char.chr n1); " EOF" ])
        | MalformedUTF_ ss ->
            raise (MalformedUTF_ (hexstring_of_char (Char.chr n1) :: " " :: ss))
      in
      let fourbyte f =
        let n1 = f (next 1) in
        try
          let n2 = check_80bf (next 2) in
          try
            let n3 = check_80bf (next 3) in
            ((((((n0 land 0x07) lsl 6) lor n1) lsl 6) lor n2) lsl 6) lor n3
          with MalformedUTF_ ss ->
            raise (MalformedUTF_ (hexstring_of_char (Char.chr n2) :: " " :: ss))
        with
        | Stream.Failure ->
            raise (MalformedUTF_ [ hexstring_of_char (Char.chr n1); " EOF" ])
        | MalformedUTF_ ss ->
      ...
      in
      match n0 with
      | n when 0x00 <= n && n <= 0x7f -> n0
      | n when 0xc2 <= n && n <= 0xdf -> twobyte check_80bf
      | 0xe0 -> threebyte check_a0bf
      | n when (0xe1 <= n && n <= 0xec) || (0xee <= n && n <= 0xef) ->
          threebyte check_80bf
      | 0xed -> threebyte check_809f
      | 0xf0 -> fourbyte check_90bf
      | n when 0xf1 <= n && n <= 0xf3 -> fourbyte check_80bf
      | 0xf4 -> fourbyte check_808f
      | _ -> raise (MalformedUTF_ [])
    with
    | Stream.Failure ->
        raise (MalformedUTF_ [ hexstring_of_char (Char.chr n0); " EOF" ])
    | MalformedUTF_ ss ->
        raise (MalformedUTF_ (hexstring_of_char (Char.chr n0) :: " " :: ss))
  with MalformedUTF_ ss -> raise (MalformedUTF_ ("UTF8 " :: ss))
...
let utf16_next next16 s =
  try
    match next16 s with
    | m when 0x0000 <= m && m <= 0xd7ff -> m
    | m1 when 0xd800 <= m1 && m1 <= 0xdbff -> (
        try
          match next16 s with
          | m2 when 0xdc00 <= m2 && m2 <= 0xdfff -> (
              match
                (((m1 land 0x3ff) lsl 10) lor (m2 land 0x3ff)) + 0x10000
              with
              | m
                when (0x0000 <= m && m <= 0xd7ff)
                     || (0xe000 <= m && m <= 0x10ffff) ->
                  m
              | m ->
                  raise
          ...
          | m2 -> raise (MalformedUTF_ [ hex4 m1; " "; hex4 m2 ])
        with Stream.Failure -> raise (MalformedUTF_ [ hex4 m1; " EOF" ]) )
    | m1 -> raise (MalformedUTF_ [ hex4 m1 ])
  with MalformedUTF_ ss -> raise (MalformedUTF_ ("UTF16 " :: ss))

let next32_be s =
  let n0 = next16_be s in
  try
    let n1 = next16_be s in
    (n0 lsl 16) lor n1
  with
  | Stream.Failure -> raise (MalformedUTF_ [ hex4 n0; " EOF" ])
  | MalformedUTF_ ss -> raise (MalformedUTF_ (hex4 n0 :: " " :: ss))

let next32_le s =
  let n1 = next16_le s in
  try
    let n0 = next16_le s in
    (n0 lsl 16) lor n1
  with
  | Stream.Failure -> raise (MalformedUTF_ [ hex4 n1; " EOF" ])
...
let utf32_next next32 s =
  try
    ...
    | m when (0x0000 <= m && m <= 0xd7ff) || (0xe000 <= m && 0xe000 <= 0x10ffff)
      ->
        m
    | m -> raise (MalformedUTF_ [ "UTF32 "; hex8 m; " -- no such code point" ])
  with MalformedUTF_ ss -> raise (MalformedUTF_ ("UTF32 " :: ss))

(************************************)

let utf8_get s i =
  if i = String.length s then uEOF
  else utf8_next (fun j -> try s.[i + j] with _ -> raise Stream.Failure)
  ...
  | _ ->
      raise
        (Miscellaneous.Catastrophe_
           [ "utf8_of_utfNchannel "; string_of_int size ])

let stream_of_utf8string s = of_utfstream utf8_next (Stream.of_string s)
...
let _ =
  ...
  for i = 0xc2 to 0xdf do
    utf8_widths.(i) <- 2
  done;
  for i = 0xe0 to 0xef do
    utf8_widths.(i) <- 3
  done;
  for i = 0xf0 to 0xf4 do
    utf8_widths.(i) <- 4
  done

let utf8width_from_header c = utf8_widths.(Char.code c)
...
let utf8_sub s i = if i = String.length s then uEOF else utf8_get s i
      ...
      if j < 0 then
        raise
          (MalformedUTF_
             [
               "UTF8 string ";
               bracketed_string_of_list hexstring_of_char "; "
                 (chars_of_string (String.sub s 0 i));
             ])
      else
        let n = utf8width_from_header s.[j] in
        if n > 0 then utf8_get s j else f (j - 1)
    in
    f (i - 1)
...
let utf8_of_ucode m cs =
  if m = uEOF then cs
  else if 0x00 <= m && m <= 0x07f then Char.chr m :: cs
  else if 0x80 <= m && m <= 0x7ff then
    let y = m lsr 6 in
    let x = m land 0x3f in
    Char.chr (y lor 0xc0) :: Char.chr (x lor 0x80) :: cs
  else if (0x800 <= m && m <= 0xdfff) || (0xe0000 <= m && 0xe0000 <= 0xffff)
  then
    let x = m land 0x3f in
    ...
    :: Char.chr (x lor 0x80)
    :: cs
  else if 0x10000 <= m && m <= 0x10ffff then
    let x = m land 0x3f in
    let y = (m lsr 6) land 0x3f in
    ...
    :: Char.chr (x lor 0x80)
    :: cs
  else raise (MalformedUTF_ [ "int "; hex8 m; " -- no such code point" ])

let utf8_implode ns = string_of_chars (List.fold_right utf8_of_ucode ns [])

let utf8_of_ucode m = string_of_chars (utf8_of_ucode m [])

let utf8width_from_ucode m =
  if 0x00 <= m && m <= 0x07f then 1
  else if 0x80 <= m && m <= 0x7ff then 2
  else if (0x800 <= m && m <= 0xdfff) || (0xe0000 <= m && 0xe0000 <= 0xffff)
  then 3
  else if 0x10000 <= m && m <= 0x10ffff then 4
  else
    raise
      (MalformedUTF_
         [ "utf8_width_from_ucode "; hex8 m; " -- no such code point" ])
...
let rec words = function
  ...
  | s ->
      let rec wds = function
        ...
        | 0x22 :: cs ->
            let ws = qds cs in
            (0x22 :: List.hd ws) :: List.tl ws
        | c :: cs ->
            let ws = wds cs in
            (c :: List.hd ws) :: List.tl ws
      and qds = function
        | [] -> [ [] ]
        | [ 0x22 ] -> [ [ 0x22 ] ]
