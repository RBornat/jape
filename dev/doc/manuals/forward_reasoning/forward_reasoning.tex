...
Jape's first logic encoding was a single-conclusion intuitionistic sequent calculus, with the rules shown in \tabref{SCS}. This is, as Mark Dawson pointed out in the thesis~\cite{Dawsonthesis} which inspired us in the early days of Jape, an `all-introduction' calculus: at each step a rule reduces either a hypothesis (left-side) or a conclusion (right-side) formula at a tip of a tree of sequents, generating new tips or (with axioms hyp and ⊥⊢) closing the tip. It's then natural to build tree proofs step by step like the partial attempt in \figref{firsttree}, working backwards from the target sequent towards the tips of the final proof tree.
