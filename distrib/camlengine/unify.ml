...
let rec freshUnknown cxt c v =
  ...
  (cxt, registerUnknown (v, c))

let rec freshalphavar cxt c v =
  try freshproofvar cxt c v
  with Catastrophe_ ss -> (
    try freshproofvar cxt c (vid_of_string (autoID c "alpha"))
    with Catastrophe_ _ -> raise (Catastrophe_ ss) )
...
let rec occurs cxt v t =
  ...
  | Unknown (_, v', _) -> (
      v = v'
      || match varmap cxt <@> v' with Some t -> occurs cxt v t | None -> false )
...
let abstract orig map term ps cxt =
  ...
  in
  let rec okname cxt _P m =
    match varmappedbyq (facts (ourprovisos ()) cxt) _P m with
    | No -> Some (cxt, _P)
    | Maybe ->
        List.iter addproviso ((fun v -> NotinProviso (v, _P)) <* realmapdom);
        Some (cxt, _P)
    | Yes -> None
  in
  ...
  let rec _Abstract cxt m _P =
    match m with
    | [] ->
        let r = Some (cxt, _P) in
        if !unifydebug then showAres [] _P r;
        r
    | _ ->
        if !unifydebug then
          consolereport
        ...
        let _Ares r =
          if !unifydebug then showAres m _P r;
          r
        in
        ...
        and analyse m _P =
          ...
          | Unknown (_, vid, c) -> (
              ...
              | Yes -> _Ares None )
          | App (_, f, a) -> (
              option_njfold (_Afold m) [ f; a ] (cxt, []) &~~ function
              | cxt, [ f; a ] -> _Ares (Some (cxt, registerApp (f, a)))
              | _ -> raise MatchinAbstract_ )
          | Tup (_, sep, ts) ->
              ( option_njfold (_Afold m) ts (cxt, []) &~~ fun (cxt, ts') ->
          ...
          | Binding (_, (bs, ss, us), env, pat) -> (
              match restrictsubstmap (facts (ourprovisos ()) cxt) m bs ss with
              | Some m' ->
                  ( option_njfold (_Afold m) us (cxt, []) &~~ fun (cxt, us') ->
                    ( option_njfold (_Afold m') ss (cxt, [])
                    &~~ fun (cxt, ss') ->
                      Some (cxt, registerBinding ((bs, ss', us'), env, pat)) )
                    |~~ fun _ -> _Ares None )
                  |~~ fun _ -> _Ares None
              | None -> defermatch () )
          | Subst (_, r, _P', m') -> (
              ...
              match vtsplit fs m vs' with
              | ys, ns, [] ->
                  ( option_njfold (_Afold m) ts' (cxt, []) &~~ fun (cxt, ts'') ->
                    ( _Abstract cxt
                        (vs' @ substmapdom ns ||| vs' @ substmapran ns)
                        _P'
                    &~~ fun (cxt, _P'') ->
                      Some
                        (cxt, registerSubst (r, _P'', substmapdom m' ||| ts''))
                    )
                    |~~ fun _ -> _Ares None )
                  |~~ fun _ -> _Ares None
              | _ -> defermatch () )
...
let rec whatever f cxt t = match f cxt t with None -> Some (cxt, t) | r -> r
...
let rec class__ cxt t =
  match t with
  | Unknown (_, v, _) -> varmap cxt <@> v &~~ whatever class__ cxt
 ...
 * RB April 95
 *)
and subststep cxt ((r, _P, m) as s) =
  let rec _S _P' = registerSubst (r, _P', m) in
  let rec _Svar () =
  ...
  let r =
    ...
    | Fixapp (_, bras, ts) -> Some (cxt, registerFixapp (bras, _S <* ts))
    | Binding (_, (bs, ss, us), env, pat) ->
        let cxt = rewritecxt cxt in
        ...
        let rec newb (b, (bs, ns, cxt)) =
          if
            List.exists
              ...
              m'
          then
            let cxt, b' = freshalphavar cxt VariableClass (vid_of_var b) in
            ( b' :: bs,
              (b, b') :: ns,
        ...
        Some
          ( plusvisibleprovisos cxt ((fun v -> NotinProviso v) <* allpairs bs'),
            registerBinding
              ( ( bs',
                  (fun s -> registerSubst (r, registerSubst (true, s, ns), m'))
                  <* ss,
    ...
    | Subst (_, r', _P', m') -> (
        let cxt = rewritecxt cxt in
        match plussubstmap (facts (provisos cxt) cxt) m m' with
  ...
  if !unifydebug then
    consolereport
      [
        "subststep rewriting ";
        diag_string_of_term (registerSubst s);
        " => ";
        string_of_option
          (string_of_pair string_of_cxt diag_string_of_term ",")
          r;
      ];
  r
...
let rec unify a1 a2 a3 a4 =
  ...
  | (t1orig, orig_of_t) :: tts, dds, progress, cxt ->
      ...
      let rec pushtts (t1s, s_of_t) =
        try unify ((t1s ||| s_of_t) @ tts) dds progress cxt with Zip_ -> None
      in
      let rec doit t1 t2 cxt =
        ...
        | Unknown (_, v1, c1), Unknown (_, v2, c2) ->
            if v1 = v2 && c1 = c2 then success cxt
              ...
              &~~ success
        | Unknown (_, v1, c1), _ -> (
            match assign (v1, c1) (reb t1orig orig_of_t t2) cxt with
            | Some cxt -> success cxt
            | None -> (
                match t2 with Subst _ -> defer DeferAssignment cxt | _ -> None )
            )
        ...
        | Binding (_, b1, _, pat1), Binding (_, b2, _, pat2) ->
            if pat1 = pat2 then
              if
                List.exists
                  (function Binding _, Binding _ -> false | _ -> true)
                  ( (fun (t1, t2) -> (snd (simp cxt t1), snd (simp cxt t2)))
                  <* tts )
              then delay cxt
              else
        ...
        | Subst (_, r1, _P1, m1), Subst (_, r2, _P2, m2) -> (
            if not (r1 && r2) then defer DeferAlignment cxt
              ...
              match subststep cxt (r1, _P1, m1) with
              | Some (cxt, t1) -> (
        ...
        | Subst (_, false, _P1, m1), t2 -> (
            match subststep cxt (false, _P1, m1) with
        ...
        | Subst (_, true, _P1, m1), _ -> (
            ...
            | None -> (
                ...
                match
                  abstract (debracket _P1) m1 (rewrite cxt t2) (provisos cxt)
                    cxt
                with
                | Some (cxt, _P2, ps, ms) ->
                    unify
      ...
      and alignbindings ((b1s, s1s, u1s), (s_of_b, s_of_s, s_of_u)) cxt =
          ...
          if v1 = v2 then (cxt, v1 :: alls, m1, m2, es)
            ...
            | Id (_, vid1, c1), Id (_, vid2, c2) ->
                let rec notins cxt v ss =
                  plusvisibleprovisos cxt ((fun s -> NotinProviso (v, s)) <* ss)
                in
                let fs = facts (provisos cxt) cxt in
        ...
        let cxt, alls, m1, m2, extras =
          nj_fold newb
            (debracket <* b1s ||| (debracket <* s_of_b))
            (cxt, [], [], [], [])
        in
...
and unifycleverly tts dds cxt =
  ...
  | ((reason, (t1, t2)) as tt) :: tts -> (
      let rec yes tts' cxt' = unify (tts' @ (snd <* tts @ dds)) [] true cxt' in
      ...
      | DeferAlignment, (Subst (_, r1, t1, vts1), Subst (_, r2, t2, vts2)) -> (
          match alignsubsts (r1, t1, vts1) (r2, t2, vts2) cxt with
      ...
      | DeferSimplification, (Subst (_, false, _P1, m1), t2) -> (
          ...
          | _ -> no cxt )
      | DeferSimplification, (_, Subst (_, false, _, _)) -> reversecleverly ()
      | _ ->
          raise
            (Catastrophe_ [ "unifycleverly "; ppc (tt :: tts); " "; ppc dds ]) )
  | [] -> lastditch dds [] cxt
...
and lastditch tts dds cxt =
  ...
  | ((reason, (t1, t2)) as tt) :: tts -> (
      ...
      | DeferAlignment, (Subst (_, _, _P1, m1), Subst (_, _, _P2, m2)) -> (
          ...
          match
            zipsubstmaps
              (facts (provisos cxt) cxt)
              (canonicalsubstmap m1, canonicalsubstmap m2)
          with
          | Some mms ->
              unify (((_P1, _P2) :: mms) @ (snd <* tts @ dds)) [] true cxt
          | None ->
              (* used to allow failure ... *)
              no cxt )
      | _ -> no cxt )
  | [] -> None
...
and zipvts pdb (vts1, vts2) =
  ...
  let rec zipup (tt1s, s_of_tt) =
    nj_fold
      (fun (((v1, t1), (v2, t2)), mms) -> (v1, v2) :: (t1, t2) :: mms)
      (tt1s ||| s_of_tt) []
  in
  let rec ok (d1, d2) =
    Some
      (zipup (d1, d2) @ zipup (vtminus pdb vts1 diff1, vtminus pdb vts2 diff2))
  in
  match
    ( List.length vts1 = List.length vts2,
      vtmetacount diff1 = 1 && vtmetacount diff2 = 1,
      diff1,
      diff2 )
  with
...
and ziptermlists pdb (t1s, s_of_t) =
  try Some (t1s ||| s_of_t) with Zip_ -> None

and alignsubsts (r1, _P1, m1) (r2, _P2, m2) cxt =
    ...
    | ys, (_ :: _ as ns), [] -> (
        ...
        match
          abstract (debracket _P1)
            (substmapdom m2 @ substmapdom ns ||| substmapdom m2 @ substmapran ns)
        ...
        | Some (cxt, _P2', ps, ms) ->
            Some
              ( plusvisibleprovisos cxt ps,
                ms
                @ [
                    (registerSubst (r1, _P1, ys), registerSubst (r2, _P2', m2));
                  ] )
        | _ -> None )
    | _ -> None
  in
  tryalign (r1, _P1, m2) (r2, _P2, m2) |~~ fun _ ->
  tryalign (r2, _P2, m2) (r1, _P1, m1)
...
and unifycollections kind (e1s, s_of_e) cxt =
  ...
  let rec ures ((r1, t1), (r2, t2)) cxt =
    ...
    | ResUnknown i1, ResUnknown i2 ->
        (* can't stop it *)
        if i1 = i2 then Some cxt else Some (plusresmap cxt (i1 |-> (r2, t2)))
  ...
  let rec collclass cxt es =
    let rec expand (e, (cxt, es)) =
      ...
      | Segvar (_, ps, Unknown (_, v, _)) -> (
          ...
          | _ -> (cxt, e :: es) )
      | Element (_, r, t) ->
          let cxt, t' = simp cxt t in
          (cxt, registerElement (r, t') :: es)
      | _ -> (cxt, e :: es)
    in
        ...
        | ps, _ -> (
            ...
            | Unknown (_, vid, c) ->
                ...
                Some
                  ( plusvarmap cxt
                      ( vid
                      |-> registerCollection (c, [ registerSegvar (ps, u) ]) ),
                    registerSegvar ([], u) )
            | _ -> None ) )
    | ps, cxt, Element (_, r, t) ->
        ...
        let rec dm a1 a2 =
          ...
          | ps, (cxt, t) ->
              ...
              let rec mkApp = function
                | [] -> u
                | p :: ps -> registerApp (p, mkApp ps)
              in
              unify [ (t, mkApp ps) ] [] false cxt &~~ fun cxt ->
              Some (simp cxt u)
        in
        dm ps (debsimp cxt t) &~~ fun (cxt, t) ->
  ...
  in
  let rec isuseg = function
    | Segvar (_, _, Unknown _) ->
        true (* oh dear this was an sml bug -- what next *)
    | _ -> false
  in
  let rec isel = function Element _ -> true | _ -> false in
  ...
  let rec unifybags (e1s, s_of_e) cxt =
    ...
    let rec uel a1 a2 a3 a4 =
      ...
      | defers, cxt, (r, t), el :: es -> (
          ...
          | Segvar (_, ps, Unknown (_, v, c)) -> (
              match demodeify ps cxt (registerElement (r, t)) with
              | Some (cxt, el') ->
      ...
      | defers, cxt, (r, t), [] -> (
          ...
          | [ (_, el', ps, v) ] ->
              ...
              let cxt =
                plusvarmap cxt
                  ( v
                  |-> registerCollection (kind, [ registerSegvar ([], u); el' ])
                  )
              in
    ...
    let rec uf cxt es =
      ...
      | Segvar (_, _, Unknown (_, v, c)) :: es ->
          uf (plusvarmap cxt (v |-> registerCollection (c, []))) es
      | _ -> []
    in
    let rec def () =
      [
        plusvisibleprovisos cxt
          [
            UnifiesProviso
              (registerCollection (kind, e1s), registerCollection (kind, s_of_e));
          ];
      ]
    in
    ...
    let rec dive () =
      ...
      | _ -> (
          ...
          | [ Segvar (_, ps, Unknown (_, v, c)) ], _ -> (
              match option_njfold (demodeifyall ps) s_of_e (cxt, []) with
              | Some (cxt, s_of_e) ->
                  res [ plusvarmap cxt (v |-> registerCollection (c, s_of_e)) ]
              | _ -> res [] )
          ...
          | _ -> (
              ...
              | _ -> (
                   ...
                   *)
                  match
                    (List.length (isuseg <| e1s), List.length (isuseg <| s_of_e))
                  with
                  | 0, 0 -> res []
                  | 1, _ -> (
                      match extract isuseg e1s with
                          ...
                          | Some (cxt, ks_of_e') ->
                              res
                                (unifybags
                                   ...
                                   (plusvarmap cxt
                                      (v |-> registerCollection (kind, ks_of_e'))))
                          | None -> res [] )
                      | _ -> res [] )
                  | _, 1 ->
                      (* can't happen, but shut up compiler *)
                      urev ()
                  | _, 0 ->
                      res
                        (if List.exists (not <.> isuseg) e1s then [] else def ())
                  | 0, _ ->
                      res
     ...
     *)
    match
      ( extract (not <.> isuseg) e1s,
        List.length e1s > 1,
        extract (not <.> isuseg) s_of_e,
        List.length s_of_e > 1 )
    with
  ...
  let rec unifylists (e1s, s_of_e) cxt =
    ...
    let rec ul svs es (cxt, hds, tls) =
      ...
      let cxt' =
        match svs with
        | [ Segvar (_, _, Unknown (_, v, _)) ] -> plusvarmap cxt (v |-> chds)
        | _ ->
            plusvisibleprovisos cxt
    ...
    let rec allsplits ps cxt es =
      ...
      match es with
      | el :: es -> (
          match demodeify ps cxt el with
              ...
              (let cxt, u' = freshUnknown cxt c v in
               ...
               let cxt =
                 plusvarmap cxt
                   ...
                   |-> registerCollection
                         ( kind,
                           [ registerSegvar ([], u'); registerSegvar ([], u'') ]
                         ) )
               in
               ( cxt,
                 [ registerSegvar (ps', u') ],
     ...
     * in that case
     *)
    let rec twuseg = function
      | (Segvar (_, ps, Unknown _) as e) :: es ->
          let rec simsv = function
    ...
    | 0, 0 -> (
        ...
        | Element (_, r1, t1) :: e1s, Element (_, r2, t2) :: s_of_e ->
            res
              ( match
                  ures ((r1, t1), (r2, t2)) cxt &~~ unify [ (t1, t2) ] [] false
                with
...
and simplifydeferred pros cxt =
  ...
  let _ = if !unifydebug then consolereport [ "** start simplifydeferred" ] in
  let rec res r =
    if !unifydebug then
      consolereport
        [
  ...
  let rec dodefer ((t1, t2) as pair) cxt =
    let rec say s =
      consolereport
        [
          "dodefer ";
          string_of_termarg (rewrite cxt t1);
          " ";
          string_of_termarg (rewrite cxt t2);
          " ";
          s;
        ]
    in
    ...
    in
    let rec nores s r =
      if !unifydebug then say ((" " ^ s) ^ " deferred again");
      r
    in
    let rec rematch a1 a2 a3 =
      match (a1, a2, a3) with
      | (t1, t2), (_, t) :: vts, go -> (
          match unifyeqtermsq (facts pros cxt) t1 t with
          | Maybe -> rematch (t1, t2) vts false
          | Yes -> res "trying unification in rematch 1" (unifyv (t1, t2) cxt)
          | No -> rematch (t1, t2) vts go )
      | (t1, t2), [], go ->
          if go then res "trying unification in rematch 2" (unifyv (t1, t2) cxt)
          else
    ...
    if not (deferrable cxt (t1', t2')) then res "not deferrable" []
      ...
      match (t1, t2) with
      | Subst _, Subst _ -> res "trying unification" (unifyv (t1', t2') cxt)
  ...
  let r =
    if null defers then [ cxt ]
      ...
      (* don't disturb sleeping provisos *)
  ...
  if !unifydebug then
    consolereport
      ...
      | _ ->
          [
            "fails -- \n(";
            debugstring_of_term t1;
            ",";
            debugstring_of_term t2;
            ");\n";
            string_of_cxt cxt;
          ] );
  r
...
and unifyv (t1, t2) cxt =
  ...
  | _ -> ( match doUnify (t1, t2) cxt with Some cxt -> [ cxt ] | None -> [] )
...
let unifytermsandcheckprovisos pair = doUnify pair &~ checkprovisos
...
let unifyvariousEQ pair cxt =
  (fun cxt' ->
    List.length (rawaslist (varmap cxt)) = List.length (rawaslist (varmap cxt'))
...
let rec dropunify (target, sources) cxt =
  ...
  let checkout tc sc =
    if
      List.exists
        ...
        sources
    then bad "some sources don't fit target"
  in
  ...
  | Segvar (_, ops, Unknown (_, u, tc) (* as v *)) -> (
      ...
      | ListClass fc, [ source ] ->
          ...
          res tc
            ( [ target ],
              [ registerSegvar (ops, u'); source; registerSegvar (ops, u'') ] )
            cxt
...
let matchedtarget origcxt newcxt uvids =
  let rec ok u =
    ...
    | Some old, Some new__ -> eqterms (rewrite origcxt old, rewrite newcxt new__)
    | None, None -> true
    | _ -> false
  in
