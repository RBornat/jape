...
let rec appflatten t ts =
  ...
  isJuxtapos t &~~ fun (f, a) ->
  appflatten f (a :: ts) |~~ fun _ -> Some (f :: a :: ts)
...
let rec infixnameflatten name t ts =
  ...
  isInfixApp t &~~ fun (name', prio, assoc, a1, a2) ->
  if name = name' then
    match assoc with
    | LeftAssoc ->
        let ts' = renderprio prio true a2 :: ts in
        infixnameflatten name a1 ts' |~~ fun _ ->
        Some (renderprio prio false a1 :: ts')
    | RightAssoc ->
    ...
    | TupleAssoc ->
        ( infixnameflatten name a2 ts |~~ fun _ ->
          Some (renderprio prio false a2 :: ts) )
        &~~ fun ts' ->
        infixnameflatten name a1 ts' |~~ fun _ ->
        Some (renderprio prio false a1 :: ts')
...
let rec infixprioflatten n mustbra t ts =
  if !termfolddebug then
    consolereport
      [
        "infixprioflatten tries ";
        string_of_int n;
        " ";
        string_of_bool mustbra;
        " (";
        debugstring_of_term t;
        ") ";
        bracketed_string_of_list
  ...
  isInfixApp t &~~ fun (_, n', assoc, a1, a2) ->
  if n' > n || (n' = n && not mustbra) then
    ( infixprioflatten n' (assoc = LeftAssoc) a2 ts |~~ fun _ ->
      Some (renderprio n' (assoc = LeftAssoc) a2 :: ts) )
    &~~ fun ts' ->
    infixprioflatten n' (assoc = RightAssoc) a1 ts' |~~ fun _ ->
    Some (renderprio n' (assoc = RightAssoc) a1 :: ts')
  else None
...
let termfold (_, font, preleading, interleading, postleading, w, t) =
  ...
  let tryfold ts tts default =
    ...
    let matchup t tts =
      ...
      let rec getend tt1 tts' =
        ...
        | " " :: tts'' -> getend (" " :: tt1) tts''
        | c :: tts'' ->
            if isInvisibleKet c then getend (c :: tt1) tts''
  ...
  | Some ts ->
      if !termfolddebug then
        consolereport
          [
            "termfold splits ";
            Stringfuns.enQuote (string_of_term t);
            " into ";
            bracketed_string_of_list
              (Stringfuns.enQuote <.> string_of_term)
              "\n" ts;
          ];
  ...
  | _ -> (
      ...
      | Some (name, n, _, _, _) ->
          tryfold
            (_The (infixnameflatten name t []))
            tstring
            (fun _ ->
              tryfold (_The (infixprioflatten n false t [])) tstring default)
      | _ -> default () )
...
module FoldCache = Cache.F (struct
  type dom = string * font * int * int * int * int * term
