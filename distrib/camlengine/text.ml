 ...
 * for me?
 *)
let string_of_textalign = function
  | FirstLine -> "FirstLine"
  | MidBlock -> "MidBlock"
  | LastLine -> "LastLine"

let rec string_of_syllable = function
  | Syllable (i, s) -> "Syllable(" ^ string_of_font i ^ "," ^ enQuote s ^ ")"
  | Gap i -> "Gap " ^ string_of_int i
  | Linebreak i -> "Linebreak " ^ string_of_int i
  | Block (c, sys) ->
      "Block(" ^ string_of_textalign c ^ ","
      ^ bracketed_string_of_list string_of_syllable "," sys
      ^ ")"

let string_of_text (Text sys) =
  "Text" ^ bracketed_string_of_list string_of_syllable "," sys

let string_of_textlayout (Textlayout t) =
  "TextLayout"
  ^ bracketed_string_of_list
      (string_of_triple string_of_pos string_of_font enQuote ",")
      ", " t
...
let rec textWAD measure c ss sys =
  ...
  and add3 w a d x (w', a', d') = f (w + w') (max a a') (max d d') (x + w') in
  f 0 0 0 0 ss sys
...
let measuretext measure c (Text sys) =
  ...
  ( textsize w a d,
    Textlayout
      (List.map (fun ((x, y), (f, s)) -> (pos x y, f, s)) (List.rev ss)) )
