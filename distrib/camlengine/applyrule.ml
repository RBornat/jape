...
let rec failwithreason ss =
  setReason ss;
  None

let rec explain a1 a2 =
  match (a1, a2) with why, None -> failwithreason (why ()) | why, r -> r
...
type inforec = {
  ...
  kind : string;
  conjecture : seq;
  conjectureinf : rewinf;
  cxt : cxt;
  args : term list;
  provisos : visproviso list;
  antecedents : seq list;
  consequent : seq;
  how : prooftree_step;
  principals : resnum list * resnum list;
...
type possmatch = (info * element list * element list * cxt * seq list) list
...
let rec answer
    ( Info { reason; args; conjecture; how },
      (thinnedL : element list),
      (thinnedR : element list),
      cxt,
      subgoals ) =
  ( cxt,
...
let rec nonempty = function [] -> None | xs -> Some xs

let rec runfilter e g = explain e <.> nonempty <.> filter g
...
let (bymatch : possmatch -> possmatch option) =
  runfilter
    ...
    (fun ( Info { cxt; conjecture; kind; args; conjectureinf; how },
           _,
           _,
           cxt',
           _ ) ->
...
let (sameprovisos : possmatch -> possmatch option) =
  runfilter
    ...
    (fun (Info { cxt; conjecture; kind; args; how }, _, _, cxt', _) ->
      let r =
        eqbags
          (fun (vp', vp) -> provisoactual vp' = provisoactual vp)
...
let answerstring
    (Info { conjecture = Seq (st, chs, cgs) }, thinnedL, thinnedR, _, _) =
...
let rec takefirst = function [] -> None | p :: ps -> Some (answer p)
...
let rec offerChoice ps =
  ...
  | [] -> None
  | [ p ] -> Some (answer p)
  | (Info { kind; how }, _, _, _, _) :: _ -> (
      beforeOffering ();
      let posses = listposs <* ps in
      let shows =
        if !tryresolution && !resolvepossible then
          posses
          @ [
              [
                "Just match the conclusion, making hypotheses of any unmatched \
                 antecedents";
              ];
            ]
        else posses
      ...
      | None ->
          failOffering ();
          resolvepossible := false;
          None
      | Some n ->
          if !tryresolution && !resolvepossible && n = List.length posses then (
            failOffering ();
            None )
          else (
            succeedOffering ();
            Some (answer (List.nth ps n)) ) )
  ...
  let rec impprovisos thinnedL thinnedR =
    if null impfreshprovisos then []
      ...
      let rec goodside ths side =
        ...
        registerCollection (c, (fun el -> not (member (el, ths))) <| els)
      in
      let cHs = goodside thinnedL _CHs in
      let cGs = goodside thinnedL _CGs in
      nj_fold
        (fun ((b, i), ps) -> expandfresh b i cHs cGs ps)
        impfreshprovisos []
  in
  ...
  *)
  ...
  let genSubGoals =
    let rec exp0 ss () =
      ...
      :: " to the problem sequent "
      :: string_of_seq (rewriteseq cxt conjecture)
      :: ", " :: ss
    in
    let rec exp1 sing plur els () =
      let w = match breakside els with _, _, [ _ ] -> sing | _ -> plur in
      exp0
        [
          "the ";
          w;
    ...
    let rec unusedprincipal sing plur els () =
      let word = match els with [ _ ] -> sing | _ -> plur in
      let ts =
        match els with
    ...
    in
    explain (exp1 "conclusion" "conclusions" _CGs)
    <.> nonempty
       ...
       <.> _BnMfilter (fun (thRs, _) ->
               ...
               if !applydebug > 0 then
                 consolereport
                   [
                     "usedconc checking ";
                     bracketed_string_of_list showel "," selconcs;
                     " against ";
                     bracketed_string_of_list showel "," thRs;
                     " => ";
                     string_of_bool r;
                   ];
               r) )
           ...
           try
             ( bads,
               ( thinnedL,
                 thinnedR,
                 verifyprovisos
                   (plusprovisos cxt (impprovisos thinnedL thinnedR)) )
               :: goods )
           with Verifyproviso_ p -> (p :: bads, goods)
         in
  ...
  if _Cst = st then genSubGoals (plusprovisos cxt newprovisos)
    ...
    failwithreason
      [
        ...
        string_of_seq conjecture;
        " because the turnstiles are different";
      ]
...
let rec showstuff stuff =
  string_of_octuple enQuote
    ...
    (string_of_pair
       ...
       (bracketed_string_of_list string_of_resnum ",")
       ",")
    (bracketed_string_of_list string_of_seq ",")
    string_of_seq
    (bracketed_string_of_list string_of_visproviso " AND ")
    ", " stuff

let rec apply checker filter taker selhyps selconcs stuff reason cxt (_C, _Cinf)
    =
  let kind, hiddencontexts, how, args, principals, antes, conseq, provs =
    stuff
  in
  if !applydebug > 0 then
    consolereport
