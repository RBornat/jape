...
let ( freezesaved,
      ...
      saveproofs ) =
  let proofs :
      (name, bool * prooftree * name list * bool * (seq * model) option) mapping
      ref =
    ref empty
  ...
  and proof_children name =
    match proof_depends name with Some ns -> ns | None -> []
  ...
  and addproof alert query name proved proof givens cxt disproved disproofopt =
    ...
    let rec putitin name thing givens place =
      ...
      proofs := !proofs ++ (name |-> (proved, proof, ds, disproved, disproofopt));
      allsaved := false;
      true
    in
    ...
    let rec doit () =
      ...
      | [], Some (Theorem (params, _, _), place) ->
          putitin name (Theorem (params, storedprovisos provisos, seq)) [] place
      ...
      | _, Some (Rule ((params, _, _, _), _), place) ->
          putitin name
            (Rule ((params, storedprovisos provisos, givens, seq), false))
            givens place
      | _ -> raise (Catastrophe_ [ "can't find theorem statement in addproof" ])
    in
  ...
  let word_of_panel p =
    ...
    | Some GivenPanelkind -> raise (Catastrophe_ [ "proof in GIVENPANEL" ])
    | None ->
  ...
  and saveproofs chan =
    let show n =
      ...
      | Some (proved, tree, provisos, givens, disproved, disproof) ->
          saveproof chan n Complete tree
            (snd <* (fst <| provisos))
            givens disproof
      | _ -> ()
    in
    let names = bool_of_opt <.> proofnamed <| thingnames () in
    let sortednames, _ = toposort names (fun n -> proved <| proof_children n) in
...
let rec lacksProof name = needsProof name (fst (_The (thingnamed name)))
