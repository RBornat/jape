...
module
  ...
  struct
        ...
        andthenr
          (getsidedselectedtext (),
           (fun (path, concsels, hypsels, givensels) ->
              ...
              in
              Some (path, MAP (deside, pairup concsels), hypsels, givensels)))
      and getsingleargsel () =
              ...
              (MAP
                 (parseit,
                  flatten
                    (getsels gs ::
                       MAP
                         ((fun ooo -> getsels (sml__hash__2 ooo)), cs @ hs))))
      in
      getsidedselectedtext, getunsidedselectedtext, getsingleargsel, getsels,
      getargs
      ...
      match getsingleargsel (), tac with
        Some arg, TermTac (name, args) ->
          if extensibletac env tac then
            try TermTac (name, args @ [quoteterm (string2term arg)]) with
    ...
    let rec noMoreTime () = Ref.dec triesleft; askTime'sUp ()
    ...
    let rec freshenv patterns cxt env =
      let us = fold (sortedmerge earliervar) (MAP (termvars, patterns)) [] in
      let rec f =
        function
    ...
    *)
    ...
    let rec expandstuff name (env, principals, thing) =
      fun (Proofstate {givens = givens}) ->
        let params = rev (mappingfuns.rawdom env) in
        (* I hope *)
    ...
    let rec preparestuff2apply stuff =
      ...
      in
      kind, hiddens, how, MAP (sml__hash__2, argmap), principals, antes',
      conseq', MAP (mkvisproviso, provisos')
    ...
    let rec doALERT tryf message m ps copt state =
      let m = message m in
      let ps = MAP ((fun (b, t) -> message b, t), ps) in
      match ps, copt with
    ...
    let rec doCOMPLETE f =
      fun
        ...
        try
          ...
          let tree' =
            mkTip cxt (rewriteseq cxt seq) treeformat.neutralformat
          in
          ...
          let (Proofstate {tree = tree''; cxt = cxt''} as state'') =
            ...
            match
              apply unifyvarious nofilter takeonlyone [] []
                ...
                (withtree
                   (state',
                    set_prooftree_fmt tree' tippath treeformat.neutralformat))
            with
      ...
      | triv, rulenames, (state : proofstate) ->
          let rec Applicable (name, cs) =
            match CanApplyInState (triv, name, state) with
              [] -> cs
            | hypmatches ->
                fold (fun (match__, cs) -> (name, match__) :: cs) hypmatches
                  cs
          in
          match fold Applicable rulenames [] with
            [] -> None
          | cs ->
    ...
    let rec fringe operator t =
      ...
      | _ -> raise Matchinassociativelawstuff_
    let eq = registerId ("=", conOperatorClass)
    let rec associativelaw operator thing =
      let rec assoc params term =
        try
          ...
          (length lf = 3 && lf = rf) && All (formulageneralisable params, rf)
        with
    ...
    let rec allrelevantthings () =
      MAP
        ((fun ooo -> (fun ooo -> sml__hash__1 (unSOME ooo)) (thingnamed ooo)),
         ( <| ) (relevant, thingnames ()))
    ...
    *)
            ...
            "DECIDEBAG", oracle :: args ->
              DECIDE
                (turnstile, cxt, HS, registerCollection (idclass, CS),
                 termstring oracle, MAP (termstring, args))
          | _ ->
              setReason
    ...
    let rec appendsubterms t q =
      match t with
        App (_, l, r) -> r :: l :: q
      | Tup (_, _, ts) -> rev ts @ q
      | Fixapp (_, _, ts) -> rev ts @ q
      | Binding (_, (bs, ss, us), _, _) -> (rev ss @ rev us) @ q
      | _ -> q
          ...
          if !FOLDdebug then
            consolereport
              ["ConcsOf "; namestring n; " (=> ";
               optionstring (fun ooo -> thingstring (sml__hash__1 ooo))
                 (thingnamed n);
               ")"];
    ...
    and ConclusionsofTactic env (spec, l) =
      ...
      | AltTac ts -> fold (ConclusionsofTactic env) ts l
      | TheoryAltTac ns ->
          fold (fun (n, l) -> ConclusionsofthingNamed (evalname env n, l)) ns
            l
    ...
    let rec doJAPE tryf display env ts =
      fun
        ...
        let rec Tickmenu on menu label state =
          try
            let rec U s = substring (s, 1, String.length s - 2) in
            let rec V t =
          ...
          with
            BadMatch ->
              setReason
                (["Badly-formed JAPE tactic: "] @ MAP (termstring, ts));
              None
        in
        ...
        let rec TtoIL =
          ...
          | Tup (_, ",", ts) ->
              ( </ ) ((fun (x, y) -> x @ y), []) (MAP (TtoIL, ts))
          | _ -> raise MatchinTtoIL
        in
        match ts with
          [] ->
            setReason ("Badly-formed JAPE tactic: " :: MAP (termstring, ts));
            None
        | t :: _ ->
            match explodeApp true t with
              Id (_, "write", _), [t] ->
            ...
            | Id (_, "cachereset", _), _ ->(* This should be done automatically when a theory-search button
            ...
            | _ ->
                setReason
                  ["Badly-formed tactic: JAPE("; liststring termstring "," ts;
                   ")"];
                None
        ...
        if noMoreTime () then Some state
          ...
          match stuff with
            _, _, Tactic (formals, tac) ->
              ...
              begin match args, tac with
                _ :: _, TheoryAltTac ns ->
                  def (AltTac (MAP ((fun n -> con (n, args)), ns))) []
              | _ -> def tac args
              end
          | _, _, Macro (formals, m) ->
          ...
          | argmap, _, _ ->
              ...
              let rec showit () =
                ...
                showProof display wholetarget wholegoal cxt wholetree
                  !autoselect;
                ()
              in
      ...
      fun (Proofstate {cxt = cxt} as state) ->
        ...
        | TheoryAltTac ns ->
            dispatchTactic display try__ env contn
              (AltTac (MAP ((fun n -> TermTac (n, [])), ns))) state
        | RepTac t ->
        ...
        | UnifyTac terms -> contn (forceUnify (MAP (eval env, terms)) state)
        | UnifyArgsTac ->
            andthenr
              (getargs "UNIFYARGS",
               (function
        ...
        | MapTac (name, args) ->
            doMAPTERMS display try__ contn (evalname env name)
              (MAP (eval env, args)) state
        | TermTac (str, args) ->
            let args' = MAP (eval env, args) in
        ...
        | ReplayTac t ->
            let noticetimewas = !(noticetime before noticetime) := false in
        ...
        | SetgoalTac p ->
            contn
              (Some
                 (withgoal
                    (state, Some (newpath "GOALPATH" (eval env) state p))))
    ...
    and tryApply display =
      tryApplyOrSubst dispatchTactic getapplyinfo mkenv
        (fun (n, args) -> TermTac (n, MAP (quoteterm, args)))(* con *)
         tracewithargs
        display
    and trySubst display =
      tryApplyOrSubst dispatchTactic (getsubstinfo true) mkenvfrommap SubstTac
            ...
            let lhss =
              optionfilter
                (fun (name, thing, t) ->
                   ...
                   | _ -> None)
                (fold (ConclusionsofTactic env) laws [])
            in
            findfirst
              (andthen
                 (Matches C,
    ...
    and doFOLDHYP name display try__ env contn (tactic, patterns) =
      fun
        ...
        let patterns = MAP (eval env, patterns) in
        match rewriteseq cxt (getTip tree goal) with
          Seq (_, HS, Collection (_, collc, [Element (_, resn, C)])) ->
            let rec HypMatches =
              function
    ...
    and doBIND tac display try__ env =
      fun (Proofstate {cxt = cxt; goal = goal; tree = tree} as state) ->
        let matching = sml__hash__1 try__ in
                ...
                setReason
                  (s :: " didn't match patterns " ::
                     liststring2 termstring ", " " and "
                       (MAP (sml__hash__1, pes)) ::
                     " to formulae " ::
                     liststring2 termstring ", " " and "
                       (MAP (sml__hash__2, pes)) ::
                     ss)
            end;
            None
          in
          let (newformals, namecxt, env) =
            freshenv (MAP (sml__hash__1, pes)) cxt env
          in
          match
            bindunify bad
              (fun p ->
                 bad [" because proviso "; provisostring p; " was violated."])
              (MAP ((fun (pattern, expr) -> eval env pattern, expr), pes))
              namecxt
          with
            None -> None
        ...
        *)
          ...
          let rec doit selpath selel ss =
            match ss with
              [b; middle; a] ->
                ...
                in
                let midterm =
                  try string2term middle with
                    ParseError_ ss -> pe ss
                in
                let newterm =
                  try string2term newtext with
                    ParseError_ ss -> pe ss
                in
                let oldterm =
                  try string2term ((b ^ middle) ^ a) with
                    ParseError_ ss ->
                      raise
                        (Catastrophe_
        ...
        let rec opensubgoals () =
          let tiprhss =
            ( <| )
              ((fun ooo ->
                  pathPrefix tree (getGoalPath goal) (sml__hash__1 ooo)),
               allTipConcs tree)
          in
          ...
          let tipterms =
            MAP
              ((fun (path, rhss) ->
                  path,
                  MAP (unSOME, ( <| ) (opt2bool, MAP (element2term, rhss)))),
               tiprhss)
          in
          flatten
            (MAP
               ((fun (path, terms) -> MAP ((fun term -> path, term), terms)),
                tipterms))
        in
        ...
        | BindArgTac spec ->
            begin match getsingleargsel () with
              Some sel ->
                begin try
                  checkBIND "LETARGSEL"
                    (cxt, env, Some (string2term sel), spec)
                with
              ...
              [FmtPath ns, conc] ->
                checkBIND "LETOPENSUBGOAL"
                  (cxt,
                   ( ++ )
                     (env,
                      ( |-> ) (name, registerTup (",", MAP (int2term, ns)))),
                  ...
                  (cxt, env,
                   Some
                     (registerTup
                        (",",
                         MAP
                           ((fun ooo -> rewrite cxt (sml__hash__2 ooo)),
                            tips))),
                   spec)
            end
    ...
    let rec applyLiteralTac display env try__ text state =
      runTactic display env try__
        (firstextend env (transTactic (string2tactic text))) state
