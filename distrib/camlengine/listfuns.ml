...
let rec foldr f z = function [] -> z | x :: xs -> f x (foldr f z xs)

let rec foldl f z = function [] -> z | x :: xs -> foldl f (f z x) xs
...
let rec ( /> ) ((e : 'a), (( ++ ) : 'a * 'b -> 'a)) : 'b list -> 'a =
  (* infix ++; *)
  let rec ff a1 a2 =
    match (a1, a2) with r, [] -> r | r, x :: xs -> ff (( ++ ) (r, x)) xs
  in
  ff e
...
let rec ( </ ) (( ++ ), e) =
  (* infix ++; *)
  let rec ff = function [] -> e | x :: xs -> ( ++ ) (x, ff xs) in
  ff
...
let rec seteq eq xs =
  nj_revfold
    (fun (x, ys) -> if List.exists (fun x' -> eq x x') ys then ys else x :: ys)
    xs []

let rec set (xs : 'a list) = seteq ( = ) xs
...
let rec catelim_sentencestring_of_list obstring sepn sep2 xs tail =
  ...
  | x :: xs ->
      obstring x
        (sepn :: catelim_sentencestring_of_list obstring sepn sep2 xs tail)
...
let rec string_of_list obstring punct =
  stringfn_of_catelim
    (catelim_string_of_list (catelim_of_stringfn obstring) punct)
...
let rec guardednth xs n =
  try List.nth xs n with Invalid_argument _ | Failure _ -> raise Bad_nth

exception Last_

let rec last = function [ x ] -> x | _ :: xs -> last xs | [] -> raise Last_
...
let rec _BMzip xs ys =
  match (xs, ys) with x :: xs, y :: ys -> (x, y) :: _BMzip xs ys | _, _ -> []
...
let rec sorteddiff ( < ) xs ys =
  ...
  | x1 :: xs, y1 :: ys ->
      ...
      else if x1 < y1 then x1 :: sorteddiff ( < ) xs (y1 :: ys)
...
let rec matchbag pp xs =
  ...
  and revapp a1 a2 =
    match (a1, a2) with [], ys -> ys | x :: xs, ys -> revapp xs (x :: ys)
  in
  match__ [] [] xs
...
let numbered xs =
  let rec r n = function x :: xs -> (n, x) :: r (n + 1) xs | [] -> [] in
  r 0 xs
...
let rec eqbags ee (xs, ys) =
  List.length xs = List.length ys && null (listsub ee xs ys)
