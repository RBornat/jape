...
let atoi s =
    match Belt_Int.fromString s with
    | Some x -> x
    | None -> raise AtoI_
...
(* this is trying not to use equality ... *)
...
let rec consolereport strings =
  let e = !errstream in
  List.iter (output_string e) strings;
  output_string e "\n";
  flush e
...
let rec error strings =
  consolereport strings;
  raise Error_

(* ********************************* settings variables ********************************* *)

let applyconjectures = ref "none"

(* whether to allow application of conjectures and derived rules --
...
let multiassumptionlines = ref true
