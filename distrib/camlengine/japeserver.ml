...
*)
    ...
    and flush s =
      try Pervasives.flush !outtoserver with
        exn ->
          (Miscellaneous.M.consolereport ["in flush "; s; " [WARNING: Server "; !servername; " may have died -- ";
                                         Printexc.to_string exn; " -- jape quitting]\n"];
    ...
    let rec readline s = flush ("readline " ^ s); front (input_line !infromserver)
    let rec nn_ =
      function
        "-" :: ds -> - dd_ 0 ds
      | ds -> dd_ 0 ds
    ...
    and out8 c =
      ...
      out r
    let rec askf s is = writef s is; ints (readline (s ^ " " ^ bracketedliststring (function Int i -> string_of_int i | Str s -> Stringfuns.M.enQuote (String.escaped s)) "," is))
