...
type treeplanrec = {
  ...
  seqbox : textbox;
  (* now there's a Lehmann-style assumption: single-line sequents! *)
...
let place subh gap
    ((Treeplan { proofbox; proofoutline } as plan), (topbox, alloutline, sps)) =
  ...
  ( (if null alloutline then newbox else topbox +||+ newbox),
    oz offset alloutline proofoutline,
    (newpos, plan) :: sps )

let rec tpH (Treeplan { proofbox }) = sH (bSize proofbox)

(* from a treeplan, find (x1,x2) of the sequent that underpins it *)
let rec subline (Treeplan { seqbox }) =
  let x1 = posX (tbP seqbox) in
  (x1, x1 + tsW (tbS seqbox))
...
let rec maketreeplan viewport proof =
  ...
  setproofparams Japeserver.TreeStyle linethickness;
  ...
  let sequentplan =
    makeseqplan
      (if !foldsequents then leading else 0)
      (invisbracketedstring_of_element true)
      showturnstiles
  in
  ...
  let rec _TP t =
    ...
    let thickness = if isMulti then 3 * linethickness else linethickness in
    let subps = List.map _TP (subtrees t) in
    let seqplan, seqbox = sequentplan origin s in
    let ((reasonsize, _) as reasoninf) =
      match r with None -> noreasoninf | Some why -> textinfo_of_reason why
    in
    ...
    let superh =
      tsH seqsize + match r with None -> 0 | _ -> vspace + tsH reasonsize
    in
    ...
    let superindent, sublineleft, sublineright =
      ...
      | _ :: _ ->
          ...
          let sublineleft = posX (fst spl) + fst (subline (snd spl)) in
          let sublineright = posX (fst spr) + snd (subline (snd spr)) in
          let sublinew = sublineright - sublineleft in
          let superindent = max 0 (sublineleft + ((sublinew - superw) / 2)) in
          ( superindent,
    ...
    let superbox = box (pos superindent supery) (size superw superh) in
    let seqoutline = (superindent, superindent + superw) in
    Treeplan
      {
        proofbox = subbox +||+ superbox;
        seqplan;
        ...
        linespec =
          ( if reasonw = 0 then None
            ...
            Some
              ( isMulti,
                pos sublineleft (supery - vspace - linethickness),
...
let rec _FIRSTOFN _P xs =
  let rec _F a1 a2 =
    ...
    | n, [] -> None
    | n, x :: xs -> (
        match _P (n, x) with None -> _F (n + 1) xs | result -> result )
  in
  _F 0 xs

let rec elinfo plan =
  match info_of_plan plan with ElementClass info -> Some info | _ -> None

let rec hit_of_pos pos path
    (Treeplan { proofbox; seqplan; seqbox; reasonplan; subplans }) =
  if within pos proofbox then
    if withintb pos seqbox then
      match findfirstplanhit (pos +<-+ tbP seqbox) seqplan &~~ elinfo with
      | Some (el, c) ->
          if c = DisplayHyp then Some (FormulaHit (HypHit (List.rev path, el)))
      ...
      | _ -> false
    then Some (ReasonHit (List.rev path))
    else
      _FIRSTOFN
...
let locateElement locel (prooforigin, proof, plan) =
  let rec locate p ps = function
    | Treeplan { seqplan; seqbox; subplans } ->
        let locsub ps (subpos, subplan) = locate (p +->+ subpos) ps subplan in
...
let allFormulaHits prooforigin p =
  let rec allroots revpath pos (Treeplan p) rs =
    ...
    let oneel pos rs (Formulaplan (_, tbox, c)) =
      ...
      | ElementClass (el, DisplayConc) ->
          (tbox, ConcHit (path, (el, None))) :: rs
      | _ -> rs
    in
  ...
  and allchilds n revpath pos ps rs =
    ...
    | [] -> rs
    | (cpos, p) :: ps ->
        allroots (n :: revpath) (pos +->+ cpos) p
          (allchilds (n + 1) revpath pos ps rs)
  in
  ...
  let rec cleanup test =
    List.iter
      (fun (oldpos, _) ->
        ...
        | Some oldhit -> if test (oldpos, oldhit) then highlight oldpos None
        | None ->
            raise
              (Catastrophe_
                 [
                   "notifyselect (treedraw) can't re-identify ";
                   string_of_pos oldpos;
                 ]))
      posclasslist
  in
  ...
  | Some (hitpos, _) -> (
      ...
      | None ->
          raise
            (Catastrophe_
               [
                 "notifyselect (treedraw) can't identify "; string_of_pos hitpos;
               ]) )
...
let rec samepath = function None, _ -> false | Some p1, p2 -> p1 = p2

let rec revgoal = function None -> None | Some path -> Some (List.rev path)

let rec elementofclass class__ plan =
  match info_of_plan plan with ElementClass (_, c) -> class__ = c | _ -> false

let rec draw goal pos proof (Treeplan { linethickness } as plan) =
  let rgoal = revgoal goal in
  let rec _D (Treeplan { seqplan; reasonplan; linespec; subplans; seqbox }) p
      here =
    ...
    ( match (samepath (rgoal, here), elementofclass DisplayConc <| seqplan) with
    | true, [ plan ] ->
        highlight (seqelementpos p seqbox plan) (Some DisplayConc)
    | _ -> () );
    _FORNUMBERED (fun (n, (stp, st)) -> _D st (p +->+ stp) (n :: here)) subplans
  in
  drawinproofpane ();
  _D plan pos []
...
let rec print str goal pos proof plan =
  ...
  let rec _D (Treeplan { seqplan; reasonplan; linespec; subplans; seqbox }) p
      here =
    out "(BY ";
    (match reasonplan with Some p -> outplan p | None -> out "\"\" ");
    out "(PROVE ";
...
let rec targetbox pos path plan =
  let rec _P a1 a2 a3 =
    match (a1, a2, a3) with
    | [], Treeplan { seqbox }, pos -> Some (tbOffset seqbox pos)
    | n :: ns, Treeplan { subplans }, pos -> (
        match subplans with
        | [] -> None
        | _ -> (
            try
  ...
  in
  match path with Some route -> _P route plan pos | None -> None

let rec defaultpos screen (Treeplan { seqbox; linethickness }) =
...
let rec postoinclude viewport box (Treeplan { proofbox } as layout) =
