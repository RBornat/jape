...
let rec mkstate provisos givens tree =
  let cxt, tree, uvs =
    rewriteProoftree givens false
      (withexterior (withprovisos newcxt provisos) (givens, sequent tree))
      tree
  in
  Proofstate
    {
      cxt =
        withresnum (withusedVIDs cxt uvs)
          ( nj_fold (uncurry2 max)
              ( maxtreeresnum tree
              :: (maxprovisoresnum <.> provisoactual <* provisos) )
              1
          + 1 );
      givens;
      tree;
...
let rec addproof report query name proved (Proofstate { cxt; tree; givens })
    disproofopt =
...
let doProof report query env name stage seq (params, givens, pros, tac)
    disproofopt =
  ...
  let rec checkproof () =
    try
      ...
      *)
      match
        applyLiteralTactic env
          ("ANY " ^ parseablestring_of_name name)
          initialstate
      with
      | None ->
          raise
    ...
    | Tacastrophe_ ss ->
        cleanup ();
        report
          ( "Error in tactic during " :: label () :: " " :: string_of_name name
          :: " -- " :: ss );
        None
    | exn ->
        cleanup ();
        raise exn
  in
  checkproof () &~~ fun (state, proved) ->
  let disproved =
    checkdisproof (proofstate_cxt state) (proofstate_tree state) disproofopt
  in
  let stage =
    match (stage, proved, disproved) with
    | Complete, false, false ->
        complain
          [ "is recorded as complete but is neither proved nor disproved" ];
        InProgress
    | _, _, _ -> stage
  in
  if stage = Complete then
    let _ =
      (addproof report query name proved state disproved disproofopt : bool)
    in
    None
