 ...
 *)
type matchresult =
  | Certain of (term, term) mapping
  | Uncertain of (term, term) mapping

let rec string_of_matchresult = function
  | Certain e -> "Certain " ^ string_of_mapping string_of_term string_of_term e
  | Uncertain e ->
      "Uncertain " ^ string_of_mapping string_of_term string_of_term e

let rec env = function Certain e -> e | Uncertain e -> e
...
let rec matchtermvars matchbra ispatvar pat term mrs =
  ...
  let rec eqt (t1, t2) mrs =
    if eqterms (t1, t2) then mrs
      ...
      (function Certain e -> Uncertain e | umr -> umr) <* mrs
    else []
  in
  let rec bindvar v t mr =
    ...
    | Some t -> (
        match eqt (term, t) [ mr ] with [ mr' ] -> Some mr' | _ -> None )
  in
  let rec matchlist f pats objs mrs =
    try nj_fold (uncurry2 (uncurry2 f)) (pats ||| objs) mrs with Zip_ -> []
  in
  ...
  let rec matchelement epat eterm mrs =
    ...
    | Element (_, _, tpat), Element (_, _, tterm) -> matchterm tpat tterm mrs
    | _ -> []
  in
  ...
  let rec bagmatch epats eterms mrs =
    ...
    let rec res rs =
      res' "(bagmatch) "
        (Collection (None, k, epats))
        (Collection (None, k, eterms))
        rs
    in
    (* both temporary *)
    ...
    | epat :: epats, _ :: _ ->
        let rec f t =
          match matchelement epat t mrs with [] -> None | mrs -> Some mrs
        in
  ...
  if null mrs then []
    ...
    | Collection (_, k, es), Collection (_, k', es') -> (
        ...
        | ListClass k, ListClass k' ->
            if k = k' then
              res' "(ListClass) " pat term (matchlist matchelement es es' mrs)
            else res' "?? (1) " pat term []
        | BagClass k, BagClass k' ->
            if k = k' then bagmatch es es' mrs else res' "?? (2) " pat term []
        | _ -> res' "?? (3) " pat term [] )
    | _ -> res' "?? " pat term []
...
let rec option_remapterm env term =
  let _Rvar v = match env <@> v with None -> v | Some t -> t in
  let rec _R = function
    | Id _ as v -> env <@> v
...
let rec simplepat term =
  ...
  let rec _S t =
    ...
    | Subst (_, r, _P, vts) ->
        registerSubst (r, mkid FormulaClass, (fun (v, t) -> (_S v, _S t)) <* vts)
...
let rec matchvars matchbra ispatvar pat t env =
  match3vars matchbra ispatvar pat t (Certain env) &~~ function
  | Certain e -> Some e
  | _ -> None
...
let rec matchtermvars matchbra ispatvar pat t envs =
  nj_fold
    (function Certain e, es -> e :: es | Uncertain e, es -> es)
    (match3termvars matchbra ispatvar pat t
       (List.map (fun v -> Certain v) envs))
    []
