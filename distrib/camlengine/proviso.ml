...
let mkBag els = registerCollection (Idclass.BagClass Idclass.FormulaClass, els)
...
let catelim_invisbracketedstring_of_proviso b p tail =
  ...
  | FreshProviso (h, g, r, v) ->
      ...
      :: ( match (h, g) with
         ...
         | false, false -> "????" )
      :: "FRESH "
      :: catelim_invisbracketedstring_of_term b v tail
  | UnifiesProviso (t1, t2) ->
      let good = List.exists (function Segvar _ -> true | _ -> false) in
      let easy =
        match (t1, t2) with
  ...
  | NotinProviso (t1, t2) ->
      catelim_invisbracketedstring_of_term b t1
        ( " NOTIN "
        :: catelim_invisbracketedstring_of_termOrCollection b "," t2 tail )
  | NotoneofProviso (vs, pat, _C) ->
      (* this madness IS used! *)
      catelim_string_of_list
        (catelim_invisbracketedstring_of_term b)
        "," vs
        ( " IN "
        :: catelim_invisbracketedstring_of_term b pat
             ( " NOTONEOF "
             :: catelim_invisbracketedstring_of_collection b "," _C tail ) )
  | DistinctProviso vs ->
      "DISTINCT "
      :: catelim_string_of_list
           (catelim_invisbracketedstring_of_term b)
...
let invisbracketedstring_of_proviso =
  stringfn_of_catelim <.> catelim_invisbracketedstring_of_proviso
...
let rec isFreshProviso = function FreshProviso _ -> true | _ -> false
...
let rec mkparentedvisproviso parent (vis, pro) =
  VisProviso { visible = vis; parent; actual = pro }
...
let rec provisoresetactual (VisProviso v) actual = VisProviso { v with actual }
...
let invisbracketedstring_of_visproviso =
  stringfn_of_catelim <.> catelim_invisbracketedstring_of_visproviso

let catelim_string_of_visproviso =
  catelim_invisbracketedstring_of_visproviso false

let string_of_visproviso = stringfn_of_catelim catelim_string_of_visproviso

let rec catelim_detailedstring_of_visproviso
    (VisProviso { visible; parent; actual }) tail =
  "{" :: string_of_bool visible :: ", actual="
  :: catelim_string_of_proviso actual
       (", parent=" :: catelim_string_of_proviso parent ("}" :: tail))

let detailedstring_of_visproviso =
  stringfn_of_catelim catelim_detailedstring_of_visproviso
        ...
        raise
          (ParseError_
             [
               ...
               " in ";
               pname;
               " proviso is not a variable or constant";
             ]))
    vars
...
let rec parseProvisos () =
  let rec parseNOTONEOF vars =
    ...
    if List.exists (not <.> ismetav) vars then
      bad
        [
          "not all the names ";
          bracketed_string_of_list string_of_term ", " vars;
          " are schematic";
        ];
    ...
    in
    let class__ = match classopt with Some c -> c | None -> listclass in
  ...
  | SHYID "DISTINCT" -> [ DistinctProviso (parseNOTINvars "DISTINCT") ]
  | sy ->
      if canstartTerm sy then
        ...
        let rec collbad s =
          raise
            (ParseError_
               [
                 "when parsing a proviso, symbol ";
                 s;
                 " found after collection ";
                 bk els;
               ])
        in
        ...
        | _, SHYID "UNIFIESWITH" ->
            ...
            let t1, t2 =
              ...
              | Some k, _, _ ->
                  (registerCollection (k, els), registerCollection (k, els'))
              | None, [ el ], [ el' ] -> (stripElement el, stripElement el')
              | _ ->
                  raise
                    (ParseError_
                       [
                         "can't determine collection class of ";
                         bk els;
                         " and ";
                         bk els';
                       ])
            ...
            raise
              (ParseError_
                 [
                   ...
                   string_of_list string_of_element ", " els;
                   " in provisos";
                 ])
      else
        raise
...
let earlierproviso p1 p2 =
  let rec lin1 = function
    | FreshProviso (h, g, r, v) ->
        (if h then 10 else 0) + (if g then 20 else 0) + if r then 40 else 0
    | DistinctProviso vs -> 300
    | NotinProviso (v, t) -> 400
    | NotoneofProviso (vs, pat, _C) -> 500
    | UnifiesProviso (t, t') -> 600
  in
  let rec lin2 = function
...
let provisoVIDs p = orderVIDs (vid_of_var <* provisovars termvars tmerge p)
  ...
  let ep p ps =
    ...
    | DistinctProviso vs ->
        let rec dp = function
          | [] -> ps
          | v :: vs ->
              foldr (fun v' ps -> DistinctProviso [ v; v' ] :: ps) (dp vs) vs
        in
        dp vs
    | p -> p :: ps
  in
...
let compressProvisos ps =
  ...
  *)
  ...
  let xys =
    sortunique (fun (_, xs) (_, ys) -> earlierlist earliervar xs ys) xys
  in
  ...
  let rec subsets = function
    ...
    | x :: xs ->
        ...
        foldr
          (fun ys yss ->
            if all (fun y -> List.mem [ x; y ] xy2s) ys then (x :: ys) :: yss
            else yss)
          xss xss
  in
  ...
  let rec comb = function
    | [] -> []
    | xs :: xss ->
        xs :: comb ((fun xs' -> sorteddiff earliervar xs' xs != []) <| xss)
  in
  let distincts =
    (function
      | [ x; y ] as xy ->
          if List.mem (false, xy) xys then _NotinProviso (x, y)
          else _DistinctProviso xy
      | xs -> _DistinctProviso xs)
    <* comb sets
  in
