...
val setworlds :
  (int * int) list ->
  ((int * int) * bool * (bool * string) list * (int * int) list) list
...
(* I toyed with the idea that enablemenuitem and tickmenuitem should have an argument which 
   ...
   RB 5/xi/02
 *)
val enablemenuitem : bool -> string -> string -> bool -> unit

(* applies to entries, checkboxes, radio buttons *)

val tickmenuitem : bool -> string -> string -> bool -> unit
...
(* don't extend beyond pos2; use linethickness from setproofparams *)

val drawRect : box -> string -> unit

(* stay inside the box; use linethickness from setproofparams *)
...
val askLemma :
  ...
  string ->
  string list ->
  string list (*  drule     thm       panels         provisos    *) ->
...
val askCancel_unpatched : int -> string -> string list -> int -> int option
...
val draginfo :
  (element * pos list) list ->
  (element * pos list) list ->
  (element list * element list) list ->
  unit
