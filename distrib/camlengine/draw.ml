...
let debugstring_of_plan f  (* was string_of_plan  *) (Formulaplan plan) =
  "Formulaplan"
  ^ string_of_triple string_of_textlayout string_of_textbox f "," plan
...
let textlayout_of_plan (Formulaplan (tl, _, _)) = tl

let textbox_of_plan (Formulaplan (_, b, _)) = b

let textbox_of_planlist ps =
  List.fold_left ( +|-|+ ) emptytextbox (List.map textbox_of_plan ps)

let info_of_plan (Formulaplan (_, _, i)) = i
...
let rec textinfo_procrustes w p ((size, Textlayout ts) as inf) =
  if (not !truncatereasons) || tsW size <= w then (false, inf)
    ...
    let rec crust ts =
      ...
      | t :: ts' ->
          crusty t |~~ fun _ ->
          crust ts' &~~ fun ts'' -> Some (t :: ts'')
    in
    match crust ts with
    | None -> (false, inf)
    | Some ts' ->
        let layout' = Textlayout ts' in
        (true, (textsize_of_textlayout layout', layout'))

(* let (rf, rs) = fontNstring_of_reason r in
   textinfo_of_string rf (Japeserver.procrustes w " ..." rf rs) *)

let plan_of_textinfo (size, layout) info p =
  Formulaplan (layout, textbox p size, info)

let plan_of_string font s info p =
...
let planfollowedby plan moref = plancons plan (moref (nextright_of_plan plan))

let planunwind tail stuff =
  let plancons' stuff plan = plancons plan stuff in
  List.fold_left plancons' tail stuff

let rec planfold thingf sepf ((p, tb, plans) as res) = function
  | [] -> res
  | [ thing ] ->
      let plan = thingf thing p in
...
let drawplan f p (Formulaplan (Textlayout tl, b, info)) =
  Japeserver.drawmeasuredtext (f info) tl (p +->+ tbP b)

let findfirstplanhit p =
  findfirst (fun pl ->
      if withintb p (textbox_of_plan pl) then Some pl else None)

let string_of_textinfo =
  string_of_pair string_of_textsize string_of_textlayout ","
