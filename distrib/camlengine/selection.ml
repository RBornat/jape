...
module
  ...
  struct
    ...
    let rec selection2Subst object__ sels cxt =
      ...
      let pvs = MAP ((fun v' -> v, v'), termvars origterm) in
      let extraps = MAP (NotinProviso, ( <| ) (indistinct cxt, pvs)) in
      ...
      let rec check t =
        ...
        if t = v then Some (List.hd ss')
          ...
          | Subst (_, r, P, vts) ->
              if E (MAP (sml__hash__1, vts)) then bb () else None
          | _ -> None
      in
