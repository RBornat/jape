...
let string_of_element =
  Termstring.debugstring_of_element Termstring.string_of_term
...
let rec fhitpath = function
  | ConcHit (p, _) -> Some p
  | HypHit (p, _) -> Some p
  | AmbigHit _ -> None

let rec hitpath = function FormulaHit h -> fhitpath h | ReasonHit p -> Some p

let rec selpath = function
  | FormulaSel f -> Some (fst_of_6 f)
  | TextSel (ths, _) -> (
      match optionmap (fhitpath <.> fst) ths with
      | Some (p :: ps) ->
...
let rec transelpath f sel =
  ...
  match sel with
  | FormulaSel (p, c, hs, tcs, ths, tg) ->
      FormulaSel (f p, c, hs, f3 tcs, f3 ths, tg)
  | TextSel (ths, tg) ->
      TextSel (List.map (fun (th, ss) -> (tranfhitpath f th, ss)) ths, tg)
  | ReasonSel p -> ReasonSel (f p)

let rec string_of_side = function Left -> "Left" | Right -> "Right"

let rec string_of_hitkind = function
  | HitPath -> "HitPath"
  | PrunePath -> "PrunePath"
...
let rec string_of_fhit string_of_path s =
  ...
  | AmbigHit pch ->
      "AmbigHit" ^ string_of_pair string_of_pelsi string_of_pel "," pch
...
let rec string_of_sel a1 a2 =
  ...
  | string_of_path, FormulaSel f ->
      ...
      ^ string_of_sextuple string_of_path
          (string_of_option string_of_elsi)
          (bracketed_string_of_list string_of_element ",")
          (bracketed_string_of_list
             (string_of_triple string_of_path string_of_elsi sstring ",")
             ",")
          (bracketed_string_of_list
             (string_of_triple string_of_path string_of_element sstring ",")
             ",")
          sstring "," f
