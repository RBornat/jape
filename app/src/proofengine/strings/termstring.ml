...
let rec catelim_debugstring_of_term t tail =
  ...
  | Id (h, v, c) ->
      "Id("
      :: catelim_string_of_option
           (catelim_of_stringfn string_of_int)
           h
           ( ",\"" :: string_of_vid v :: "\"," :: string_of_idclass c :: ")"
           :: tail )
  | Unknown (h, v, c) ->
      "Unknown("
      :: catelim_string_of_option
  ...
  | Collection (h, k, es) ->
      ...
      :: catelim_string_of_option
           ...
           ( "," :: string_of_idclass k :: ","
           :: dolist
                (catelim_debugstring_of_element catelim_debugstring_of_term)
                es (")" :: tail) )
...
let rec remake mapterm ((_, (bs, ss, us), env, pat) as b) =
  ...
  with Listfuns.Bad_nth ->
    raise
      (Catastrophe_ [ "Bad_nth in remake "; debugstring_of_term (Binding b) ])
...
let rec mapterm f t =
  ...
  | None -> (
      ...
      | Binding (_, (bs, ss, us), env, pat) ->
          Binding (None, (mtf <* bs, mtf <* ss, mtf <* us), env, pat)
      | Subst (_, r, p_, vts) ->
...
let rec stripelement = function
  | Element (_, _, t) -> t
  | Segvar (_, [], v) -> v
  | Segvar (_, p :: ps, v) -> App (None, p, stripelement (Segvar (None, ps, v)))

(* *********************** prettyprinter proper starts here ************************* *)

let rec firstatom = function
  | [] -> ""
  | "" :: ss -> firstatom ss
...
let rec tricolon a b =
  ...
  | atom, rest ->
      if mustseparate (atom, firstatom rest) then " " :: atom :: " " :: rest
      else atom :: rest
...
and insertspace = function
  | r :: rs as rest -> if r = invisket then r :: insertspace rs else " " :: rest
  | [] -> [ " " ]
...
let isJuxtapos t =
  ...
  | App (_, (App (_, f', _) as f), a) -> (
      match opsymb f' with Some (INFIXC _) -> None | _ -> Some (f, a) )
  | App (_, f, a) -> (
      match opsymb f with
      | Some (INFIX _) -> (
...
let rec _T ivb ivk n a t s =
  ...
  let rec tip_ n' assoc arg1 f arg2 s =
    quadcolon (_OB n')
      (let afterf =
         _T ivb ivk n' (assoc <> RightAssoc) arg2 (quadcolon (_CB n') s)
       in
       _T ivb ivk n' (assoc <> LeftAssoc) arg1 (tricolon f afterf))
  in
  let rec _TFA sy f t s =
    match sy with
    | PREFIX name ->
        let m = prio sy in
        quadcolon (_OBprefix m)
          (quadcolon name (_T ivb ivk m false t (quadcolon (_CBprefix m) s)))
    | POSTFIX name ->
        let m = prio sy in
        quadcolon (_OB m)
          (_T ivb ivk m false t (quadcolon name (quadcolon (_CB m) s)))
    | _ -> _TAP f t s
  in
  let rec _TT n b sep ts s =
    let rec _TT' = function
      | [] -> s
  ...
  | Unknown (_, v, _) ->
      ivb t :: quadcolon (metachar_as_string ^ string_of_vid v) (ivk t :: s)
  ...
  | Tup (_, sep, ts) ->
      let n' = match lookup sep with INFIX _ as sy' -> prio sy' | _ -> 0 in
      ivb t
      :: quadcolon (_OB n')
           (_TT n' true sep ts (quadcolon (_CB n') (ivk t :: s)))
  | Literal (_, Number k) ->
      (* take in the brackets as well ... *)
      ivb t :: quadcolon (string_of_int k) (ivk t :: s)
  | Literal (_, String k) -> ivb t :: quadcolon (("\"" ^ k) ^ "\"") (ivk t :: s)
  | Fixapp (_, ss, ts) -> (
      match lookup (List.hd ss) with
      | BRA _ ->
          ivb t
          :: quadcolon (List.hd ss)
               (_TS1 ivb ivk 0 (List.tl ss) false ts (ivk t :: s))
      | LEFTFIX _ as sy' ->
          let m = prio sy' in
          ivb t
          :: quadcolon (_OBprefix m)
               (quadcolon (List.hd ss)
                  (_TS2 ivb ivk m (List.tl ss) true ts
                     (quadcolon (_CBprefix m) (ivk t :: s))))
      | MIDFIX _ as sy' ->
          let m = prio sy' in
          ivb t
          :: quadcolon (_OB m)
               (_TS2 ivb ivk m ss true ts (quadcolon (_CB m) (ivk t :: s)))
      | RIGHTFIX _ as sy' ->
          let m = prio sy' in
          ivb t
          :: quadcolon (_OB m)
               (_TS1 ivb ivk m ss true ts (quadcolon (_CB m) (ivk t :: s)))
      | sy' ->
          raise
            (Catastrophe_ [ "Matchintermstring_ "; debugstring_of_symbol sy' ])
      )
  | Subst (_, _, t, m) ->
      ivb t
...
and _TS ivb ivk ts r =
  ...
  | t :: ts -> _T ivb ivk 0 true t (quadcolon "," (_TS ivb ivk ts r))
...
and _TS1 ivb ivk m seps b ts s =
  ...
  | sep :: seps, t :: ts ->
      ...
      _T ivb ivk m b t (quadcolon sep (_TS1 ivb ivk m seps b ts s))
  | _ -> quadcolon "???_TS1???" s
...
and _TS2 ivb ivk m seps b ts s =
  ...
  | sep :: seps, t :: ts ->
      _T ivb ivk m b t (quadcolon sep (_TS2 ivb ivk m seps b ts s))
  | _ -> quadcolon "???_TS2???" s
...
let rec diag_string_of_term t =
  (if !termstringdebug then debugstring_of_term else string_of_term) t
...
let catelim_invisbracketedstring_of_termfun b =
  catelim_invisbracketedstring_of_prioterm b !appfix false
...
let rec catelim_string_of_termarg =
  catelim_invisbracketedstring_of_termarg false
...
let catelim_invisbracketedstring_of_collection b sep t =
  ...
  | _ ->
      raise
        (Catastrophe_ ("string_of_collection " :: catelim_string_of_term t []))

let invisbracketedstring_of_collection b sep =
  stringfn_of_catelim (catelim_invisbracketedstring_of_collection b sep)

let catelim_string_of_collection =
  catelim_invisbracketedstring_of_collection false

let string_of_collection sep =
  stringfn_of_catelim (catelim_string_of_collection sep)
...
let catelim_string_of_termOrCollection =
  catelim_invisbracketedstring_of_termOrCollection false

let string_of_termOrCollection sep =
  stringfn_of_catelim (catelim_string_of_collection sep)

let string_of_termlist = bracketed_string_of_list string_of_term ";"

let catelim_string_of_termlist =
