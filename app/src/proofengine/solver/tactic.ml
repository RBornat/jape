...
let rec catelim_string_of_tactic sep t tail =
  ...
  let rec trpathexpr p tail =
    ...
    | Subgoal (p, ns) ->
        "(SUBGOAL" :: argsep :: trpathexpr p (argsep :: trns ns (")" :: tail))
  ...
  let tss =
    ...
    | UnfoldTac (s, tacs) -> "UNFOLD " :: pnstr s :: nextsep :: trtacs tacs tail
    | FoldTac (s, tacs) -> "FOLD " :: pnstr s :: nextsep :: trtacs tacs tail
    | AssignTac (s, term) ->
    ...
    | BindHyp2Tac (t1, t2, tactic) -> "LETHYP2 " :: termsNtac [ t1; t2 ] tactic
    | BindHypsTac (term, tactic) -> "LETHYPS " :: termsNtac [ term ] tactic
    | BindSubstTac (term, tactic) -> "LETSUBSTSEL " :: termsNtac [ term ] tactic
    ...
    | LayoutTac (tactic, layout) ->
        "LAYOUT " :: string_of_treelayout layout :: argsep :: unSEQ tactic tail
    | AssocFlatTac term -> "FLATTEN " :: catelim_string_of_termarg term tail
    | MapTac (s, terms) -> "MAPTERMS " :: pnstr s :: " " :: trterms terms tail
    ...
    | SubstTac (s, vts) ->
        catelim_string_of_term
          (Subst
             ( None,
               true,
               Id (None, vid_of_string (pnstr s), FormulaClass),
               ...
               | Some lookup ->
                   let rec encodeterm t =
                     ...
                     | _ -> (
                         ...
                         | None -> t )
                   and encodeelement = function
                     | Element (_, i, t) -> Element (None, i, encodeterm t)
                     | el -> el
                   in
    ...
    | WithSelectionsTac tac -> "WITHSELECTIONS" :: argsep :: unSEQ tac tail
    ...
    | ApplyOrResolveTac tac -> "APPLYORRESOLVE" :: argsep :: unSEQ tac tail
    ...
    | AlertTac (m, ps) ->
        ...
        :: catelim_string_of_termarg m
             (catelim_string_of_list
                ...
                argsep ps tail)
    | ShowHowToTac s ->
        "SHOWHOWTO" :: argsep :: catelim_string_of_termarg s tail
...
and catelimstring_of_tacarg sep t tail =
  let ok = function SkipTac -> true | TermTac (s, []) -> true | _ -> false in
...
let stringwithNLs_of_tactic =
  stringfn_of_catelim catelim_stringwithNLs_of_tactic

let remaptactic env t =
  let _E = remapterm env in
  let rec _Ep = function
  ...
  let rec _T t =
    ...
    | SubstTac (s, vts) -> SubstTac (s, (fun (v, t) -> (_E v, _E t)) <* vts)
    ...
    | BindTuplistTac (carpat, cdrpat, expr, t) ->
        BindTuplistTac (_E carpat, _E cdrpat, _E expr, _T t)
    | BindMatchTac (ptm, vtm, t) -> BindMatchTac (_E ptm, _E vtm, _T t)
    | BindOccursTac (pt, vt, st, t) -> BindOccursTac (_E pt, _E vt, _E st, _T t)
...
let mustbeSTR term =
  ...
  | _ ->
      raise (TacParseError_ [ string_of_termarg term; " should be a string" ])

let maybeSTR term = try Some (mustbeSTR term) with _ -> None

let tacname term =
  try Name (mustbeSTR term)
  with TacParseError_ _ ->
...
let explodeForExecute t =
  let f, rs = explodeApp false t in
  (tacname f, rs)
...
let badLayout () =
  raise
    (TacParseError_
       [
         "Syntax is LAYOUT fmt tactic; fmt can be HIDEROOT, or HIDECUT, or \
          COMPRESS label subtrees, or just label subtrees; label can be an str \
          or a non-empty bracketed tuple of strs where an str can be a string, \
          an identifier, an unknown or a number;\n\
         \        subtrees must be a bracketed tuple of numbers or the word \
          ALL. \n\n\
          It's possible to stop after label, in which case ALL is assumed; \
          it's possible to stop after COMPRESS, in which case \"%s\" ALL is \
          assumed.";
       ])
...
let checkINTS errf t =
  match debracket t with
  | Tup (_, ",", ts) as t' ->
      let _ = List.map (checkINT errf) ts in
      t'
  | t -> checkINT errf t
...
let rec mkSEQ = function [] -> SkipTac | [ t ] -> t | ts -> SeqTac ts
...
and transTactic tacterm =
        ...
        *)
        let vts =
          if !stripextrabag then
            match
              split (function _, Collection _ -> true | _ -> false) vts
            with
            | [ _ ], oks ->
                (* if not (exists (existsterm isUnknown) (#2 MAP vts)) then oks else vts *)
                oks
        ...
        match vts with [] -> TermTac (_P, []) | _ -> SubstTac (_P, vts) )
    | t -> (
        let ((n, ts) as parts) = explodeForExecute t in
        let f = string_of_name n in
        let _Bad why =
          raise (TacParseError_ [ string_of_term t; " -- "; why ])
        in
        let rec _Assignments = function
          | [] -> []
          | s :: t :: ts ->
        ...
        in
        let atleasttwo = function
          | _ :: _ :: _ as ts -> ts
          | ts -> _Bad "should have at least two arguments"
        in
        let mkBind f bind str tac = function
          | x :: ts -> tac (bind x, _SEQTAC ts)
          | [] ->
              raise
                (TacParseError_
                   [ "Syntax is ("; f; " <"; str; "> tactics ...)" ])
        in
        let mkBind2 f b1 b2 s1 s2 tac = function
          | x1 :: x2 :: ts -> tac (b1 x1, b2 x2, _SEQTAC ts)
          | _ ->
              raise
                (TacParseError_
                   [ "Syntax is ("; f; " <"; s1; "> <"; s2; "> tactic ...)" ])
        in
        let mkBind3 f b1 b2 b3 s1 s2 s3 tac = function
        ...
        let mkLayout = function
          | [] -> badLayout ()
          | fmt :: stuff -> (
              let lyt con fmt = function
                | [] -> transLayout con fmt None []
              ...
              | Some "COMPRESS", fmt :: ts ->
                  lyt (fun v -> CompressedLayout v) fmt ts
              | Some "COMPRESS", ts ->
                  lyt
                    (fun v -> CompressedLayout v)
                    (registerLiteral (String "%s"))
                    ts
              | _, ts -> lyt (fun v -> NamedLayout v) fmt ts )
        in
        let mkFold tac = function
          ...
          | m :: ps ->
              let rec f rs = function
                ...
                | t :: _ ->
                    raise
                      (TacParseError_
                         [ "badly formed argument "; string_of_term t ])
              in
              f [] ps
        ...
        let rec parsegoalexpr t =
          let err _ =
            [ "path expected after GOALPATH -- found "; string_of_term t ]
          in
        ...
        | "LETHYP2" ->
            mkBind2 f patbind patbind "pattern" "pattern"
              (fun v -> BindHyp2Tac v)
              ts
        ...
        | "LETGOALPATH" ->
            mkBind f namebind "name" (fun v -> BindGoalPathTac v) ts
        | "LETOPENSUBGOAL" ->
            mkBind2 f namebind patbind "name" "pattern"
              (fun v -> BindOpenSubGoalTac v)
              ts
        | "LETOPENSUBGOALS" ->
            mkBind f patbind "pattern" (fun v -> BindOpenSubGoalsTac v) ts
        | "LETARGSEL" -> mkBind f patbind "pattern" (fun v -> BindArgTac v) ts
        | "LETARGTEXT" ->
            mkBind f namebind "name" (fun v -> BindArgTextTac v) ts
        | "LETSUBSTSEL" ->
            mkBind f patbind "pattern" (fun v -> BindSubstTac v) ts
        | "LETHYPSUBSTSEL" ->
            mkBind f patbind "pattern" (fun v -> BindSubstInHypTac v) ts
        | "LETCONCSUBSTSEL" ->
            mkBind f patbind "pattern" (fun v -> BindSubstInConcTac v) ts
        | "LETMULTIARG" ->
            mkBind f patbind "pattern" (fun v -> BindMultiArgTac v) ts
        | "LETHYPFIND" ->
            mkBind f patbind "pattern" (fun v -> BindFindHypTac v) ts
        | "LETCONCFIND" ->
            mkBind f patbind "pattern" (fun v -> BindFindConcTac v) ts
        | "LETLISTMATCH" ->
            mkBind3 f patbind patbind valbind "car-pattern" "cdr-pattern" "expr"
              (fun v -> BindTuplistTac v)
              ts
        | "LETMATCH" ->
            mkBind2 f patbind valbind "pattern" "expr"
              (fun v -> BindMatchTac v)
              ts
        | "LETOCCURS" ->
            mkBind3 f valbind valbind patbind "subexpr" "expr"
              "substitution expr"
              (fun v -> BindOccursTac v)
              ts
        | "LAYOUT" -> mkLayout ts
        ...
        | "BADUNIFY" ->
            mkBind2 f namebind namebind "name" "name"
              (fun v -> BadUnifyTac v)
              ts
        | "BADMATCH" ->
            mkBind2 f namebind namebind "name" "name"
  ...
  | TacParseError_ ss ->
      raise
        (ParseError_ ([ "Bad tactic: ("; string_of_term tacterm; ") -- " ] @ ss))
...
and transLayout con fmt bopt ts =
  ...
  let nserr t =
    [
      ...
      string_of_term (_The bopt);
    ]
  in
  let bopt =
    match optf (checkINTS nserr) bopt with
    | Some (Id (_, v, _)) as r -> (
        match string_of_vid v with "ALL" -> None | _ -> r )
    | r -> r
  in
