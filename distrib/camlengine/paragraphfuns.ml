...
let provisovars = Proviso.provisovars Termfuns.termvars Termfuns.tmerge
...
let rec interpret report query where params provisos enter
    (paragraph, ((env, proofs, buttonfns) as res)) =
  let rec _Par = function [] -> params | ps -> ps in
  let rec _Pro = function [] -> provisos | ps -> ps in
  let rec newbuttonenv env text var settings defval =
    ...
    let env' =
      try
        ( try checkrange env var settings
          with OutOfRange_ range ->
            report
              [
                text;
                " settings are ";
                ...
                " - variable ";
                parseablestring_of_name var;
                " can only be set to ";
                range;
              ];
            raise Use_ );
        ( try checkrange env var [ defval' ]
          with OutOfRange_ range ->
            report
              [
                text;
    ...
    in
    Japeenv.stringset env' var defval';
    (env', defval')
  in
  let rec processCheckBox env (var, label, (vval1, vval2), defvalopt) =
    let env, _ = newbuttonenv env "CHECKBOX" var [ vval1; vval2 ] defvalopt in
    let rec doit a1 a2 =
      match (a1, a2) with
      | f, ("true", true) -> f true
  ...
  let rec processRadioButton env (var, entries, defval) =
    ...
    let rec basefn f (v, tf) =
      match atmapping map v with Some label -> f (label, tf) | None -> ()
    in
    (env, basefn)
  in
  let rec filterpros vars ps =
    ...
    | [] ->
        (fun p ->
          not (List.exists (fun v -> not (member (v, vars))) (provisovars p)))
        <| provisos
    | ps -> ps
  in
  ...
  | AutoRule (sense, commands) ->
      List.iter (fun com -> addautorule (sense, com)) commands;
      res
  | Conjecture (RuleHeading (name, params, provisos), sequent) ->
      let svs = seqvars sequent in
      let thing =
        Theorem
          ( filterparams svs params,
            visprovisos (filterpros svs provisos),
            sequent )
      in
  ...
  | InitVar (name, term) ->
      let rec lreport ss =
        ...
        raise Use_
      in
      ( try Japeenv.termset env name term with
      | OutOfRange_ range ->
          lreport
            [
              " to ";
              string_of_term term;
              " - variable can only be set to ";
              range;
            ]
      | NotJapeVar_ -> lreport [ " - it isn't a variable in the environment" ]
      | ReadOnly_ ->
          lreport
  ...
  | MacroDef (TacticHeading (name, params), macro) ->
      addthing (name, Macro (params, macro), where);
      res
  | Menu (mproof, mlabel, mparas) ->
      ...
      let rec processcommand (mp, (env, buttonfns, mcs, mps (* as res *))) =
        ...
        | Menustuff c -> (
            match c with
            | MCdata e ->
                let env, buttonfns = processdata e env buttonfns in
      ...
      and processdata e env buttonfns =
        ...
        | Mcheckbox ((var, label, _, _) as stuff) ->
            ...
            let rec tickfn tf =
              setmenuentry (string_of_name mlabel) mproof (string_of_name label)
                None
                ( "assign "
                ^ parseablestring_of_name var
                ^ if tf then " false" else " true" );
              tickmenuitem mproof (string_of_name mlabel) (string_of_name label)
                tf
            in
      ...
      nj_revfold
        (interpret report query (InMenu mlabel) params provisos enter)
        mps (env, proofs, buttonfns)
  ...
  | StructureRule (stype, rule) ->
      addstructurerule report query stype rule;
      res
  | Proof
    ...
    ->
      ...
      | [], None ->
          addthing (name, Theorem (params, visprovisos provisos, seq), where)
      | _ :: _, None ->
          addthing
            ( name,
      ...
      let res =
        match
          doProof report query env name stage seq
            (params, givens, provisos, tac)
            disproofopt
        with
      ...
      let thing =
        Rule
          ( ( filterparams rvs params,
              visprovisos (filterpros rvs provisos),
              top,
              bottom ),
            axiom )
      in
  ...
  | TacticDef (TacticHeading (name, params), tac) ->
      addthing (name, Tactic (params, tac), where);
      res
...
and addalttactic name paras params where =
  addthing
    (name, Tactic (params, TheoryAltTac (optionfilter rulename paras)), where)
...
and interpretParasFrom report query res filenames =
  freezesaved ();
  (* profileOff(); profileReset(); *)
  (* profileOn(); *)
  let r =
    try
      nj_revfold
        (interpret report query InLimbo [] [] true)
        (nj_fold
           (fun (x, y) -> x @ y)
           (paragraphs_of_file report query <.> disQuote <* filenames)
           [])
        res
    with
    | Catastrophe_ ss ->
        (* profileOff(); *)
        report ("Catastrophic error: " :: ss);
        thawsaved ();
        raise Use_
    | CompileThing_ ss ->
        (* profileOff(); *)
...
let rec conjecturename = function
  | Conjecture (RuleHeading (name, _, _), _) -> name
  | _ -> raise Use_

let interpret report query params provisos res paragraph =
  (* for export *)
