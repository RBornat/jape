...
type process_id = Unixoid of int | Windowsoid of (in_channel * out_channel)

let ignorePipeSignals () =
  if os_type = "Win32" then (* I daresay we'll have to think of something *)
    ()
  else Sys.set_signal Sys.sigpipe Sys.Signal_ignore
...
let execute_in_env cmd args env =
  if os_type <> "Win32" then (
    ...
    let pid =
      Unix.create_process_env cmd (Array.of_list args) (Array.of_list env)
    ...
    (Windowsoid (inchan, outchan), inchan, outchan)
...
let reap = function
  ...
  | Windowsoid chans -> (
      try
        let _ = Unix.close_process chans in
        ()
      with _ -> () )
