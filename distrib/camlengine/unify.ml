...
module
  ...
  struct
    ...
    let rec abstract orig map term ps cxt =
      ...
      in
      let rec ourprovisos () =
        ps @ MAP ((fun p -> mkvisproviso (true, p)), !newprovisos)
      in
      let rec okname cxt =
        fun P m ->
          ...
          | Maybe ->
              List.iter addproviso
                (MAP ((fun v -> NotinProviso (v, P)), realmapdom));
              Some (cxt, P)
          | Yes -> None
      in
      let rec showAres m =
    ...
    and subststep cxt =
      fun (r, P, m as s) ->
        ...
        let r =
          ...
          | Tup (_, sep, ts) -> Some (cxt, registerTup (sep, MAP (S, ts)))
          | Literal k -> Some (cxt, debracket P)
          | Fixapp (_, bras, ts) ->
              Some (cxt, registerFixapp (bras, MAP (S, ts)))
          | Binding (_, (bs, ss, us), env, pat) ->
              let cxt = rewritecxt cxt in
              let m = rewritesubstmap cxt m in
              let bs = MAP (rewrite cxt, bs) in
              ...
              in
              let (bs', ns, cxt) = fold newb bs ([], [], cxt) in
              Some
                (plusvisibleprovisos (cxt, MAP (NotinProviso, allpairs bs')),
                 registerBinding
                   ((bs',
                     MAP
                       ((fun s ->
                        ...
                        ss),
                     MAP ((fun u -> registerSubst (r, u, m)), us)),
                    env, pat))
          | Subst (_, r', P', m') ->
              let cxt = rewritecxt cxt in
    ...
    let rec unify a1 a2 a3 a4 =
      ...
      | [], dds, true, cxt -> unify (MAP (sml__hash__2, dds)) [] false cxt
      | (t1orig, t2orig) :: tts, dds, progress, cxt ->
            ...
            | Binding (_, b1, _, pat1), Binding (_, b2, _, pat2) ->
                if pat1 = pat2 then
                  if List.exists
                       ...
                       (MAP
                          ((fun (t1, t2) ->
                              sml__hash__2 (simp cxt t1),
                              sml__hash__2 (simp cxt t2)),
                           tts))
                  then
                    delay cxt
                  else
          ...
          and alignbindings ((b1s, s1s, u1s), (b2s, s2s, u2s)) cxt =
                ...
                match v1, v2 with
                  Id (_, vid1, c1), Id (_, vid2, c2) ->
                    let rec notins cxt v ss =
                      plusvisibleprovisos
                        (cxt, MAP ((fun s -> NotinProviso (v, s)), ss))
                    in
                    let fs = facts (provisos cxt) cxt in
            ...
            let (cxt, alls, m1, m2, extras) =
              fold newb (( ||| ) (MAP (debracket, b1s), MAP (debracket, b2s)))
                (cxt, [], [], [], [])
            in
            let news1s = if null m1 then s1s else MAP (news m1, s1s) in
            let news2s = if null m2 then s2s else MAP (news m2, s2s) in
            plusvisibleprovisos (cxt, MAP (NotinProviso, allpairs alls)),
    ...
    and unifycleverly tts dds cxt =
      ...
      match tts with
        (reason, (t1, t2) as tt) :: tts ->
          let rec yes tts' cxt' =
            unify (tts' @ MAP (sml__hash__2, tts @ dds)) [] true cxt'
          in
        ...
        (reason, (t1, t2) as tt) :: tts ->
          ...
          begin match reason, (t1', t2') with
            DeferAlignment, (Subst (_, _, P1, m1), Subst (_, _, P2, m2)) ->
              ...
              with
                Some mms ->
                  unify ((P1, P2) :: mms @ MAP (sml__hash__2, tts @ dds)) []
                    true cxt
    ...
    and zipvts pdb (vts1, vts2) =
      ...
      let rec zipup (tt1s, tt2s) =
        fold (fun (((v1, t1), (v2, t2)), mms) -> (v1, v2) :: (t1, t2) :: mms)
          (( ||| ) (tt1s, tt2s)) []
      in
      let rec ok (d1, d2) =
        Some
    ...
    and unifycollections kind (e1s, e2s) cxt =
          ...
          match e with
            Segvar (_, [], Unknown (_, v, _)) ->
              begin match at (varmap cxt, v) with
                Some (Collection (_, _, es')) -> fold expand es' (cxt, es)
              | _ -> cxt, e :: es
              end
          | Segvar (_, ps, Unknown (_, v, _)) ->
              begin match at (varmap cxt, v) with
                Some (Collection (_, _, es')) ->
                  (* fold expand (modeifyelement ps MAP es') (cxt,es) *)
        ...
        let rec uel a1 a2 a3 a4 =
          match a1, a2, a3, a4 with
            defers, cxt, (r, t), el :: es ->
              ...
              let recv = MAP (push, uel defers cxt (r, t) es) in
              begin match el with
                Element (_, r', t') ->
                  begin match
                    andthenr
                  ...
                  with
                    Some cxt' ->
                      (cxt', MAP (sml__hash__1, defers) @ es) :: recv
                  | None -> recv
                  end
          ...
          | defers, cxt, (r, t), [] ->
              ...
              | _ ->
                  let segvs = MAP (sml__hash__1, defers) in
                  let (cxt, newsegvs) =
                    fold
        ...
        let rec dive () =
          ...
          match extract isel e1s with
            Some (Element (_, r, t), e1s) ->
              res (flatten (MAP (ub e1s, uel [] cxt (r, t) e2s)))
          | _ ->
              ...
              | _ ->
                  match extract isel e2s with
                    Some (Element (_, r, t), e2s) ->
                      res (flatten (MAP (ub e2s, uel [] cxt (r, t) e1s)))
                  | _ ->
      ...
      let rec unifylists (e1s, e2s) cxt =
        ...
        let rec def ps svs es es' =
          res (flatten (MAP (ul svs es, allsplits ps cxt es')))
        in
    ...
    let rec unifyvarious pair cxt =
      flatten (MAP (checkdeferred pair, unifyv pair cxt))
