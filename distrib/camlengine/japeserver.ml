...
let deadGUI () =
  consolereport
    [
      "the GUI is not responding";
      (if !_GUIresponded then "" else " (and has never responded)");
    ];
  raise DeadGUI_

(* if the server is dead, this will definitely cause a problem *)
let flush s =
  try flush outtoGUI with
  | Sys_error _ -> deadGUI ()
  | exn ->
      consolereport [ Printexc.to_string exn; " in Japeserver.flush "; s ];
      deadGUI ()
...
let out s =
  ...
  *)
  try output_string outtoGUI s with
  | Sys_error _ -> deadGUI ()
  | exn ->
      consolereport [ Printexc.to_string exn; " in Japeserver.out " ];
      deadGUI ()
...
and vis c =
  if isInvisibleUcode c then "\\" ^ string_of_int c else utf8_of_ucode c

(* if the server has crashed, input_line may give an exception *)
let readline s =
  flush s;
  let r =
    try input_line infromGUI with
    | End_of_file -> deadGUI ()
    | exn ->
        consolereport [ Printexc.to_string exn; " in Japeserver.readline "; s ];
        deadGUI ()
  in
  _GUIresponded := true;
  r

let rec readlines s = function 0 -> [] | n -> readline s :: readlines s (n - 1)

(* this is a strange bit of code.  I _think_ it converts a space-separated sequence of numbers
   into a list of numbers.
 *)
let rec nn_ = function
...
let rec writef orig_s orig_is =
  ...
  let rec ww_ cs is =
    ...
    | 0x25 :: 0x25 :: cs, is ->
        out (utf8_of_ucode 0x25);
        ww_ cs is
    | 0x25 :: cs, Bool b :: is ->
        out (if b then "T" else "F");
        ww_ cs is
    | 0x25 :: cs, Int i :: is ->
        out (signedstring_of_int i);
        ww_ cs is
    | 0x25 :: cs, Str s :: is ->
        outs s;
        ww_ cs is
...
let intlist_of_ask s is =
  writef s is;
  ints_of_reply (readline s)

let string_of_ask s is =
  writef s is;
  readline s
...
let rec getPointSize n =
  ...
  | [ n ] -> n
  | _ ->
      output_string stderr "[POINTSIZE FAILS]\n";
      0

let invischars : string list ref = ref []

let rec printable s = utf8_implode (not <.> isInvisibleUcode <| utf8_explode s)
...
let setFontNames fs =
  ...
  else fontnames := Array.of_list fs

let rec getfontname n =
  try !fontnames.(n)
  with Invalid_argument _ ->
    if length !fontnames = 0 then (
      (* we never initialised it *)
      writef "FONTNAMES\n" [];
      ...
      raise
        (Catastrophe_
           [
             "Japeserver.getfontname can't decode fontnumber "; string_of_int n;
           ])
...
let rec measurestring font string =
  ...
  with Not_found ->
    let wad =
      match
        intlist_of_ask "STRINGSIZE % %\n"
          [ Int fontnum; Str (printable string) ]
      with
...
let rec loadFont (fontn, name) = writef "LOADFONT % %\n" [ Int fontn; Str name ]

(***************)

let linethickness = ref 1

let rec setinvischars (onbra, onket) (offbra, offket) (outbra, outket)
    (lockbra, lockket) =
  invischars :=
    ...
    [
      Int
        ...
        | _ ->
            raise
              (Catastrophe_
                 [ "Japeserver.settextselectionmode "; Stringfuns.enQuote m ])
        );
    ]
  ...
  writef "SETMULTIHYPSEL %\n"
    [
      Bool
        ...
        | _ ->
            raise
              (Catastrophe_ [ "Japeserver.multihypsel "; Stringfuns.enQuote m ])
        );
    ]
...
let procrustes width ellipsis font text =
  if fst_of_3 (measurestring font text) <= width then text
    ...
    try Hashtbl.find procrusteanCache arg
    with Not_found ->
      let trunc =
        writef "PROCRUSTES % % % %\n"
...
let rec ask_unpatched severity message buttons default =
  ...
  | [ n ] ->
      n
      (* warning: can be -1 if the interface doesn't stop Windoze users closing alerts *)
...
let rec askDangerously_unpatched message doit dont =
  ...
  | ns ->
      raise
        (Catastrophe_
           [
             "askDangerously protocol failure ";
...
let existsmenu name =
  name = "File" || name = "Edit" || List.exists (fun m -> m = name) !menus
...
let rec closeproof number report =
...
let rec menuentry menu proofsonly label keyopt entry =
  writef "MENUITEM % % % % %\n"
    [
      Str menu;
      Str label;
      Str (match keyopt with Some s -> s | None -> " ");
      Str entry;
...
let rec mapmenus = function true -> makemenusVisible () | false -> ()
...
let rec filetypeToString = function
  | 0 -> "jt"
  | 1 -> "j"
  | 2 -> "jp"
  | 3 -> "jdb"
...
let rec readFileName message filetype =
  ...
  match readline "READFILENAME" with "" -> None | s -> Some s

let rec writeFileName message filetype =
  writef "WRITEFILENAME % %\n" [ Str message; Str (filetypeToString filetype) ];
  match readline "WRITEFILENAME" with "" -> None | s -> Some s
...
let positioned_textsels id =
  ...
  while
    ...
    | x :: y :: ss ->
        l := (pos (atoi x) (atoi y), ss) :: !l;
        true
    | _ ->
        (* consolereport ["textselections terminate with "; Stringfuns.enQuote line]; *)
        false
  do
    ()
  done;
...
let rec getProofSelections () =
  ...
  while
    ...
    | [ x; y; classn ] ->
        l := (pos x y, displayclass_of_int classn) :: !l;
        true
    | _ ->
        (* consolereport ["getFormulaSelection terminates on "; Stringfuns.enQuote line]; *)
        false
  do
    ()
  done;
...
let rec nontrivial_selections line =
  match strings_of_reply line with [ "" ] -> [] | xs -> xs
...
let rec getDisproofSelections () =
  ...
  while
    ...
    | [ x; y ] ->
        ...
        true
    | _ ->
        (* consolereport ["getDisproofSelections terminates on "; Stringfuns.enQuote line]; *)
        false
  do
    ()
  done;
...
let rec draginfo s2pos t2pos els2els =
  ...
  let posof map e =
    ...
    | None ->
        raise
          (Catastrophe_
             [
               "Japeserver.draginfo no mapping for ";
               string_of_element e;
               " in ";
  ...
  let writedragmap (ss, ts) =
    let wf n = writef "% " [ _Int n ] in
    writelist " " " " wf (posof s2int <* ss);
    writef "\n" [];
...
let rec setworlds selected worlds =
  writef "WORLDSSTART\n" [];
  List.iter
    (fun ((cx, cy), emphasis, labels, children) ->
...
let int_of_displaystyle d = match d with BoxStyle -> 0 | TreeStyle -> 1
...
let askUnify s =
  writef "ASKUNIFY %\n" [ Str s ];
  match readline "ASKUNIFY" with "" -> None | s -> Some s
...
let askLemma druleString thmString panels provisos =
  writef "ASKLEMMA % % % %\n"
    [
      Str druleString;
      Str thmString;
      Int (List.length panels);
      Int (List.length provisos);
    ];
  List.iter
    (fun s ->
      out s;
