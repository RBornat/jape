...
let profileswitcher b = profiling := b

(* ; if b then Profile.profileOn () else Profile.profileOff () *)
...
let defaultenv =
  ...
  let pairs =
    [
      ...
      ( "applyconjectures",
        sj [ "none"; "all"; "rules"; "theorems" ] "none" applyconjectures );
      ...
      ( "textselectionmode",
        sj [ "subformula"; "token" ] "subformula" textselectionmode );
...
let boxdisplaynames =
  [
    ...
    "hidereflexivity";
    "hideuselesscuts";
    "multiassumptionlines";
  ]

let treedisplaynames = [ "foldsequents" ]

let displayvars =
  List.map Name.name_of_string
    (displaynames @ boxdisplaynames @ treedisplaynames)
...
let mustredisplay env vals =
  ...
  in
  let lookup s =
    match nenv <@> s &~~ fun n -> Japeenv.( <@> ) env n with
    | Some t -> Some (string_of_term t)
    | None -> None
  in
  let changed s = lookup s <> (nenv <@> s &~~ fun n -> dispenv <@> n) in
...
let profileOn (* Profile.profileOn *) _ = ()

let profileOff (* Profile.profileOff *) _ = ()

let profileReset (* Profile.reset *) _ = ()

let profileReport (* Profile.report *) _ = ()

let disproof_finished = function
  | Some state ->
      let res =
...
let string_of_hist f (Hist h) =
  "Hist{now=" ^ f h.now ^ "; pasts="
  ^ bracketed_string_of_list f ";" h.pasts
  ^ "; futures="
  ^ bracketed_string_of_list f ";" h.futures

type winhistrec = {
  changed : bool;
  proofhist : proofstate hist;
  disproofhist : disproofstate hist option;
}

type winhist = WinHist of winhistrec

let winhist_changed (WinHist { changed }) = changed

let winhist_proofhist (WinHist { proofhist }) = proofhist

let winhist_disproofhist (WinHist { disproofhist }) = disproofhist

let winhist_proofnow (WinHist { proofhist = Hist { now } }) = now

let winhist_disproofnow = function
  | WinHist { disproofhist = Some (Hist { now }) } -> Some now
  | _ -> None

let hist_now (Hist { now }) = now

let hist_pasts (Hist { pasts }) = pasts

let hist_futures (Hist { futures }) = futures

let withchanged (WinHist wh) changed = WinHist { wh with changed }

let withproofhist (WinHist wh) proofhist = WinHist { wh with proofhist }

let withdisproofhist (WinHist wh) disproofhist =
  WinHist { wh with disproofhist }

let new_hist now = Hist { now; pasts = []; futures = [] }

let withnow (Hist h) now = Hist { h with now }

let withpasts (Hist h) pasts = Hist { h with pasts }

let withfutures (Hist h) futures = Hist { h with futures }

(* there are several things we can do with a history *)

let forward_step (Hist { now; pasts }) now' =
  Hist { now = now'; pasts = now :: pasts; futures = [] }

let insert_step (Hist { now; pasts; futures }) now' =
  Hist { now = now'; pasts; futures = now :: futures }

let append_step (Hist { now; pasts; futures }) now' =
  Hist { now = now'; pasts = now :: pasts; futures }

let redo_step = function
  | Hist { now; pasts; futures = now' :: futures } ->
      Some (Hist { now = now'; pasts = now :: pasts; futures })
  | _ -> None

let undo_step = function
  | Hist { now; pasts = now' :: pasts; futures } ->
      Some (Hist { now = now'; pasts; futures = now :: futures })
  | _ -> None
...
let string_of_proofinfo (Pinf { title; proofnum }) =
  "Pinf{title="
  ^ Stringfuns.string_of_pair string_of_name string_of_int "," title
  ^ "; proofnum=" ^ string_of_int proofnum ^ "}"

let proofinfo_title (Pinf { title }) = title

let proofinfo_proofnum (Pinf { proofnum }) = proofnum

let proofinfo_displayvarsvals (Pinf { displayvarsvals }) = displayvarsvals

let proofinfo_needsrefresh (Pinf { needsrefresh }) = needsrefresh

let proofinfo_displaystate (Pinf { displaystate }) = displaystate

let proofinfo_hist (Pinf { hist }) = hist

let proofinfo_fromstore (Pinf { fromstore }) = fromstore

let withtitle (Pinf pi) title = Pinf { pi with title }

let withproofnum (Pinf pi) proofnum = Pinf { pi with proofnum }

let withdisplayvarsvals (Pinf pi) displayvarsvals =
  Pinf { pi with displayvarsvals }

let withneedsrefresh (Pinf pi) needsrefresh = Pinf { pi with needsrefresh }

let withdisplaystate (Pinf pi) displaystate = Pinf { pi with displaystate }

let withhist (Pinf pi) hist = Pinf { pi with hist }

let withfromstore (Pinf pi) fromstore = Pinf { pi with fromstore }
...
let docommand displaystate env target comm
    (Proofstate { cxt; tree; givens } as state) =
  let r =
    let state =
      withroot (withtarget (withgoal state (Some target)) (Some target)) None
    in
...
let pointToSequent displaystate env comm
    ...
    (Proofstate { tree } as state) =
  try
    let doit path =
      selections := Some (path, concopt, hyps, csels, hsels, givensel);
                ...
                badsel
                  [
                    "there is more than one unproved conclusion which \
                     corresponds to that hypothesis - you must select one of \
                     them as well as the hypothesis.";
                  ] )
  ...
  try
    let textsels =
      ...
      | Some (proofsels, givensels) ->
          ...
          Some
            (nj_fold
               (function
                 | (ConcHit (cp, c), ss), (cs, hs, gs) ->
                     ((cp, c, ss) :: cs, hs, gs)
                 | (HypHit (hp, h), ss), (cs, hs, gs) ->
                     (cs, (hp, h, ss) :: hs, gs)
                 | (AmbigHit (_, (hp, h)), ss), (cs, hs, gs) ->
                     (cs, (hp, h, ss) :: hs, gs))
               proofsels ([], [], givensels))
      | None -> None
    in
    let target =
      ...
      | true, _ :: _ ->
          ...
          badsel
            [
              "There is more than one unproved conclusion. Please select the \
    ...
    if !selectiondebug then
      consolereport
        [
          "inside nohitcommand with target ";
          string_of_path target;
          "; allTipConcs says ";
          bracketed_string_of_list
            ...
            ";" (allTipConcs tree);
          "; rootPath says ";
          string_of_path (rootPath tree);
        ];
    (selections :=
       match textsels with
       | Some (concsels, hypsels, givensels) ->
    ...
    if !selectiondebug then
      consolereport
        [
          "selections now ";
          string_of_option
            (string_of_sextuple string_of_path
               (string_of_option
                  (string_of_pair string_of_element
               ...
               (bracketed_string_of_list
                  ...
                  ";")
               (bracketed_string_of_list
                  (string_of_triple string_of_path string_of_element
                     (bracketed_string_of_list enQuote ";")
                     ",")
                  ";")
               (bracketed_string_of_list enQuote ";")
...
let rotateFormat = function
  | tfk, DefaultFormat -> Some (tfk, defaultfolded)
  | tfk, RotatingFormat (i, nfs) ->
      Some
        (tfk, RotatingFormat ((if i >= List.length nfs then 0 else i + 1), nfs))

let getfmt mess tree path =
  try
    let subproof = followPath tree path in
    if isTip subproof then (
      showAlert [ mess (); " -- it's not a proved conclusion." ];
      None )
    else match format subproof with TreeFormat tf -> Some tf
  with FollowPath_ _ -> None

let parent tree path =
  try Some (parentPath tree path) with FollowPath_ _ -> None

let leftCutParent tree path =
  match parent tree path with
  | Some parentpath ->
      if
        isCutStep tree parentpath
        &&
        (* going left should be impossible *)
        try
          let _ = (siblingPath tree path true : path) in
          false
        with _ -> true
      then Some parentpath
      else None
  | _ -> None
...
let doLayout command (Proofstate { tree } as state) path =
  match command with
  | BacktrackCommand -> raise (Catastrophe_ [ "doLayout BacktrackCommand!" ])
  | PruneCommand -> Some (true, prunestate path state)
  | HideShowCommand ->
      (getfmt (fun () -> "can't hide subproofs there") tree path &~~ function
       | tfk, RotatingFormat (i, nfs) ->
           ( findfirst
      ...
      &~~ fun fmt' ->
      Some
        (false, withtree state (set_prooftree_fmt tree path (TreeFormat fmt')))
  | ExpandContractCommand ->
      getfmt
        (fun () ->
          raise (Catastrophe_ [ "getLayout sees EXPAND/CONTRACT on a Tip!!!" ]))
        tree path
      &~~ (function
            | (_, DefaultFormat) as fmt ->
                if null (subtrees (followPath tree path)) then (
                  showAlert
                    [
                      "no point double-clicking that -- it doesn't have any \
                       subproofs";
                    ];
                  None )
                else Some fmt
  ...
  | HideRootCommand -> (
      let hideit () =
        try
          ...
          Some
            ( false,
              withtree state
                (set_prooftree_fmt tree path (TreeFormat (HideRootFormat, tff)))
            )
        with FollowPath_ stuff ->
          showAlert [ "FollowPath_ in HIDEROOT?? " ];
          None
      in
      ...
      | Some _ -> (
          ...
          | Some [ _ ] -> hideit ()
          | _ ->
              showAlert [ "don't hide that conclusion: it's a cut hypothesis" ];
              None )
      | None -> hideit () )
  | ExposeParentCommand -> (
      match parent tree path with
      ...
      | Some parentpath -> (
          ...
          | _ ->
              showAlert [ "the parent isn't hidden!" ];
              None ) )
  | HideCutCommand -> (
      match leftCutParent tree path with
      | None ->
          showAlert [ "can't hide that line -- it's not a cut hypothesis" ];
          None
      | Some cutpath ->
...
let getLayoutPath displaystate c pathkind =
  findLayoutSelection displaystate pathkind |~~ fun _ ->
  showAlert
    [
...
let tryLayout displaystate c pathkind hist =
  getLayoutPath displaystate c pathkind &~~ doLayout c (winhist_proofnow hist)
  &~~ function
  | false, proof' ->
      let phist = winhist_proofhist hist in
...
let recorddisplayvars env =
  try (fun s -> string_of_term (_The (Japeenv.( <@> ) env s))) <* displayvars
  with _The_ ->
    raise
      (Catastrophe_
         [
...
let proofmove (WinHist { proofhist } as hist) proof' =
...
let disproofmove a1 a2 =
  ...
  | (WinHist { disproofhist = None } as hist), disproof' ->
      withdisproofhist hist
        (Some (Hist { now = disproof'; pasts = []; futures = [] }))

(* disproofevolve does set changed *)
let disproofevolve a1 a2 = withchanged (disproofmove a1 a2) true
 ...
 *)
let tryForward f (WinHist { proofhist = Hist { now = proof } } as hist) =
  f proof &~~ (_Some <.> proofmove hist)

let evolve_proof_explain explain displaystate env f
    (WinHist { proofhist = Hist { now = proof } } as hist) =
  match f proof with
  | None ->
      explain ();
      None
           ...
           (proofmove hist
              (rewriteproofstate
                 (autoTactics (Some displaystate) env
                    (if !applyautotactics then Proofstate.autorules () else [])
                    proof')))
           true)
...
let reset () =
  resetallcachesandvariables ();
  savefilename := None;
  mbcache := empty

let cleanup () = ()

let parseargs args = try parseTermCOMMAList args with ParseError_ _ -> []
  ...
  | (env, proofs, mbs), (path :: args, _) -> (
      try
        let rec doargs = function
          ...
          | "-" :: args -> ([], args)
          | "-tree" :: args ->
              Interaction.setdisplaystyle "tree";
              doargs args
          | "-box" :: args ->
              Interaction.setdisplaystyle "box";
              doargs args
          | "-proofs" :: name :: args ->
              savefilename := Some name;
              doargs args
          | "" :: args -> doargs args
          | name :: args ->
        ...
        (let env, proofs, mbs =
           ...
           with
           | ParseError_ m ->
               showInputError showAlert m;
               (env, proofs, mbs)
           | Use_ -> (env, proofs, mbs)
           (* already reported, we hope *)
         in
...
and rundialogue env mbs proofs =
  try
    ...
    let _Int _ =
      consolereport [ "[Interrupted]" ];
      interruptTactic ()
      (* but don't crash *)
    in
    Moresys.onInterrupt _Int dialogue;
    terminateGUI ()
  with
  ...
  | exn ->
      reportGUIdead
        [
          "Unexpected exception ["; Printexc.to_string exn; "] - Jape quitting";
        ]
...
and addproofs fromstore env
    (proofs : (name * proofstate * (seq * model) option) list)
    (pinfs : proofinfo list) =
  let f
      ( (name, (Proofstate { goal; cxt; tree; givens } as state), disproofopt),
        pinfs ) =
    let n = List.length ((fun (Pinf { title = t, _ }) -> t = name) <| pinfs) in
    if
      n <> 0
      && not
           (screenquery
              [
                ( if n = 1 then
                  ...
                  "There are already " ^ string_of_int n
                  ^ (if fromstore then " new " else "")
                  ^ " proofs of " );
                string_of_name name;
                " in progress - do you want to ";
                ( if fromstore then "show the stored proof too?"
                else "add another?" );
              ]
      ...
      Pinf
        {
          title = (name, index);
          proofnum = num;
          displayvarsvals = recorddisplayvars env;
          needsrefresh = false;
          displaystate =
          ...
          hist =
            WinHist
              {
                changed = false;
                proofhist = new_hist (withcxt state state_cxt);
...
and biggestproofnum name pinfs =
  nj_fold
    (fun (Pinf { proofnum; title = t, index }, (n, i)) ->
      (max proofnum n, if t = name then max index i else i))
    pinfs (0, 0)

and endproof num name st dis =
  let proved = isproven st in
  let disproved = disproof_finished dis in
  Runproof.addproof showAlert uncurried_screenquery name proved st disproved
    (model_of_disproofstate dis)
  &&
...
and commands
    ((env, mbs, (showit : showstate), (pinfs : proofinfo list)) as thisstate) =
  let findproof pinfs nstring =
    let n = atoi nstring in
    try extract (fun (Pinf { proofnum }) -> n = proofnum) pinfs
    with Extract_ ->
      raise
        (Catastrophe_
           [
             "Dialogue.findproof can't find proof ";
             nstring;
             " in pinfs ";
  ...
  let newfocus ((env, mbs, showit, (pinfs : proofinfo list)) as state) =
    ...
    | Pinf fg :: bgs ->
        ...
        (env, mbs, (if fg.needsrefresh then ShowBoth else DontShow), pinfs)
    | [] -> state
  in
  ...
  let defineconjecture panel text =
    let ispanel, novel, paratext =
      ...
      | p ->
          let exists =
            List.exists
              (fun (panelname, _) -> panelname = panel)
              (Menu.getpanels ())
          in
    ...
    try
      ...
      if ispanel then (
        ...
        if novel then
          ...
          Japeserver.panelentry (string_of_name panel) (string_of_name name)
    ...
    try
      let text =
        if (try String.sub text 0 8 with _ -> "") = "THEOREM " then (
          try
            let _ = getpara text in
            text
          with ParseError_ rs ->
            showAlert
              ( [ "Cannot parse new conjecture "; text; " -- "; UTF.utf8LSQUOTE ]
              @ rs @ [ UTF.utf8RSQUOTE; "." ] );
            raise AddConjecture_ )
        else
          (* praps it's just a sequent *)
          try
            let t = "THEOREM IS " ^ text in
            let _ = getpara t in
            t
          with ParseError_ rs -> (
            try
              (* praps it has params and stuff *)
              let t = "THEOREM " ^ text in
              let _ = getpara t in
              t
            with ParseError_ rs' ->
              showAlert
                ...
                if rs = rs' then rs
                  ...
                  "\n\nTrying to read it as a sequent gave the error "
                  :: UTF.utf8LSQUOTE
                  :: ( rs
                     @ UTF.utf8RSQUOTE
                       :: ".\n\n\
                           Trying to read it as a line of Japeish gave the \
                           error " :: UTF.utf8LSQUOTE
                       :: (rs' @ [ UTF.utf8RSQUOTE; "." ]) ) );
              raise AddConjecture_ )
      in
  ...
  let printproof path state =
    ...
    try
      ...
      Interaction.printState s st true;
      close_out s
    with Io exn ->
      showAlert
        [ "Cannot write file "; path; " ("; Printexc.to_string exn; ")" ]
  in

  let writetonamedfile action filename =
    try
      let sfile =
        try Usefile.open_output_file (disQuote filename)
        with exn -> raise (Io exn)
      in
  ...
  let saveproofs newfile =
    let doit sfile =
      let f (Pinf { title = t, _; hist }) =
        let (Proofstate { cxt; tree; givens }) = winhist_proofnow hist in
        saveproof sfile t Proofstage.InProgress tree
          (useful_provisos (provisos cxt))
          givens
          (model_of_disproofstate (winhist_disproofnow hist))
      in
      Proofstore.saveproofs sfile;
      List.map f pinfs
    in
    ...
    | _ -> (
        match
          Japeserver.writeFileName "Save proofs as:" Japeserver.prooffiletype
        with
  ...
  let needssaving () =
    List.exists
      (function
        | Pinf { hist = WinHist { changed = true } } -> true | _ -> false)
      pinfs
  ...
  let disproofundoable () =
    match pinfs with
    | Pinf { hist = WinHist { disproofhist = Some (Hist { pasts = _ :: _ }) } }
      :: _ ->
        true
    | _ -> false
  in
  ...
  let disproofredoable () =
    match pinfs with
    | Pinf
        { hist = WinHist { disproofhist = Some (Hist { futures = _ :: _ }) } }
      :: _ ->
        true
    | _ -> false
  in

  let finished proof disproof = isproven proof || disproof_finished disproof in
  ...
  let resetable () =
    (* eggstolay, wormstoscratch, ... *) thingstodo () || saveable ()
  in
  ...
  let askResettheory anyway =
    if resetable () then
      if needssaving () then
        ...
        anyway || screenquery [ "Erase the current theory?" ] "Erase" "Cancel" 1
    else (
      reset ();
      true )
  in
  ...
  let inside c f =
    ...
    | [] ->
        ...
        default
    | (Pinf { displaystate; hist } as pinf) :: pinfs ->
        let hist, showit =
  ...
  *)
  let showproof = function
    | Some hist -> Some (ShowProof, hist)
    | None -> None
  in
  let showdisproof = function
  ...
  let rec processcommand
    ...
    *)
      ...
      let doit givens seq provisos kind =
        try
          ...
          let rec uncache_mb ((var, _) as pair) =
            mbcache := !mbcache -- [ var ];
            match !mbcache <@> var with Some _ -> uncache_mb pair | None -> ()
          in
          ...
          ( env,
            ...
            DontShow,
            addproofs false env
              [ (name, withgivens (withcxt state cxt') givens, None) ]
              pinfs )
      ...
      | Some (Rule ((_, provisos, givens, seq), ax)) ->
          if ax then (
            showAlert
              [
                string_of_name name;
                " is a rule, not a conjecture or a derived rule";
              ];
            default )
          else doit givens seq (mkvisproviso <* provisos) "derived rule"
      | Some (Tactic _) ->
          showAlert
            [
              string_of_name name;
    ...
    let disproofact act move =
      inside c (fun displaystate ->
          showdisproof <.> function
    ...
    let disproofacteval act =
      disproofact (fun facts_now cxt_now proof d ->
          act facts_now d
          &~~ (_Some <.> evaldisproofstate facts_now (proofstate_tree proof)))
    in

    let disproofuniverseact act =
      disproofacteval (fun facts_now d ->
          act facts_now (disproofstate_universe d) &~~ fun u ->
          Some (withdisproofuniverse d u))
    in

    let worldlabelact act move cx cy s =
      disproofuniverseact
        (fun facts_now u ->
          act facts_now u (atoi cx, atoi cy) (parseTerm (disQuote s)))
        move
    in
    ...
    let interpretConjecture action comm =
      ...
      | Some name -> (name, ts)
      | _ ->
          raise (Catastrophe_ [ "name in "; action; " is "; string_of_term f ])
    in
    ...
    | word :: words -> (
        ...
        | "applygiven", args ->
            doApply "applygiven"
              (parseablestring_of_name (name_of_string !givenMenuTactic) :: args)
        | "assign", name :: value -> (
            try
            ...
            | Japeenv.OutOfRange_ s ->
                showAlert
                  [
                    "error in "; respace c; " --  value assigned should be "; s;
                  ];
                default
            | Japeenv.NotJapeVar_ ->
                showAlert
        ...
        | "refreshdisplay", [] ->
            ...
            | Pinf
                {
                  hist =
                    WinHist { proofhist = Hist { now = Proofstate { givens } } };
                }
              :: _ ->
                displayGivens givens
            | _ -> () );
        ...
        | "use", (file :: _ as files) -> (
            ...
            try
              ...
              Japeserver.emptymenusandpanels
                ( !autoAdditiveLeft
                && List.length (getsyntacticturnstiles ()) = 1 );
              reloadmenusandpanels Proofstore.provedordisproved (get_oplist ());
              mbcache := empty;
            ...
            | Use_ -> default )
        | "version", [] ->
            showAlert [ "Jape proof engine "; Version.version ];
            default
        ...
        | "moveworldlabel", [ wx; wy; cx; cy; s ] ->
            worldlabelact
              (fun _ u -> moveworldlabel u (atoi wx, atoi wy))
              disproofevolve cx cy s
        | "deleteworldlabel", [ cx; cy; s ] ->
            worldlabelact (fun _ -> deleteworldlabel) disproofevolve cx cy s
        | "tileact", [ s ] ->
            disproofacteval
              (fun _ d -> Disproof.newtile d (parseTerm (disQuote s)))
              disproofmove
        | "addworld", [ px; py; cx; cy ] ->
            disproofuniverseact
              (fun _ u ->
                Disproof.addchild u (atoi px, atoi py) (atoi cx, atoi cy))
              disproofevolve
        | "addworldtolink", [ px; py; cx; cy; lpx; lpy; lcx; lcy ] ->
            disproofuniverseact
              (fun _ u ->
                Disproof.addchildtolink u
                  (atoi px, atoi py)
                  (atoi cx, atoi cy)
                  (atoi lpx, atoi lpy)
                  (atoi lcx, atoi lcy))
        ...
        | "deleteworldlink", [ fromx; fromy; tox; toy ] ->
            disproofuniverseact
              (fun _ u ->
                Disproof.deletelink u
                  (atoi fromx, atoi fromy)
                  (atoi tox, atoi toy))
              disproofevolve
        | "deleteworld", [ cx; cy ] ->
            disproofacteval
              (fun _ d -> Disproof.deleteworld d (atoi cx, atoi cy))
              disproofevolve
        | "moveworld", [ x; y; x'; y' ] ->
            disproofacteval
              (fun _ d ->
                ...
                let rec pair = function
                  ...
                  | _ ->
                      raise
                        (Catastrophe_
                           [
                             "bad command (odd number of arguments): \
                              worldselect ";
        ...
        | "disproof_selection_change", [] ->
            disproofacteval
              (fun _ d ->
                ...
                if disproofstate_selections d = selections then None
                else Some (withdisproofselections d selections))
              disproofmove
            (* i.e. selections can be undone .... hmmm. *)
        | "disprove", [] ->
            inside c (fun displaystate ->
                showdisproof <.> fun hist ->
                let proof = winhist_proofnow hist in
                ...
                let process_disproof disproof' =
                  ...
                  | Some state ->
                      ...
                      if eqseqs (seq, seq') then
                        ...
                        then (
                          showAlert
                            [ "You are already disproving "; string_of_seq seq ];
                          None )
                        else if
                          screenquery
                            [
                              ...
                              string_of_seq seq;
                              " - do you want to wipe clean the world(s) you \
                               have built?";
                            ]
                            "Wipe" "Cancel" 1
                        then doit ()
                ...
                | Some (FormulaSel (path, _, hyps, _, _, _)) ->
                    process_disproof
                      (disproof_start facts_now (proofstate_tree proof)
                         (Some path) hyps)
                | _ ->
                    process_disproof
                      (disproof_start facts_now (proofstate_tree proof) None []))
        (* ******************* proof stuff ************************)
        | "makelemma", [] -> (
            match pinfs with
            | [] -> raise (Catastrophe_ [ "makelemma with no proof" ])
            | Pinf { displaystate; hist } :: _ -> (
                match findSelection displaystate with
                | Some (FormulaSel (_, Some (conc, _), hyps, [], [], [])) -> (
                    let stile =
                      match Sequent.getsyntacticturnstiles () with
                      | [ s ] -> s
                      | ss ->
                          raise
                            (Catastrophe_
                               [
                                 "makelemma with choice of turnstile ";
                    ...
                    let druleString, thmString =
                      if not !autoAdditiveLeft then
                        raise
                          (Catastrophe_ [ "makelemma without autoAdditiveLeft" ])
                      else
                        ( ( if hyps = [] then " "
                            (* non-empty argument essential for transmission to GUI *)
                          else
                    ...
                    in
                    let panels =
                      List.map string_of_name (Menu.getconjecturepanels ())
                    in
                    let provisos =
                      let (Proofstate { cxt }) = winhist_proofnow hist in
                      let vpros = useful_provisos (provisos cxt) in
                      string_of_proviso
                      <* Proviso.expandProvisos
                           (Provisofuns.relevantprovisos
                              (Sequent.mkSeq (stile, hyps, [ conc ]))
                              vpros)
                    in
                    match
                      Japeserver.askLemma druleString thmString panels provisos
                    with
                    | None -> default
                    | Some (isThm, lemma, panel, provisos) ->
                        ...
                        in
                        let nametext =
                          match lemma with None -> "" | Some n -> enQuote n
                        in
                        let text =
                          if isThm then
                            "THEOREM " ^ nametext ^ " " ^ provisotext ^ " IS "
                            ^ thmString
                          else
                            "DERIVED RULE " ^ nametext ^ " " ^ provisotext
                            ^ " IS " ^ druleString
                        in
                        (* consolereport ["text is "; text]; *)
                        defineconjecture (name_of_string panel) text;
                        default )
                | _ ->
                    let tsels = get_textselections displaystate in
                    showAlert
                      ( "Can't make a lemma unless you select a conclusion"
                      ::
                      ( if tsels != [] then
                        [
        ...
        | "unify", stuff -> (
            try
              inside c (fun displaystate ->
                  ...
                  let args =
                    try
                      ( if null sels || null stuff then parseCurriedArgList
                      else (fun t -> [ t ]) <.> parseTerm )
                        (respace stuff)
                    with ParseError_ es ->
                      showAlert
                        ("cannot parse unify " :: respace stuff :: " -- " :: es);
                      raise Unify_
                  in
                  let getit s =
                    try parseTerm s
                    with ParseError_ es ->
                      showAlert
                        ("your selection " :: s :: " didn't parse -- " :: es);
                      raise Unify_
                  in
        ...
        | "done", [] -> (
            match pinfs with
            | [] ->
                showAlert [ "Not in a proof" ];
                default
            | Pinf { title = t, _; proofnum; hist } :: pinfs' ->
                let proof = winhist_proofnow hist in
                let disproof = winhist_disproofnow hist in
                if finished proof disproof then
                  if endproof proofnum t proof disproof then
                    newfocus (env, mbs, DontShow, pinfs')
                  else default
                else (
                  showAlert [ "Not finished yet!" ];
                  default ) )
        | "showproof", stuff -> (
            ...
            | Some (_, tree, provisos, givens, _, disproofopt) ->
                ...
                newfocus
                  ( env,
                    mbs,
                    DontShow,
                    addproofs true env [ (name, proofstate, disproofopt) ] pinfs
                  ) )
        | "print", [ path ] ->
            inside c (fun _ hist ->
                printproof path (winhist_proofnow hist);
                None)
        | "quitnow", [] ->
            (*
          |  ("cd", [path]) =>
        ...
        | "backtrack", _ ->
            inside c (fun displaystate ->
                ...
                try
                  ...
                  let stillcomposite
                      (WinHist
                        { proofhist = Hist { now = Proofstate { tree } } }) =
                    try
                      let _ = (findTip tree path : seq) in
                      false
                    with
                    | FollowPath_ _ -> false
                    | FindTip_ -> true
                  in
                  let undo_proofstep (WinHist { proofhist } as hist) =
        ...
        | "layout", _ ->
            ...
            inside c (fun displaystate ->
                showproof
                <.> tryLayout displaystate ExpandContractCommand LayoutPath)
        ...
        | "proveconjecture", comm ->
            ...
            then
              ...
              if ts = [] then newstate
                ...
                processcommand newstate
                  ( "apply"
                  :: List.map (string_of_tactic <.> Tactic.transTactic) ts )
            else default
        | "reset", [] ->
        ...
        | "profile", [ "report" ] ->
            ( match
              ...
              with
            | Some s ->
                let _ = (writetonamedfile profileReport s : bool) in
                ()
            | None -> () );
            default
        | "profile", [ "report"; filename ] ->
            let _ = (writetonamedfile profileReport filename : bool) in
            default
        | "fonts_reset", [] ->
              ...
              | fg :: bgs ->
                  ...
                  ( if Japeserver.canbackgroundfocus then (
                    let f pinf =
                      ...
                      Japeserver.setbackgroundfocus (proofinfo_proofnum pinf);
                      withhist
                        (withdisplaystate
                           (withneedsrefresh pinf false)
                           (let r =
                              showState
                                (proofinfo_displaystate pinf)
                                proof !autoselect
                            in
                            displayGivens (proofstate_givens proof);
                            r))
                        (reparentprovisos hist)
                    in
                    let r = f <* bgs in
                    Japeserver.setforegroundfocus ();
                    r )
                  else (fun pinf -> withneedsrefresh pinf true) <* bgs )
              | _ -> pinfs
            in
            (env, mbs, ShowBoth, pinfs)
        | "showfile", [ filename ] ->
            (* here cos I can't work out how to get round the NOSELCOMMAND trap. RB 14/ii/94 *)
            Japeserver.showfile (disQuote filename);
            default
        | "saveproofs", [ w ] ->
            let newfile = w = "true" in
            let pinfs' =
              (fun pinf ->
                withhist pinf (withchanged (proofinfo_hist pinf) false))
              <* pinfs
            in
            if
              (newfile && (* Save As... *) saveable ())
              || ((not newfile) && (* Save *) needssaving ())
            then saveproofs newfile
        ...
        | "setfocus", [ nstring ] ->
            if not (null pinfs) then
              ...
              showAlert
                [
                  "GUI protocol error: setfocus ";
                  nstring;
                  " with no proof windows";
                ];
              default )
        | "windowwidened", [ n ] ->
            ...
            *)
            ...
            if
              Interaction.getdisplaystyle () = "box"
            ...
            then processcommand thisstate [ "refreshdisplay" ]
            else default
        | "setfonts", fontnames ->
            (* let _ = consolereport ["font names now "; bracketed_string_of_list idf "; " fontnames] in *)
            Japeserver.setFontNames (List.map Stringfuns.disQuote fontnames);
            default
        | "closeproof", [ nstring ] ->
            (* when closing proofs we must consider proof hist AND disproof hist *)
            let n = atoi nstring in
            let Pinf { hist; title = t, _; fromstore }, pinfs' =
            ...
            *)
            if finished proofstate disproof then
              if
                ...
                | Some d ->
                    (not (null (hist_pasts d))) && disproof_finished disproof
                | None -> false
              then
                ...
                in
                Alert.askDangerously
                  ( "The " ^ saywhat ^ " of " ^ string_of_name t ^ " "
                  ^ sayisare ^ " complete - do you want to record " ^ sayitthem
                  ^ "?" )
                  ( "Record",
                    fun () ->
                      if endproof n t proofstate disproof then closed ()
                      else default )
            ...
            then
              ...
              if
                screenquery
                  [ "Abandon proof of "; string_of_name t; "?" ]
                  "Abandon" "Cancel" 1
              then closeOK ()
              else default
        ...
        | "closedbugfile", [] ->
            ...
            default
        | _ ->
            cannotprocesscommand c;
            default )
  (*processcommand*)
  and cannotprocesscommand command =
    showAlert [ "Dialogue.processcommand cannot process "; respace command ]
  in
  let domb (var, notify) =
    let setting =
      try _The (Japeenv.( <@> ) env var)
      with None_ ->
        raise
          (Catastrophe_
             [
    ...
    | HitCommand (tree, hit, sel) -> (
        ...
        | FormulaHit fh, FormulaSel ((path, concopt, hyps, _, _, _) as fsel)
          -> (
             ...
             *)
            let (Seq (st, hs, gs)) =
              try sequent (followPath tree path)
              with _ ->
                raise
                  (Catastrophe_
                     [
            ...
            let hypword hs ss =
              ...
              | _ ->
                  "hypotheses "
                  :: sentencestring_of_list string_of_element ", " " and " hs
                  :: ss
            in
            let bang f = "double-click is not defined on " :: f () in
            let comm sense ps ss =
              findfirst (trymatch sense) ps |~~ fun _ ->
              showAlert (bang ss);
              None
            in
            let sense, (seqs, errf) =
                            ...
                            ( [ (st, [], [ el ]); (st, [ h ], [ el ]) ],
                              fun _ ->
                                [
                                  ...
                                  " with no selected hypothesis, or with \
                                   hypothesis ";
                                  string_of_element h;
                                ] )
                        | _ ->
                        ...
                        | [ g ] ->
                            ( [ (st, hyps, []); (st, hyps, [ g ]) ],
                              fun _ ->
                                hypword hyps
                                  [
                                    " with no selected conclusion, or with \
                                     conclusion ";
                                    string_of_element g;
                                  ] )
                        | _ ->
                            ( [ (st, hyps, []) ],
                              fun _ ->
                                hypword hyps [ " with no selected conclusion" ]
                            ) )
                    | Some (c, _) ->
                        ( [ (st, hyps, [ c ]) ],
                          fun _ ->
            ...
            | None -> default )
        | _, ReasonSel _ ->
            inside [] (fun displaystate ->
                showproof
                <.> tryLayout displaystate ExpandContractCommand LayoutPath)
        | _ ->
            raise
              (Catastrophe_
                 [ "funny hit (commands): "; string_of_command command ]) )
  in
  (* assignment to displayvars can force redisplay *)
  let nextargs =
    try
      match pinfs with
      | (Pinf { displayvarsvals; displaystate; hist } as pinf) :: rest -> (
          let doShowProof cxt state show =
            ( if Interaction.getdisplaystyle () = "tree" then
              ...
              if List.length dNdmapping != 0 then
                let sortedconcat cmp =
                  Listfuns.remdups <.> Listfuns.sort cmp <.> List.concat
                in
                let sortedconcat_es = sortedconcat earlierresource in
                let get_es sel = sortedconcat_es (sel <* dNdmapping) in
                let elementmap es =
                  (fun e -> (e, Interaction.locateElement state e)) <* es
                in
                ...
                *)
                ...
                let sourcefilter (e, ps) =
                  let tes =
                    sortedconcat_es
                      ( snd
                      <* ( (fun (ss, _) -> List.exists (fun s -> e = s) ss)
                         <| dNdmapping ) )
                  in
                  let tpss =
                    (fun tps -> explodePos <* tps)
                    <.> _The
                  ...
                  let spslow =
                    Listfuns.takewhile (fun (x, y) -> y > targety) sps
                  in
                  let sps =
                    if null spslow then sps else [ List.hd (List.rev spslow) ]
                  in
            ...
            ( env,
              ...
              :: rest )
          in
          let (Proofstate { tree; cxt; goal } as proofstate) =
          ...
          if mustredisplay env displayvarsvals then
            let target =
              findSelection displaystate &~~ selpath |~~ fun () -> goal
            in
            doShowProof cxt
    ...
    | CompileThing_ ss ->
        ...
        (env, mbs, DontShow, pinfs)
    | Tacastrophe_ ss ->
        showAlert ("Tacastrophe_ in commands (shouldn't happen)!! -- " :: ss);
        (env, mbs, DontShow, pinfs)
    | Japeserver.DeadGUI_ as exn -> raise exn
    | QuitJape as exn -> raise exn
    | UTF.MalformedUTF_ ss ->
        showAlert
          [ "Malformed UTF ("; string_of_list idf "" ss; ") in commands" ];
        (env, mbs, DontShow, pinfs)
    | FindTip_ ->
...
and start () =
  (* Japeserver.quit ();
     ...
     cleanup (); *)
  initButtons ();
  main (defaultenv (), [], []) (Array.to_list Sys.argv, []);
  (* empty environment ... ?? *)
  ()
