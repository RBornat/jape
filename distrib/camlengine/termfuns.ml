...
let elementnumbers = function
  | Collection (_, _, es) ->
      let f = function Element (_, r, _) -> Some r | _ -> None in
      optionfilter f es
  | _ -> []
...
let elementnumbered a1 a2 =
  ...
  | Collection (_, _, es), r ->
      findfirst
        (function
          | Element (_, r', t) -> if r = r' then Some t else None | _ -> None)
        es
  | _, _ -> None

let isProperResnum = function Resnum _ -> true | _ -> false

let collectionkind = function Collection (_, c, _) -> Some c | _ -> None
...
let rec option_mapterm f t =
  ...
  | Id _ -> None
  | Unknown _ -> None
  | App (_, f, a) -> option_rewrite2 mtff mtff (f, a) &~~ (_Some <.> registerApp)
  | Tup (_, s, ts) -> mtfl ts &~~ fun ts' -> Some (registerTup (s, ts'))
  | Literal _ -> None
  | Fixapp (_, ss, ts) -> mtfl ts &~~ fun ts' -> Some (registerFixapp (ss, ts'))
  | Subst (_, r, p_, vts) ->
  ...
  | Binding (_, bs_ss_us, env, pat) ->
      option_rewrite3 mtfl mtfl mtfl bs_ss_us &~~ fun bs_ss_us' ->
      Some (registerBinding (bs_ss_us', env, pat))
  | Collection (_, k, es) ->
      option_mapelements f es &~~ fun es' -> Some (registerCollection (k, es'))
...
let rec foldterm f z t =
  ...
  | None -> (
      ...
      | Subst (_, _, p_, vts) -> ff (ff z (snd <* vts)) (p_ :: (fst <* vts))
...
let rec findterm g t =
  ...
  | None -> (
      ...
      | Subst (_, r, p_, vts) ->
          findterm g p_ |~~ fun _ -> findfirst (fun (v, t) -> fx [ v; t ]) vts
      | Collection (_, k, es) ->
          let fe = function
...
let findhole g t =
  let rec fh h t =
    let fhs sel build h xs =
      let rec fhx h = function
        | [] -> None
        | x :: xs ->
            fh (h <.> fun t -> build x t :: xs) (sel x) |~~ fun _ ->
            fhx (h <.> fun xs -> x :: xs) xs
      in
      fhx h xs
    in
    let selt t = t in
    ...
    | None -> (
        match t with
        | App (_, f, a) ->
            fh (h <.> fun f -> registerApp (f, a)) f |~~ fun _ ->
            fh (h <.> fun a -> registerApp (f, a)) a
        | Tup (_, s, ts) ->
            fhs selt buildt (h <.> fun ts -> registerTup (s, ts)) ts
        | Fixapp (_, ss, ts) ->
            fhs selt buildt (h <.> fun ts -> registerFixapp (ss, ts)) ts
        | Binding (_, (bs, ss, us), env, pat) ->
            fhs selt buildt
              (h <.> fun ss -> registerBinding ((bs, ss, us), env, pat))
              ss
            |~~ fun _ ->
            fhs selt buildt
              (h <.> fun us -> registerBinding ((bs, ss, us), env, pat))
              us
        | Subst (_, r, p_, vts) ->
            fh (h <.> fun p_ -> registerSubst (r, p_, vts)) p_ |~~ fun _ ->
            fhs
              (fun (v, t) -> t)
              (fun (v, _) t -> (v, t))
        ...
        | Collection (_, k, es) ->
            ...
            in
            fhs sele builde (h <.> fun es -> registerCollection (k, es)) es
        | _ -> None )
    | res -> res
  in
  fh idf t

let searchterm g z t = match findterm g t with Some v -> v | None -> z
...
let rec specialisesto (c1, c2) =
  ...
  | BagClass c1, BagClass c2 ->
      (* the lines above are essential at present, to allow (=), for example, to
...
let rec idclass t =
  ...
  | Subst (_, _, t, vts) -> (
      ...
      | VariableClass ->
          (* ohmygod *)
          if all (fun t -> idclass t = VariableClass) (List.map snd vts) then
            VariableClass
          else SubstClass
      | c -> (
          match debracket t with
              ...
              (Catastrophe_
                 [
                   ...
                   bracketed_string_of_list
                     ...
                     "," es;
                   " doesn't contain ";
                   debugstring_of_element string_of_term el;
                 ])
      in
  ...
  let stemNstern s =
    ...
    let rec f i =
      if i = 0 then ("", s)
        ...
        if isdigit d then f (i - utf8width_from_ucode d)
    ...
    if symclass str <> class__ then
      raise
        (Catastrophe_
           [
             "uniqueVID ";
             string_of_idclass class__;
             " ... ";
             str;
             " (which is ";
             string_of_idclass (symclass str);
             ")";
           ])
...
let earliervar t1 t2 =
  let ordinal = function Id _ -> 0 | Unknown _ -> 1 | _ -> 2 in
...
let isId t = match debracket t with Id _ -> true | _ -> false

let isUnknown t = match debracket t with Unknown _ -> true | _ -> false
...
let isidentifier t =
  match t with Id _ -> true | Unknown _ -> true | _ -> false

let issegvar e = match e with Segvar _ -> true | _ -> false

let isCollection = function Collection _ -> true | _ -> false

let isemptycollection = function Collection (_, _, []) -> true | _ -> false

let isselectionSubst e =
  match e with Element (_, _, Subst (_, false, _, _)) -> true | _ -> false

let emptycollection k = registerCollection (k, [])

let term_of_element = function Element (_, _, t) -> Some t | _ -> None

let term_of_collection = function
...
let rec eqterms (t1, t2) =
  let rec fEQs = function
    | t1 :: t1s, t2 :: t2s -> eqterms (t1, t2) && fEQs (t1s, t2s)
    | [], [] -> true
    | _ -> false
  in
  match (debracket t1, debracket t2) with
  | Id (_, s1, c1), Id (_, s2, c2) ->
      (* first the ones which alter the interpretation *)
      s1 = s2 && c1 = c2
  | Unknown (_, s1, c1), Unknown (_, s2, c2) -> s1 = s2 && c1 = c2
  | App (_, f1, a1), App (_, f2, a2) -> eqterms (f1, f2) && eqterms (a1, a2)
  ...
  | Collection (_, ListClass k1, es1), Collection (_, ListClass k2, es2) ->
  ...
  let rec eq t1bs bs_of_t (t1, t2) =
    ...
    | Binding (_, (bs, ss, us), _, pat), Binding (_, (bs', ss', us'), _, pat')
      -> (
        ...
        try
          (pat = pat' && fEQs (us, us'))
          && all (eq ((bs ||| ns) @ t1bs) ((bs' ||| ns) @ bs_of_t)) (ss ||| ss')
        with Zip_ -> false )
    | Collection (_, BagClass k1, es1), Collection (_, BagClass k2, es2) ->
    ...
    | _ -> false
  in
  let r = t1 = t2 || eq [] [] (t1, t2) in
  (* if !eqalphadebug then
 ...
 * RB 20/i/93
 *)
let termoccursin t p_ = existsterm (curry2 eqterms t) p_
      ...
      | Subst (_, _, p_, vts) ->
          Some
            (nj_fold (uncurry2 bmerge)
               (doit2 (fst <* vts) p_ :: dothem (snd <* vts))
               ps)
      | Binding (_, (bs, ss, us), _, _) ->
          Some
            (nj_fold (uncurry2 bmerge)
...
let freevarsfrombindings inf notins =
  ...
  let domf (v, bcs) =
    let truncate bc = takewhile (fun bvs -> not (member (v, bvs))) bc in
    let bcs = truncate <* bcs in
    let dominators =
      nj_fold (fun (bc, ds) -> nj_fold (uncurry2 tmerge) bc ds) bcs []
    in
   ...
   *)
  let allbindings = nj_fold (uncurry2 (sortedmerge bcorder)) (snd <* inf) [] in
  let allbpairs bc =
    let vpss =
    ...
    List.filter
      (fun (x, y) -> x <> y)
      (nj_fold (uncurry2 (sortedmerge vvorder)) vpss [])
  in
  let vps =
  ...
  let commonpairs =
    (fun (x, y) -> not (member ((x, y), notins) || member ((y, x), notins)))
    <| sortedsame vvorder vps rvps
  in
  let domrel =
    nj_fold (uncurry2 mmerge)
      ((fun (x, y) -> [ (x, [ y ]); (y, [ x ]) ]) <* commonpairs)
      domrel
  in
  ...
  let badbindings (v, bcs) =
    ...
    let escapers bvs = (fun bv -> not (member ((bv, v), notins))) <| bvs in
  ...
  if !varbindingsdebug then
    consolereport
      [
        "freevarsfrom bindings ";
        bracketed_string_of_list showin ", " inf;
        " ";
        bracketed_string_of_list
...
let conVIDs ts =
  nj_fold (function Id (_, v, c), vs -> v :: vs | _, vs -> vs) ts []
...
let simterms (t1, t2) =
  let rec similar t1subst t1 subst_of_t t2 =
    ...
    let sims t1s s_of_t =
      try all (uncurry2 sim) (t1s ||| s_of_t) with Zip_ -> false
    in
    ...
    if t1 = t2 then true
      ...
      | Subst (_, _, p1, _), Subst (_, _, p2, _) -> similar true p1 true p2
...
let explodebinapp t =
  ...
  | App (_, App (_, Id (_, v, OperatorClass), e), f) ->
      Some (e, string_of_vid v, f)
  | Tup (_, ",", _) -> None
...
let rec int_of_term t =
  ...
  with _ -> raise (Catastrophe_ [ "int_of_term "; debugstring_of_term t ])

let enbracket t = registerFixapp ([ "("; ")" ], [ t ])

let comma_enbracket t =
  match t with Tup (_, ",", _ :: _ :: _) -> enbracket t | _ -> t

let explodeCollection = function
  | Collection (_, _, es) -> es
  | t -> [ registerElement (Nonum, t) ]

let augmentCollection a1 a2 =
  match (a1, a2) with
  | Collection (_, kind, es), els -> Some (registerCollection (kind, es @ els))
  | _, _ -> None

let decodeSubst = function
  | Subst (_, r, p_, vts) -> Some (r, p_, vts)
  | _ -> None

let decodeBinding = function Binding (_, info, _, _) -> Some info | _ -> None

let decodeBracketed = function
  | Fixapp (_, [ "("; ")" ], [ t ]) -> Some t
  | _ -> None
