...
let rec ( -- ) xs ys =
  ...
  | [], ys -> []
  | (x, xv) :: ps, ys ->
      if member (x, ys) then ps -- listsub (fun (x, y) -> x = y) ys [ x ]
      else (x, xv) :: (ps -- ys)
...
let rec mem a1 a2 =
  match (a1, a2) with x, [] -> false | x, x' :: xs -> x = x' || mem x xs
...
let rec lfold f r m =
  let rec ff a1 a2 a3 =
    ...
    | dom, r, ((x, y) as pair) :: map ->
        if mem x dom then ff dom r map else ff (x :: dom) (f (pair, r)) map
  in
  ff [] r m

let rec remapping (f, mapping) =
  lfold (fun (pair, m) -> uncurry2 ( |-> ) (f pair) ++ m) empty mapping

(* dom now gives its result in reverse insertion order, just like rawdom *)

let aslist (m : ('a * 'b) list) = seteq (fun (a, b) (a1, b1) -> a = a1) m
