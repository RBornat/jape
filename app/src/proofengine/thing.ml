...
let string_of_provisolist =
  bracketed_string_of_list
    (string_of_pair string_of_bool string_of_proviso ",")
    " AND "

let rec string_of_antecedentlist heavy =
  bracketed_string_of_list
    (if heavy then debugstring_of_seq else string_of_seq)
    " AND "

let rec string_of_consequent heavy =
  if heavy then debugstring_of_seq else string_of_seq

let rec string_of_ruledata heavy r =
  string_of_quadruple string_of_paramlist string_of_provisolist
    (string_of_antecedentlist heavy)
    (string_of_consequent heavy)
    ", " r

let rec string_of_thmdata heavy t =
  string_of_triple string_of_paramlist string_of_provisolist
    (string_of_consequent heavy)
    ", " t

let rec string_of_thing = function
  | Rule r ->
      "Rule" ^ string_of_pair (string_of_ruledata false) string_of_bool "," r
  | Theorem t -> "Theorem" ^ string_of_thmdata false t
  | Tactic t ->
      "Tactic" ^ string_of_pair string_of_paramlist string_of_tactic ", " t
  | Macro m ->
      "Macro" ^ string_of_pair string_of_paramlist string_of_term ", " m

let rec string_of_cooked f = string_of_pair string_of_arglist f ","

let rec doublestring f = string_of_pair f f ","

let rec string_of_storedthing = function
  ...
  | CookedTheorem t ->
      "CookedTheorem"
      ^ doublestring (string_of_cooked (string_of_ruledata false)) t

let rec string_of_thingplace = function
...
let rec numberrule (antes, conseq) =
  let rec numberseq fld _R n leftenv rightenv (Seq (st, hs, gs) as seq) =
    ...
    | Collection (_, hkind, hes), Collection (_, gkind, ges) ->
        ...
        ( n,
          ...
          Seq
            ( st,
              registerCollection (hkind, hes),
              registerCollection (gkind, ges) ) )
    | _ ->
        raise
          (Catastrophe_
             [
               "in numberseq argument ";
               string_of_seq seq;
               " exploded into (";
               debugstring_of_term hs;
               ",";
  ...
  let rec rvfld f xs z =
    let n, old, new__, ys = nj_revfold f xs z in
    (n, old, new__, List.rev ys)
  in
  let n, antes' =
    nj_fold
      (fun (seq, (n, seqs)) ->
...
let rec numberforapplication n (antes, conseq) =
  let rec renumberseq n (Seq (st, hs, gs) as seq) =
    ...
    let rec renumberel (el, (m, rs, els)) =
      ...
      | Element (_, Resnum m', t) ->
          (max m (new__ m'), rs, registerElement (Resnum (new__ m'), t) :: els)
      | _ -> (m, rs, el :: els)
    in
    match (hs, gs) with
    | Collection (_, hkind, hes), Collection (_, gkind, ges) ->
  ...
  let m, antes' =
    nj_fold
      ...
      antes (m, [])
  in
  let res = (m + 1, rs, antes', conseq') in
  (* desperation ...
     if !thingdebug then
...
let rec numberforproof (antes, conseq) =
  ...
  let conseq' =
    ...
    | Seq (st, Collection (_, hkind, hes), Collection (_, gkind, ges)) ->
        Seq
          ( st,
            registerCollection (hkind, renumberel <* hes),
            registerCollection (gkind, renumberel <* ges) )
    | Seq (st, hs, gs) ->
        raise
          (Catastrophe_
             [
               "in numberforproof argument ";
        ...
        (foldterm
           (function
             ...
             | Binding (_, ((_ :: _ :: _ as bs), ss, us), _, _), qs ->
                 Some
                   (nj_fold (uncurry2 dmerge)
                      (([ sort earliervar bs ] :: (dbs <* ss)) @ (dbs <* us))
                      qs)
             | _ -> None)
           [] t)
    in
...
let rec freshc defcon cxt env params args =
  ...
  in
  let rec usearg con vc vs arg args = _F cxt (extend con vc arg) vs args in
  match (params, args) with
  | [], [] -> (cxt, env)
  | [], _ :: _ -> raise (Fresh_ [ "too many arguments provided" ])
  | Ordinaryparam vc :: vs, [] -> newname newVID registerId defcon vc vs
  | Ordinaryparam vc :: vs, arg :: args -> usearg registerId vc vs arg args
  | Objectparam (v, c) :: vs, [] ->
      let sv = string_of_vid v in
  ...
  | Unknownparam vc :: vs, arg :: args -> usearg registerUnknown vc vs arg args
  | Abstractionparam vc :: vs, [] -> newname newVID registerId defcon vc vs
  | Abstractionparam vc :: vs, arg :: args -> usearg registerId vc vs arg args
...
let rec allparams params allvs =
  ...
  @ listsub
      (function
        | Unknownparam v1, Unknownparam v2 -> fst v1 = fst v2
        | Unknownparam v1, _ -> false
        | _, Unknownparam v2 -> false
        | v1, v2 -> fst (paramidbits v1) = fst (paramidbits v2))
      allvs params

let rec extraBag_vid () =
  vid_of_string (autoID (BagClass FormulaClass) "extraBag")

let rec newvar con class__ (vid, vars) =
  let vid = uniqueVID class__ (vid_of_var <* vars) [] vid in
  let v = con (vid, class__) in
  (v, tmerge [ v ] vars)
...
let rec ehb vars c =
  if extensible c then
    ...
    in
    let sv = registerSegvar ([], v) in
    Some (vars, sv, extend sv c)
  else None
...
let rec augment el er (conseq, antes, vars) =
  ...
  in
  let rec lhs (Seq (_, lhs, _)) = lhs in
  let rec setlhs (Seq (st, _, rhs)) lhs = Seq (st, lhs, rhs) in
  let rec rhs (Seq (_, _, rhs)) = rhs in
  let rec setrhs (Seq (st, lhs, _)) rhs = Seq (st, lhs, rhs) in
...
let rec compileR el er (params, provisos, antes, conseq) =
  ...
  let abstractions =
    nj_fold
      (function Abstractionparam vc, vcs -> vc :: vcs | _, vcs -> vcs)
      params []
  in
  ...
  in
  let conseq_pbs =
    try discardzeroarities (findpredicatebindings isabstraction (conseq, []))
    with Predicate_ ss -> raise (CompileThing_ ss)
  in
  let all_pbs =
    try
      discardzeroarities
   ...
   *)
  let rec findbinders ((_P, abss), ps) =
    let rec g (ts, bss) =
  ...
  let proofps =
    sortunique
      (earlierpair earliervar earliervar)
  ...
  let objparams =
    Mappingfuns.lfold
      (function (_, (true, bs)), vs -> bs @ vs | _, vs -> vs)
      [] env
  in
  ...
  let ( proofbodyvars,
    ...
    ( bodyvars,
      antes,
      conseq,
      makeprovisos proofps,
      applybodyvars,
      compilepredicates isabstraction env <* antes,
      compilepredicates isabstraction env conseq,
      makeprovisos applyps,
      params
      @ nj_fold
          (function
            | Id (_, v, c), ps -> Objectparam (v, c) :: ps | _, ps -> ps)
          objparams [] )
  in
  let _ =
    if !thingdebug then
      consolereport
        [
          "applyantes are ";
          bracketed_string_of_list string_of_seq ", " applyantes;
          " and applyconseq is ";
          string_of_seq conseq;
        ]
  in
  ...
  in
  let rec mkvars vs = isextensibleID <.> string_of_vid <.> vid_of_var <| vs in
  let res =
...
let rec compilething name thing =
  ...
  | Rule (r, ax) ->
      let r1, r2 =
        ...
        | CompileThing_ ss ->
            raise (CompileThing_ ("Rule " :: string_of_name name :: ": " :: ss))
        | BadAdditivity_ ss ->
            raise
              (CompileThing_
  ...
  | Theorem (params, provisos, bot) -> (
      try
        ...
        *)
      with CompileThing_ ss ->
        raise (CompileThing_ ("Theorem " :: string_of_name name :: ": " :: ss))
      )
  | _ -> Rawthing thing
...
let rec addstructurerule ctn_ tn_ kind name =
  ...
  let rec seqmatch (Seq (_, phs, pcs)) (Seq (st, hs, cs)) =
    seqmatchvars false ispatvar (Seq (st, phs, pcs)) (Seq (st, hs, cs))
  in
  ...
  let rec matchrule (mparams, mprovs, mtops, mbottom) =
    ...
    | Some (CookedRule (_, (_, (params, provs, tops, bottom)), _) (* as r *))
      -> (
        ...
        ( if !thingdebug then
          let rec myseqstring (Seq (_, hs, gs)) =
          ...
          consolereport
            [
              ...
              " against ";
              string_of_quadruple
                (string_of_option string_of_termlist)
                (string_of_option
                   (bracketed_string_of_list string_of_proviso " AND "))
                (bracketed_string_of_list myseqstring " AND ")
                myseqstring ", "
        ...
        try
          ...
          | Some env -> (
              ...
              | Some mprovs ->
                  eqbags
                    (fun (x, y) -> x = y : proviso * proviso -> bool)
                    (remapproviso env <* mprovs, snd <* provs)
              | None -> true )
          | _ -> false
        with Zip_ -> false )
    | _ -> false
      ...
      *)
      ...
      | Some
          ( Rule
              ( ( _,
                  _,
                  [ Seq (a1st, a1l, a1r); Seq (st_of_a, l_of_a, r_of_a) ],
                  Seq (cst, cl, cr) ),
                _ ),
            _ ) -> (
          ...
          &&
          match
            ( term_of_collection a1r,
              term_of_collection r_of_a,
              term_of_collection cr )
          with
          | Some a1, Some a2, Some ct -> (
              match (explodebinapp a1, explodebinapp a2, explodebinapp ct) with
              | Some (a, _, b), Some (c, _, d), Some (e, _, f) ->
                  ((eqterms (a, e) && eqterms (d, f)) && eqterms (b, c))
                  &&
                  ( registerRelationpat ct;
                    true )
              | _ -> false )
          | _ -> false )
      | _ -> false )
...
let rec uniqueCut () =
  match (function CutRule, _ -> true | _ -> false) <| !structurerules with
  | [ (_, r) ] -> Some r
  | _ -> None
...
let ( clearthings,
  ...
  let lookup, update, reset, sources, targets =
    ...
    ( (fun name -> try Some (Hashtbl.find store name) with Not_found -> None),
      (fun name thing ->
        (try Hashtbl.remove store name with Not_found -> ());
  ...
  and compiledthinginfo name =
    ...
    if !thingdebug then
      consolereport
        [ " "; string_of_name name; " is "; string_of_option pst res ];
    res
  ...
  and getthing trim name =
    ...
    | Some (CookedTheorem ((_, (params, provisos, _, conseq)), _), place) ->
        Some (trim (Theorem (params, provisos, conseq)), place)
    | None -> None
  and goodthing = function
    | Theorem (params, provisos, seq) -> Theorem (params, fst <| provisos, seq)
    | Rule ((params, provisos, antes, conseq), ax) ->
        Rule ((params, fst <| provisos, antes, conseq), ax)
    | Tactic _ as t -> t
    | Macro _ as m -> m
  and badthing t = t in

  let thingnamed = getthing goodthing and thinginfo = getthing badthing in
  ...
  let rec addthing (name, thing, place) =
    match
      findfirst
        (fun ((k, n) as pair) -> if n = name then Some pair else None)
        !structurerules
    with
    | Some (kind, _) ->
        erasestructurerule name;
        ...
        if not (addstructurerule compiledthingnamed thingnamed kind name) then
          Alert.showAlert Alert.Warning
            ( "STRUCTURERULE "
            ^ string_of_structurerule kind
            ^ " " ^ string_of_name name ^ " cancelled by definition "
    ...
    | None ->
        ...
        let newplace =
          match lookup name with
          | Some (_, oldplace) -> (
              match place with InLimbo -> oldplace | _ -> place )
          | None -> place
        in
        ...
        if !thingdebug then
          consolereport
            [
              "addthing ";
              string_of_triple string_of_name string_of_thing
                string_of_thingplace "," (name, thing, place);
              " was ";
              string_of_option pst (lookup name);
              "; is now ";
  ...
  if !thingdebug then
    consolereport
      [
        ...
        " ";
        string_of_termlist args;
        " ";
        "..cxt.. ";
        "..defcon.. ";
        "(..paramsused.., ..ruleVIDs..)";
        " => ";
        string_of_pair string_of_cxt
...
let rec freshRule env args cxt (params, provisos, antes, conseq) defcon
    (paramsused, ruleVIDs) =
  let cxt', env =
    freshc defcon
      (plususedVIDs cxt ruleVIDs)
    ...
    consolereport
      [
        ...
        string_of_triple string_of_maplist string_of_cxt
          (string_of_ruledata !thingdebugheavy)
          ", " res;
      ];
  res
  ...
  ( if !thingdebug then
    ...
    consolereport
      [
        ...
        af args;
        " ";
        string_of_termlist vars;
        " ";
        string_of_ruledata !thingdebugheavy rd;
        " ";
        " => ";
        string_of_quadruple nostring_of_cxt
          (string_of_mapping string_of_term string_of_term)
          (string_of_pair rnl rnl ",")
          (string_of_ruledata !thingdebugheavy)
          ", " res;
      ] );
  res
...
let rec freshRuletoapply cxt args vars ((params, provisos, antes, conseq) as rd)
  ...
  let cxt'', env =
    env4Rule empty args cxt' registerUnknown (freshparamstouse vars args params)
  in
  freshRuleshow "freshRuletoapply" string_of_termlist cxt args vars rd
    (cxt'', env, interesting_resources, (params, provisos, antes', conseq'))

let rec freshRuletosubst cxt argmap vars
    ((params, provisos, antes, conseq) as rd) =
  ...
  let rec bad w n vs =
    raise
      (Fresh_
         [
           string_of_list string_of_term " and " vs;
           " ";
           w;
           " not schematic ";
           n;
  ...
  let cxt'', env =
    env4Rule
      (mkmap (List.rev (argvars ||| args)))
      [] cxt' registerUnknown
      ( listsub
          (fun (p, v) -> fst (paramidbits p) = vid_of_var v)
          paramsused argvars,
        ruleVIDs )
  in
...
let rec rearrangetoResolve antes (Seq (st, lhs, rhs) as conseq) =
  let rec extractsegv = function
    | Collection (_, cc, els) ->
        let segvs, els' = split issegvar els in
        (cc, segvs, els')
    | t ->
        raise
          (Catastrophe_
  ...
  let r = ((newante <.> renum <* lels) @ antes, Seq (st, newlhs, rhs)) in
  let showrule =
    string_of_pair
      (bracketed_string_of_list string_of_seq " AND ")
      string_of_seq " INFER "
  ...
  if !thingdebug then
    consolereport
      [
        "rearrangetoResolve given ";
        showrule (antes, conseq);
        "; delivers ";
        showrule r;
      ];
  r
...
let rec wehavestructurerule kind stilesopt proved =
  ...
  let rec getstile (Seq (st, _, _)) = st in
  (not (null names))
  &&
      ...
      List.exists
        (fun name ->
          ...
          | Some (CookedRule (_, (_, (params, provs, tops, bottom)), ax)) ->
              ( ax || proved name || !applyconjectures = "all"
              || !applyconjectures = "rules" )
              && cst = getstile bottom
...
let rec ftaors fR fThm weakenthms name cxt stuff proved =
  ...
  | Some (CookedTheorem (_, (vars, ((_, _, _, Seq (st, _, _)) as toapply)))) ->
...
let rec compiletoprove (params, pros, antes, conseq) =
  let (_, (_, pros', antes', conseq')), _ =
    compileR false false (params, (fun p -> (true, p)) <* pros, antes, conseq)
  in
  let antes', conseq' = numberforproof (antes', conseq') in
  (pros', antes', conseq')
...
let rec freshGiven weaken (Seq (st, lhs, rhs) (* as seq *)) cxt proved =
  ...
  let rec extend doit side cxt =
    ...
    | true, Collection (_, BagClass FormulaClass, es) ->
        let cxt, vid = freshVID cxt (BagClass FormulaClass) (extraBag_vid ()) in
        let sv =
          registerSegvar ([], registerUnknown (vid, BagClass FormulaClass))
        in
  ...
  let cxt, lhs =
    extend
      (weaken && wehavestructurerule LeftWeakenRule (Some [ st; st ]) proved)
      lhs cxt
  in
  let cxt, rhs =
    extend
      (weaken && wehavestructurerule RightWeakenRule (Some [ st; st ]) proved)
      rhs cxt
  in
  let rec unknownres = function
    | Collection (_, cc, els) ->
        registerCollection
          ( cc,
            (function
              | Element (_, Resnum r, t) -> registerElement (ResUnknown r, t)
              | el -> el)
            <* els )
    | t -> t
  in
