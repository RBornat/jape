...
let rec optf f = function Some x -> Some (f x) | None -> None

let rec somef f x = match f x with None -> Some x | v -> v

let rec anyway f x = match f x with Some v -> v | None -> x

let rec failpt f x = match f x with Some y -> failpt f y | None -> x

let ( &~~ ) v g = match v with None -> None | Some v' -> g v'

let rec ( |~~ ) v g = match v with None -> g () | v -> v

let rec ( &~ ) f g x = f x &~~ g

let rec ( |~ ) f g x = match f x with None -> g x | v -> v

let rec optioncompose (f, g) x =
  match g x with Some y -> Some (f y) | None -> None

let rec optionmap f = function
  | [] -> Some []
  | x :: xs ->
      f x &~~ fun x ->
      optionmap f xs &~~ fun xs -> Some (x :: xs)
...
let rec findfirst f = function
  | [] -> None
  | x :: xs -> ( match f x with None -> findfirst f xs | t -> t )
...
let rec stripoption = function None -> None | Some x -> x

let rec optordefault = function Some v, _ -> v | None, v -> v
...
let rec option_rewritelist f xs =
  ...
  | [] -> None
  | x :: xs -> (
      match (f x, option_rewritelist f xs) with
