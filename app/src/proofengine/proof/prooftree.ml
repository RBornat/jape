...
module type Tree = sig
  ...
  val augmenthyps :
    ...
    treeformat prooftree ->
    element list ->
    treeformat prooftree * (bool * bool * term) list
...
module Tree :
  Tree
    with type term = Termtype.term
     ...
     and type rewinf = Rewinf.rewinf = struct
  open Cxtfuns
  open Cxtstring
  open Cxtexterior
  open Idclass
  ...
  *)
  ...
  let join_why (why, how, cutnav, args, fmt, hastipval, seq, trs, ress) = why

  let join_how (why, how, cutnav, args, fmt, hastipval, seq, trs, ress) = how

  let join_cutnav (why, how, cutnav, args, fmt, hastipval, seq, trs, ress) =
    cutnav
  ...
  let join_fmt (why, how, cutnav, args, fmt, hastipval, seq, trs, ress) = fmt

  let join_hastip (why, how, cutnav, args, fmt, hastipval, seq, trs, ress) =
    hastipval

  let join_seq (why, how, cutnav, args, fmt, hastipval, (seq, rewinf), trs, ress)
      =
    seq
  ...
  let rewinfProoftree = function
    ...
    | Join (why, how, cutnav, args, fmt, hastipval, seq, trs, ress) ->
        rewinf_merge
          (snd args, rewinf_merge (snd seq, rewinf_merge (snd trs, snd ress)))

  let tip_seq (seq, rewinf, hastipval) = seq

  let joinopt f = function Tip _ -> None | Join j -> Some (f j)

  let howrule = function Apply (r, _, _) -> Some r | _ -> None

  let howparams = function Apply (_, ps, _) -> Some ps | _ -> None

  let rule = function Tip _ -> None | Join j -> howrule (join_how j)

  let params = function Tip _ -> None | Join j -> howparams (join_how j)

  let args = function Tip _ -> [] | Join j -> join_args j
  ...
  let ruleprovisos name =
    ...
    | _ ->
        raise
          (Catastrophe_
             [
               "prooftree.ruleprovisos can't find thing named ";
               string_of_name name;
             ])
  ...
  let thinned = function Join j -> join_thinned j | _ -> ([], [])

  let isTip = function Tip _ -> true | _ -> false

  let hasTip = function Tip _ -> true | Join j -> join_hastip j

  let tshaveTip ts = nj_fold (fun (a, b) -> a || b) (hasTip <* ts) false

  let subtrees = function Tip _ -> [] | Join j -> join_subtrees j

  let sequent = function Tip t -> tip_seq t | Join j -> join_seq j

  let rec format = function Tip (_, _, fmt) -> fmt | Join j -> join_fmt j
  ...
  let rec decode_cutnav j =
    ...
    | ( why,
        ...
        ress ) ->
        CutNav (l, r, tl, tr)
    | why, how, _, args, fmt, hastipval, seq, (ts, rewinf), ress -> NormalNav ts
  ...
  let rec joinstep go stop skip j path =
    ...
    | why, how, _, args, fmt, hastipval, seq, (ts, rewinf), ress -> (
        ...
        | n :: ns ->
            go n
              ( try Listfuns.guardednth ts n
                with Listfuns.Bad_nth ->
                  raise (FollowPath_ ("out of range", path)) )
              ns
        | [] -> stop () )

  let rec pathto t =
    match t with
    | Join (why, how, Some (l, r), args, fmt, hastipval, seq, (ts, rewinf), ress)
      ->
        [ r ]
    | _ -> []
  ...
  let rec followPath_ns t ns =
    ...
    | Join j ->
        joinstep
          (fun _ -> followPath_ns)
          (fun _ -> t)
          (fun (l, r, tl, tr) -> followPath_ns tr)
          j ns

  let rec onestep_ns t path =
    match t with
    | Tip _ ->
        if null path then None else raise (FollowPath_ ("onestep at tip", path))
  ...
  let rec fakePath_ns rf t ns =
    ...
    | Join j ->
        joinstep
          (fun n -> fakePath_ns (n :: rf))
          (fun _ -> List.rev rf)
          (fun (l, r, tl, tr) -> fakePath_ns (r :: rf) tr)
          j ns

  let rec getTip_ns t ns =
    match followPath_ns t ns with Tip t -> t | _ -> raise FindTip_
  ...
  let rec allTips_ns t =
    let rec moretips a1 a2 a3 =
      ...
      | ns, Join j, tips -> (
          ...
          | NormalNav ts ->
              nj_fold
                (fun ((n, t), tips) -> moretips (n :: ns) t tips)
                (numbered ts) tips
          | CutNav (l, r, tl, tr) -> moretips (l :: ns) tl (moretips ns tr tips)
          )
    in
    moretips [] t []
  ...
  let rec allTipConcs_ns t =
    (function
      ...
      | _ -> raise (Catastrophe_ [ "allTips gave non-Tip in allTipConcs_ns" ]))
    <* allTips_ns t
  ...
  let rec findRightwardsGoal_ns skip t path =
    ...
    | Join j ->
        let rec go n subt ns =
          search (findRightwardsGoal_ns skip subt ns) n |~~ fun _ ->
          match decode_cutnav j with
        ...
        joinstep go
          (fun _ -> _G t)
          (fun (l, r, tl, tr) -> findRightwardsGoal_ns skip tr)
          j path
  ...
  let rec deepest_samehyps tree (FmtPath ks) =
    let rec shs t path =
      let rec topres () =
        ...
        (Some hs, pathto t)
      in
      let unFRESH = not <.> List.exists isFreshProviso <.> List.map snd in
      let rec check a1 a2 =
        match (a1, a2) with
        | (None, ns), f -> (None, f ns)
        | (Some hs, ns), f ->
  ...
  let rec siblingPath_ns t path left =
    ...
    let rec f t ns =
      ...
      | Join j ->
          let rec go n t ns =
            search (f t ns) n |~~ fun _ ->
            match decode_cutnav j with
            | NormalNav ts ->
                if left then if n = 0 then badLeft () else Some [ n - 1 ]
  ...
  let rec validelement_ns ishyp proof el ns =
    ...
    let rec elements tree =
      let (Seq (_, hs, cs)) =
        match tree with Join j -> join_seq j | Tip t -> tip_seq t
      in
      explodeCollection (if ishyp then hs else cs)
    in
    try isin (elements (followPath_ns proof ns)) with _ -> false
  ...
  let rec stillopen_ns proof ns =
    try match followPath_ns proof ns with Tip _ -> true | _ -> false
    with _ -> false
  ...
  let rec string_of_prooftree_step = function
    | Apply a ->
        "Apply"
        ^ string_of_triple parseablestring_of_name string_of_termlist
            string_of_bool "," a
    | Given g ->
        "Given" ^ string_of_triple idf string_of_int string_of_bool "," g
    | UnRule u ->
        "Unrule"
        ^ string_of_pair idf
  ...
  let rec string_of_Join tlf subtreesf
      (why, how, cutnav, args, fmt, hastipval, seq, trs, ress) =
    implode
      [
        "Join(";
        ...
        ", args=";
        string_of_pair string_of_termlist string_of_rewinf ", " args;
        ", fmt=";
        tlf fmt;
        ", hastipval=";
        string_of_bool hastipval;
        ", seq=";
        string_of_pair elementstring_of_seq string_of_rewinf ", " seq;
        ", subtrees=";
        string_of_pair subtreesf string_of_rewinf ", " trs;
        ", ress=";
        (let string_of_ths =
           bracketed_string_of_list (debugstring_of_element string_of_term) ", "
         in
         string_of_pair
           (string_of_pair string_of_ths string_of_ths ", ")
           string_of_rewinf ", " ress);
        ")";
      ]
  ...
  let rec string_of_prooftree tlf t =
    let rec pft tlf rp t =
      ...
      | Tip t ->
          "Tip"
          ^ string_of_triple elementstring_of_seq string_of_rewinf tlf ", " t
      | Join j ->
          implode
            (interpolate "\n"
               ...
               ( match decode_cutnav j with
               | NormalNav ts ->
                   (fun (i, t) -> pft tlf (i :: rp) t) <* numbered ts
               | CutNav (l, r, tl, tr) ->
  ...
  let step_argmap ps args =
    ...
    *)
    let argmap =
      ...
      with Zip_ ->
        raise
          (Catastrophe_ [ "params and args won't zip in prooftree.step_argmap" ])
    in
    if !autoAdditiveLeft then
      match split (function _, Collection _ -> true | _ -> false) argmap with
      | [ _ ], oks ->
          (* if not (List.exists (existsterm isUnknown) args) then oks else argmap *)
          oks
          (* it can't be slower, even if it contains unknowns ... *)
      | _ -> argmap
    else argmap
  ...
  let rec catelim_prooftree2tactic tree provisos givens tail =
    ...
    let rec traverse = function
      ...
      | Join j, ts -> (
          ...
          | [] -> tr ts
          | ls ->
              nj_fold (fun (l, t) -> LayoutTac (t, l)) ls (SeqTac (tr [])) :: ts
          )
    in
    let rec hashables = function
      | Tip _, hs -> hs
      | Join j, hs ->
          let rec catalogue = function
            | Collection (_, _, els), hs ->
                nj_fold
                  (function
                    | Element (_, _, t), hs -> catalogue (t, hs) | _, hs -> hs)
                  els hs
            | arg, hs -> (
                ...
                | None ->
                    if
                      existsterm
                        (function Literal (_, Number _) -> true | _ -> false)
                        arg
      ...
      try
        ...
        let lookup, reset =
          ...
          *)
          ...
          (Cache.lookup, Cache.reset)
        in
        let hs =
          try sortunique ( < ) (hashables (tree, [])) with Can'tHash_ -> []
        in
        let body =
        ...
        let hashables_body =
          if null hs then body
            ...
            :: catelim_string_of_list
                 (fun (i, t) tail ->
                   string_of_int i :: " "
                   :: catelim_string_of_term (comma_enbracket t) tail)
                 ",\n"
  ...
  let rec getrawinfPairDiff rawf1 rawf2 ((x1, x2), z) = rawf1 (x1, rawf2 (x2, z))

  let rec getrewinfList rawf xs = raw2rew_ (getrawinfList rawf (xs, nullrawinf))
  ...
  let rec updaterewinf cxt rewinf =
    ...
    let changeduts =
      nj_fold
        ...
        uVIDs []
    in
    let addedinf = getrewinfList (rawinfTerm cxt) (snd <* changeduts) in
    let changedus = fst <* changeduts in
    let vars' =
      sortedlistsub
        (function Unknown (_, u, _), u' -> u = u' | _ -> false)
        vars changedus
    in
    let uVIDs' = sortedlistsub (fun (x, y) -> x = y) uVIDs changedus in
    let badres' =
      sortedlistsub (fun (x, y) -> x = y) badres (fst <* changedres)
      @ nj_fold
          (function ResUnknown i, is -> i :: is | _, is -> is)
          (snd <* changedres) []
    in
    rewinf_merge (raw2rew_ (mkrawinf (vars', uVIDs', badres', psig)), addedinf)

  let rec rew_stuff cxt rew (x, inf) =
    match rew cxt x with Some x -> (x, updaterewinf cxt inf) | None -> (x, inf)
    ...
    | ( cxt,
        ...
        if ((ra || rs) || rts) || rress then
          ...
          let a =
            if ra then
              rew_stuff cxt
                (fun cxt -> option_rewritelist (rew_Term true cxt))
                a
            else a
          in
          let s =
          ...
          let t =
            if rts then
              rew_stuff cxt
                (fun cxt -> option_rewritelist (rew_Prooftree cxt))
                t
            else t
          in
  ...
  let rec rewriteProoftree givens grounded cxt tree =
    ...
    let tvars =
      tmerge (rewinf_vars inf)
        (match exteriorinf cxt with Some ri -> rewinf_vars ri | None -> [])
    in
    let cxt =
      match
    ...
    let gvars =
      nj_fold (uncurry2 tmerge) (seqvars termvars tmerge <* givens) []
    in
  ...
  let rec mkJoin cxt why how args fmt seq tops ress =
    ...
    Join
      ( why,
        how,
        None,
        (args, getrewinfargs cxt args),
        fmt,
        tshaveTip tops,
        (seq, getrewinfSeq "mkJoin" cxt seq),
        (tops, getrewinfProoftreeList tops),
        (ress, getrewinfress cxt ress) )

  exception AlterProof_ of string list

  let rec applytosubtree_ns path t f =
    let rec ans () =
      let infchanged, t' = f t in
      (infchanged, pathto t', t')
    in
    match t with
    | Tip _ ->
    ...
    | Join j ->
        let rec res infchanged ns subts =
          ...
          let t' =
            Join
              (withhastipval (withsubtrees j (subts, subinf)) (tshaveTip subts))
          in
  ...
  let rec set_prooftree_fmt tree (FmtPath ns) fmt' =
    ...
    thrd
      (applytosubtree_ns ns tree (function
    ...
    thrd
      (applytosubtree_ns ns tree (function
        | Join (why, how, _, args, fmt, htv, seq, ts, ress) ->
            if
              match (cutnav, ts) with
              | None, _ -> true
              | Some _, ([ _; _ ], _) -> true
              | _ -> false
            then (false, Join (why, how, cutnav, args, fmt, htv, seq, ts, ress))
            else
  ...
  let rec truncateprooftree cxt (FmtPath ns) tree =
    let _, ns, tree =
      applytosubtree_ns ns tree (function
        | Join j ->
            let t' = mkTip cxt (rewriteseq cxt (join_seq j)) (join_fmt j) in
            (true, t')
        | tip -> (false, tip))
    in
    (FmtPath ns, tree)
  ...
  let rec insertprooftree cxt (FmtPath ns) tree subtree =
    let _, ns, tree =
      applytosubtree_ns ns tree (fun inspoint ->
          if
            eqseqs
          ...
          then
            let t' =
              set_prooftree_fmt subtree
                (FmtPath (rootPath_ns subtree))
            ...
            raise
              (AlterProof_
                 [
                   ...
                   " => ";
                   string_of_seq (anyway (rew_Seq true cxt) (sequent subtree));
                 ]))
    in
    (FmtPath ns, tree)
  ...
  let rec makewhole a1 a2 a3 a4 =
    ...
    | cxt, Some (oldtree, (FmtPath oldns as oldpath)), newtree, FmtPath newns ->
        let FmtPath ns, tree = insertprooftree cxt oldpath oldtree newtree in
  ...
  let augmenthyps cxt tree els =
    ...
    let rec augseq (Seq (st, hs, cs)) =
      try Seq (st, _The (augmentCollection hs els), cs)
      with _The_ ->
        raise
          (Catastrophe_
             [ "prooftree.augmenthyps can't augment non-Collection hyps" ])
    in
    let rec aug names = function
      | Tip (seq, inf, fmt) ->
          (Tip (augseq seq, rewinf_merge (inf, rewinf), fmt), names)
          (* Tips have no FRESH provisos, I'm sure *)
      | Join
          ( why,
            how,
            cutnav,
          ...
          let names' =
            ...
            | Apply (n, ps, b) ->
                let provisos =
                  Optionfuns.optionfilter
                    (function
                      | b, Provisotype.FreshProviso (true, _, r, v) ->
                          Some (b, r, v)
                      | _ -> None)
                    (ruleprovisos n)
                in
                if null provisos then []
                else
                      ...
                      raise
                        (Catastrophe_
                           [
                             "can't translate ";
                             string_of_term v;
                             " in prooftree.augmenthyps";
                           ]))
                  <* provisos
            | _ -> []
          in
          let rec augargs = function
            | [] ->
                raise
                  (Catastrophe_
                     [
          ...
          ( Join
              ( why,
                ...
                | Given _ ->
                    ((try augargs args with _ -> args), arginf)
                    (* Given doesn't have args, unfortunately *)
                | _ -> (augargs args, rewinf_merge (arginf, rewinf)) ),
                fmt,
                hastipval,
                (augseq seq, rewinf_merge (seqinf, rewinf)),
                (trs', rewinf_merge (trsinf, rewinf)),
                ress ),
            names'' )
    in
    aug [] tree
  ...
  let rec maxtreeresnum t =
    ...
    | Join j ->
        nj_fold (uncurry2 max)
          (maxseqresnum (join_seq j) :: (maxtreeresnum <* join_subtrees j))
          0
    | Tip t -> maxseqresnum (tip_seq t)

  let isCutRule = isstructurerule CutRule

  let rec isCutStep t (FmtPath ns) =
    try
  ...
  let rec isCutjoin j =
    match join_how j with Apply (r, _, _) -> isCutRule r | _ -> false
    ...
    let rec hideroots fmt ascut (ins, outs) =
      let rec h ((n, ((p, t) as pt)), (ins, outs)) =
        ...
        let rec hr hard j' =
          ...
          then
            let hs =
              (fun (ns, t) -> (p @ ns, t)) <* fst (visibles showall j')
            in
            (hs @ ins, pt :: outs)
          else nohide ()
        in
        ...
        | Join j -> (
            ...
            | TreeFormat (HideRootFormat, _) ->
                ...
                if
                  ascut && n = 0
                  (* && List.length (fst r) <> 1 + List.length ins *)
                then (
  ...
  let visfn f showall = function Tip _ -> None | Join j -> Some (f showall j)

  let visible_subtrees showall = optf fst <.> joinopt (visibles showall)

  let invisible_subtrees showall = optf snd <.> joinopt (visibles showall)
  ...
  let viewpathtopath showall t (VisPath ns) =
    let rec rev1 a1 a2 =
      match (a1, a2) with [], ys -> ys | x :: xs, ys -> rev1 xs (x :: ys)
    in
    let rec _P a1 a2 a3 =
      match (a1, a2, a3) with
      | n :: ns, rns, t -> (
          visible_subtrees showall t &~~ fun ts ->
          try
            let ns', t = Listfuns.guardednth ts n in
            _P ns (rev1 ns' rns) t
          with Listfuns.Bad_nth -> None )
  ...
  let rec visreason proved showall t = joinopt (join_visreason proved showall) t
  ...
  and join_visreason proved showall j =
    ...
    let rec rprintf cs invis def =
      ...
      (* bloody OCaml constant syntax.
         ...
         0x73 s
      *)
      let rec rpr = function
        | [] -> []
        | 0x25 (* % *) :: 0x68 (* h *) :: cs ->
            let ss = invis () in
            ss @ rprintf cs (fun () -> ss) def
        | 0x25 (* % *) :: 0x73 (* s *) :: cs ->
            let ss = def () in
            ss @ rprintf cs invis (fun () -> ss)
    ...
    implode
      ( if showall then
        ...
        *)
        let invisf () =
          interpolate ","
            ((* implode (default_reason ()) :: *) invisiblereasons proved
               showall j)
        in
        let f (TreeFormat (_, fmt)) =
          match fmt with
          | RotatingFormat (i, nfs) ->
              let nohidf () = [ !nohidefmt ] in
              let fmt, invis =
                try
                  match Listfuns.guardednth nfs i with
  ...
  and allreasons proved showall t =
    ...
    ( match t with
    | Tip _ -> []
    | Join j -> List.concat (allreasons proved showall <* join_subtrees j) )
  ...
  let rec tranproof proved showall hideuselesscuts t =
    let rec visp = function
      | Tip (seq, rewinf, _) -> ([], Tip (seq, rewinf, VisFormat (false, false)))
      | Join ((why, how, cutnav, args, fmt, htv, seq, subts, ress) as j) as it
        ->
          ...
          let lps =
            nj_fold
              (uncurry2 (sortedmerge earlierresource))
              (fst <* lpsNsubts')
              (sort earlierresource (fst (fst ress)))
          in
          let subts' = snd <* lpsNsubts' in
          let hidecut =
            if showall || not (isCutjoin j) then false
            else
              let newhyps subt =
                listsub sameresource (hyps (sequent subt)) (hyps (fst seq))
              in
                  ...
                  | [ t1; t2 ], _ -> (
                      ...
                      *)
                      ...
                      | Some (TreeFormat (HideRootFormat, _)) -> (
                          ...
                          | [ ch ], [ c ] -> not (eqelements eqterms (ch, c))
                          | _ -> false (* why not? *) )
          ...
          ( lps,
            Join
              ( _The (visreason proved showall it),
                how,
                None,
                args,
                VisFormat (join_multistep j viss, hidecut),
                tshaveTip subts',
                seq,
                (subts', snd subts),
                ress ) )
  ...
  module Fmttree :
    ...
    let rec dePath (FmtPath ns) = ns

    let rec rePath (ns, x) = (FmtPath ns, x)

    let rec followPath t = followPath_ns t <.> dePath

    let rec onestep t = optf rePath <.> onestep_ns t <.> dePath
    ...
    let rec validelement b t el (FmtPath ns) = validelement_ns b t el ns
    ...
    let rec findRightwardsGoal skip t (FmtPath ns) =
      optf fFmtPath (findRightwardsGoal_ns skip t ns)
  ...
  module Vistree :
    ...
    let rec validelement b t el (VisPath ns) = validelement_ns b t el ns
    ...
    let parentPath t = fVisPath <.> parentPath_ns t <.> dePath

    let siblingPath t path = fVisPath <.> siblingPath_ns t (dePath path)

    let subgoalPath t path = fVisPath <.> subgoalPath_ns t (dePath path)
    ...
    let rec findRightwardsGoal skip t (VisPath ns) =
      optf fVisPath (findRightwardsGoal_ns skip t ns)
