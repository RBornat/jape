...
let rec words s =
  let rec wd a1 a2 =
    ...
    | res, [] -> (List.rev res, [])
    | res, [ c ] ->
        if c <= Char.code ' ' then (List.rev res, [])
        else (List.rev (c :: res), [])
    | res, 0x22 :: cs -> qd res cs
    | res, c :: cs ->
        if c <= Char.code ' ' then (List.rev res, unspace cs)
        else wd (c :: res) cs
  and qd a1 a2 =
    match (a1, a2) with
...
type oraclerec = {
  translatehyps : term -> string;
  translateconcs : term -> string;
  turnstile : term -> term -> string;
  from_or : unit -> string;
  to_or : string -> unit;
  kill_or : unit -> unit;
}
  ...
  try
    ...
    let table =
      Array.init 256 (fun n ->
          if n < 128 then String.make 1 (Char.chr n) else "\\" ^ string_of_int n)
    in
    ...
    ( try
        while true do
          ...
          | "ASCII" :: wd :: (wd2 :: _ as wds) ->
              let n =
                try Stdlib.int_of_string wd with Failure _ -> ordof wd 0
              in
              mapped := true;
              if 1 <= n && n <= 255 then table.(n) <- respace wds
          | wd :: wds -> (
              match wd.[0] with
              | '#' -> ()
...
let rec string_of_hypconc punct mapped table term =
  let rec translatewith table string =
    implode (List.map (fun c -> table.(c)) (utf8_explode string))
  in
...
let rec line_from s =
  let r = ref "" in
  Moresys.onInterrupt
    (fun _ -> raise Interrupt)
    (fun () -> try r := input_line s with Interrupt -> r := "INTERRUPTED");
  !r

let rec flush s = try Stdlib.flush s with _ -> consolereport [ "[Flush]" ]

let rec string_to s t =
  try output_string s t with _ -> consolereport [ "[String To]" ]

let rec createoracle oraclename (store, table, mapped) =
  let trans d s = try Hashtbl.find store s with Not_found -> d in
  let rec transhyps _HS =
    string_of_hypconc (trans ", " "hypjoin") mapped table _HS
  in
  let rec transconcs _CS =
    string_of_hypconc (trans ", " "concjoin") mapped table _CS
  in
  let _TS = trans "|-" "turnstile" in
  let rec turnstile _HS _CS =
  ...
  | server :: args -> (
      try
        ...
        ( Some
          ...
          : oraclerec option )
      with _ -> _NoneBecause [ "Oracle cannot start "; server ] )
  | [] -> (
      ...
      | inpipe :: outpipe :: _ -> (
          try
            ...
            ( Some
              ...
              : oraclerec option )
          with _ ->
            _NoneBecause [ "Oracle couldn't open pipes "; inpipe; " "; outpipe ]
          )
      | [ _ ] ->
          _NoneBecause [ "Oracle pipes attribute needs in and out parts" ]
      | _ ->
          _NoneBecause
...
let rec getmapping oraclename =
  ...
  | None -> (
      ...
      | Some m -> (
          match createoracle oraclename m with
          | Some _or ->
              mappings :=
                Mappingfuns.( ++ ) !mappings
                  (Mappingfuns.( |-> ) oraclename _or);
              Some _or
          | None -> None )
      | None -> None )
...
let rec resetoracle () =
  Mappingfuns.formappingpairs
    ((fun (name, ({ kill_or } : oraclerec)) -> kill_or ()), !mappings)

let rec _Oracle (turnstile : string) (cxt : Cxttype.cxt) (_HS : term)
    (_CS : term) (oracle : string) (args : string list) =
  let oracle = disQuote oracle in
  match getmapping oracle with
  | (Some { translatehyps; translateconcs; turnstile; from_or; to_or; kill_or } :
      oraclerec option) -> (
      ...
      match from_or () with
      | "yes\n" -> Some cxt
      | "no\n" -> _NoneBecause [ "Oracle "; oracle; " couldn't prove it" ]
      | "INTERRUPTED" ->
          kill_or ();
          _NoneBecause [ "Oracle "; oracle; " was interrupted (killed it)" ]
      | "" ->
          kill_or ();
          _NoneBecause [ "Oracle "; oracle; " was interrupted (killed it)" ]
      | s -> _NoneBecause [ "Oracle "; oracle; " replied \""; s; "\"" ] )
  | None -> None
