...
\item [Various \textsc{let} ... \textit{tactic... tactic} tacticals] all of which use `matching' (specialised unification, see `binding tactic' below). All of them are `guarded tactics' which can be used in \textsc{when}. In each case, if the match succeeds, proceed with \textit{tactic... tactic}; otherwise fail. \vspace{-25pt}
...
\item [\textsc{letmatch} \textit{pat1 pat2} \textit{tactic... tactic}]: match \textit{pat1} with \textit{pat2};
