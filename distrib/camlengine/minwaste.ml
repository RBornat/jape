...
let rec minw w a n =
  ...
  let rec split i j mgs ss =
    let rec foundone first = first :: split (i + 1) j (mgs - a.(i)) ss in
    (* *)
    let _ =
  ...
  let rec choose ss =
    ...
    in
    (* *)
    if List.exists (function 0 -> true | _ -> false) thiss then prevs
...
module Hashtab = Cache.F (struct
  type dom = int * int list
