...
let rec varmappedto facts v vts =
  let rec check a1 a2 =
    match (a1, a2) with
    | def, (v', t') :: vts -> (
        match substeqvarsq facts v v' with
        | Yes -> Some t'
...
let rec vtmetacount vts =
  nj_fold
    (fun (x, y) -> x + y)
    ((fun (v, _) -> if isUnknown v then 1 else 0) <* vts)
    0

let rec vtmaps facts vts (v, _) =
  List.exists (fun (v', _) -> qDEF (substeqvarsq facts v v')) vts

let rec vtminus facts vts1 vts2 = not <.> vtmaps facts vts2 <| vts1
...
let rec plussubstmap facts vtout vtin =
  ...
  | ys, ns, [] ->
      Some
        (((fun (v, t) -> (v, simplifySubstAnyway facts vtout t)) <* vtin) @ ns)
  | _ -> None
...
and restrictsubstmap facts vts bs ss =
    ...
    if !substdebug then
      consolereport
        [
          ...
          string_of_vts vts;
          " ";
          string_of_termlist bs;
          " ";
          string_of_termlist ss;
          " => ";
          f ();
        ];
    r
  ...
  let rec foundinside v =
    List.exists (not <.> qDEFNOT <.> varoccursinq (newfacts v) v) ss
  in
  ...
  let rec res r _P =
    if !substdebug then (
      consolereport
        [
          "varoccursinq ";
          string_of_facts facts;
          " ";
          string_of_term v;
          " ";
          string_of_termarg _P;
          " => ";
          string_of_answer r;
        ];
      r )
      ...
      | Subst (_, _, _P, vts) ->
          Some
            (res
               (orelseq
                  (existsq
                     (fun (v', t') -> andalsoq (_OFv t') (fun _ -> _OF v' _P))
                     vts)
                  (fun _ -> insidebinding (substmapdom vts) [ _P ]))
               _P)
      | Binding (_, (bs, ss, us), _, _) ->
          Some
            (res (orelseq (existsq _OFv us) (fun _ -> insidebinding bs ss)) _P)
      | _ -> None
  in
  let rec foldsearch (_P, sofar) =
    match sofar with
    | Yes -> Some Yes
    | No -> search _P
    | Maybe -> (
        match search _P with None -> None | Some v -> Some (orq (sofar, v)) )
  in
  foldterm foldsearch No _P

and simplifySubstAnyway facts vts _P =
...
and simplifySubst a1 a2 a3 =
  ...
  | facts, [], (Subst (_, _, _P', vts') as _P) ->
      ...
      if !substdebug then simpres [] _P r;
      r
  | facts, [], _P ->
      let r = Some _P in
      if !substdebug then simpres [] _P r;
  ...
  | facts, vts, _P -> (
      let rec res r =
        if !substdebug then simpres vts _P r;
        r
      in
      let _S = simplifySubstAnyway facts in
      let rec more vts' _P' = Some (_S vts' _P') in
      let rec _Svar v =
      ...
      | Tup (_, sep, ts) -> res (Some (registerTup (sep, _S vts <* ts)))
      | Literal k -> res (Some _P)
      | Fixapp (_, ss, ts) -> res (Some (registerFixapp (ss, _S vts <* ts)))
      | Subst (_, _, _P', vts') -> (
          match plussubstmap facts vts vts' with
          | Some vts'' -> res (more vts'' _P')
          | None -> fail _P )
      | Binding (_, (bs, ss, us), env, pat) -> (
          match restrictsubstmap facts vts bs ss with
          | Some vts' ->
              res
                (Some
                   (registerBinding
                      ((bs, _S vts' <* ss, _S vts <* us), env, pat)))
          | None -> fail _P )
      | Collection (_, k, es) ->
          let rec se = function
          ...
          in
          res
            (try Some (registerCollection (k, se <* es)) with Whoops_ -> None) )

and simplifysubstmap facts _P vts =
  ...
  let rec _W = function
    | v, Subst (_, r, _P, m) -> (
        match simplifySubst facts m _P with
        | Some t -> Some (v, t)
        | None -> None )
    | _ -> None
  in
  let rec _V (v, t) = eqterms (v, t) || qDEFNOT (varoccursinq facts v _P) in
  let rec _U = function
    | [] -> None
    | vt :: vts -> (
        if _V vt then Some (anyway _U vts)
          ...
          | None -> (
              match _U vts with Some vts -> Some (vt :: vts) | None -> None ) )
  in
  let r = _U vts in
  if !substdebug then
    consolereport
