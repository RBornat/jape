...
let rec stringfromsymbol sy =
  match name_of_stringsymbol sy with Some (Name s) -> Some s | None -> None
...
let rec parseRuleHeading mustdecl ignoreable =
  ...
  ignore ignoreable;
  RuleHeading (name, formals, provisos)
...
and parseformal mustdecl =
  try
    ...
    | SHYID "ABSTRACTION" -> (
        ...
        | ID (s, Some c) ->
            scansymb ();
            Abstractionparam (vid_of_string s, c)
    ...
    | SHYID "OBJECT" -> (
        ...
        | ID (s, Some c) ->
            scansymb ();
            Objectparam (vid_of_string s, c)
        | _ -> raise (Matchinparseformal "an identifer after OBJECT") )
    | ID (s, Some c) ->
        scansymb ();
        Ordinaryparam (vid_of_string s, c)
    | ID (s, None) ->
        if mustdecl then
          raise
            (ParseError_
               [ "unclassified identifier "; s; " in formal parameter list" ])
        else (
          scansymb ();
          Ordinaryparam (vid_of_string s, NoClass) )
    | UNKNOWN (s, Some c) ->
        scansymb ();
        Unknownparam (vid_of_string s, c)
    | UNKNOWN (s, None) ->
        if mustdecl then
          raise
            (ParseError_
          ...
          scansymb ();
          Unknownparam (vid_of_string s, NoClass) )
    | sy -> raise (Matchinparseformal "an identifier in formal parameter list")
  with Matchinparseformal s ->
    raise
      (ParseError_
...
let rec itisaparamlist () =
  match currsymb () with
  | BRA "(" ->
      let rec res ps (v : bool) =
        List.iter (fun s -> putbacksymb s) ps;
        v
      in
      let rec param ps =
        match currsymb () with
...
let rec parseClassID () =
  ...
  | s ->
      raise
        (ParseError_
           [ "Expecting an identifier, found "; debugstring_of_symbol s ])

let rec processKEYBOARD report query =
  let _ = ignore _ISWORD in
  let symbols =
    parseUnsepList canstartnovelsymb (fun _ ->
        let r = currnovelsymb () in
        scansymb ();
        r)
  in
...
let rec processClassDecl report query class__ =
  let rec doit s =
    ...
    | Some (s', class') ->
        (* showInputError report ["warning: ", unparseidclass class, " ", s,
...
let rec processCLASS report query =
  ...
  and warn s (s', class') =
    showInputError report
      [
        "warning: CLASS ";
        unparseidclass class__;
        " ";
        s;
        " may be confused with earlier ";
        unparseidclass class';
        " ";
        s';
        " directive";
      ]
...
let rec processInfix _SYMBCON =
  ...
  let a =
    ...
    | s ->
        ...
        *)
        raise
          (ParseError_ [ s; " found where associativity (L, R or T) expected" ])
  in
  let symbs = parseUnsepList canstartnovelsymb (fun _ -> parseFixitySYMB ()) in
...
let processPushSyntax () =
  ...
  | STRING s ->
      scansymb ();
      Symbol.pushSyntax s;
      Termparse.pushSyntax s;
      Sequent.pushSyntax s
  | s ->
      raise
        (ParseError_
           [ "PUSHSYNTAX expects a string; found: "; debugstring_of_symbol s ])
...
let popAllSyntaxes () =
  Symbol.popAllSyntaxes ();
  Termparse.popAllSyntaxes ();
  Sequent.popAllSyntaxes ()
...
let rec parseUntilSHYID parser__ =
  if match currsymb () with EOF -> true | SHYID _ -> true | _ -> false then []
  else
    let v = parser__ () in
    v :: parseUntilSHYID parser__
...
let rec processRightfix () =
  let n = parseNUM () in
  processOutRightfix "RIGHTFIX" (fun bra ->
      enter bra (Some n) None (RIGHTFIX bra))

let rec processOutfix () =
  processOutRightfix "OUTFIX" (fun v -> enter v None None (BRA v))

let rec processBind () =
  let vars = parseUnsepList canstartidentifier (fun _ -> parseVariable ()) in
  let scope =
    match currsymb () with
    | SHYID "SCOPE" ->
        scansymb ();
...
let isScopeHyp t =
  Optionfuns.findfirst
    (fun (vs, pat) ->
      Match.matchvars false (fun v -> List.mem v vs) pat t Mappingfuns.empty)
    !scopehyps
...
let rec processPatchAlert () =
            ...
            if s = commasymbol then (
              ...
              | s ->
                  raise
                    (ParseError_
                       [
                         "bracket expected after alert spec in PATCHALERT, \
                          found ";
              ...
              raise
                (ParseError_
                   [
                     "comma or bracket expected after button label in \
                      PATCHALERT, found ";
                     string_of_symbol (currsymb ());
                   ]) )
    | _ ->
  ...
  and parsealertspec _ =
    ...
    | STRING m -> (
        scansymb ();
        match parsebuttonspecs () with
        | [] ->
            raise (ParseError_ [ "alert in PATCHALERT has no button specs" ])
    ...
    | s -> (
        ...
        | Some "HowToFormulaSelect" ->
            scansymb ();
            Alert.HowToFormulaSelect
        | Some "HowToTextSelect" ->
            scansymb ();
            Alert.HowToTextSelect
        | Some "HowToDragFormulae" ->
            scansymb ();
            Alert.HowToDragFormulae
        | Some "HowToDragDisproofStuff" ->
            scansymb ();
            Alert.HowToDragDisproofStuff
        | _ -> bang () )
  and parsebuttonspecs () =
    parseUnsepList (function BRA "(" -> true | _ -> false) parsebuttonspec
  ...
  | STRING h -> (
      ...
      | BRA "(" ->
          ...
          Alert.patchalert
            (h, Alert.Alert ("", parsebuttonspecs (), parsedefaultindex ()))
...
let rec parseRadioButton report query con =
  ...
  | entries -> (
      ...
      | SHYID "END" ->
          scansymb ();
          con (var, entries, defval)
      | s ->
          showInputError report
            [
              "found symbol ";
              debugstring_of_symbol s;
...
let rec checkvalidruleheading report objkind wherekind
    (RuleHeading (name, params, provisos)) antes conseq fbvs =
  let bodyvars =
  ...
  let rec check vs errorf =
    ...
    | None -> (
        ...
        | _ ->
            obvs := Some (tmerge bodyvars (fbvs ()));
            check vs errorf )
    | Some bvs -> (
    ...
    | [], _ ->
        showInputError report
          [
            ...
            " which ";
            (if List.length vs = 1 then "isn't" else "aren't");
            " in the ";
            wherekind;
            ".";
          ]
    ...
    | dups, rogues ->
        showInputError report
          [
            ...
            has;
            " duplicate ";
            add_an_s "variable" (List.length dups > 1);
            " ";
            sentencestring_of_list string_of_term ", " " and " dups;
            ", and also ";
  ...
  check provars
    (error
       (fun _ ->
         implode
           [
             add_an_s "the proviso" (List.length provisos > 1);
             " ";
...
let rec parseParagraph report query =
  ...
  | SHYID "AUTOMATCH" ->
      scansymb ();
      Some (parseAutoRule true)
  | SHYID "AUTOUNIFY" ->
      scansymb ();
      Some (parseAutoRule false)
  | SHYID "BIND" ->
      scansymb ();
      processBind ();
      more ()
  | SHYID "CONCHIT" ->
      scansymb ();
      Some (parseHitDef DClickConc)
  | SHYID "CONSTANT" ->
      scansymb ();
      processClassDecl report query ConstantClass;
      more ()
  | SHYID "CLASS" ->
      scansymb ();
      processCLASS report query;
      more ()
  | SHYID "CONJECTUREPANEL" ->
      scansymb ();
      Some (parseConjecturePanel report query)
  | SHYID "CURRENTPROOF" ->
      scansymb ();
      Some (parseProof report InProgress)
  | SHYID "CUT" ->
      scansymb ();
      Some (parseStructure "CUT")
  | SHYID "DERIVED" ->
      scansymb ();
      Some (parseDerived report)
  | SHYID "DISPROOF" ->
      scansymb ();
      Some (parseProof report Complete)
      (* some legacy files will have this word *)
  | SHYID "FONTS" ->
      scansymb ();
      Some (parseFontSpec ())
  | SHYID "FORCEDEF" ->
      scansymb ();
      Some (parseForceDefSpec ())
  | SHYID "FORMULA" ->
      raise (ParseError_ [ "FORMULA without CLASS is meaningless" ])
  | SHYID "GIVENPANEL" ->
      scansymb ();
      Some (parseGivenPanel report query)
  | SHYID "HYPHIT" ->
      scansymb ();
      Some (parseHitDef DClickHyp)
  | SHYID "IDENTITY" ->
      scansymb ();
      Some (parseStructure "IDENTITY")
  | SHYID "INFIX" ->
      scansymb ();
      processInfix (fun v -> INFIX v);
      more ()
  | SHYID "INFIXC" ->
      scansymb ();
      processInfix (fun v -> INFIXC v);
      more ()
  | SHYID "JUXTFIX" ->
      scansymb ();
      appfix := parseNUM ();
  ...
  | SHYID "KEYBOARD" ->
      scansymb ();
      processKEYBOARD report query;
      more ()
  | SHYID "INITIALISE" ->
      scansymb ();
      Some (parseInitialise ())
  | SHYID "LEFTFIX" ->
      scansymb ();
      processLeftMidfix (fun v -> LEFTFIX v);
      more ()
  | SHYID "LEFTWEAKEN" ->
      scansymb ();
      Some (parseStructure "LEFTWEAKEN")
  | SHYID "MACRO" ->
      let h =
        scansymb ();
  ...
  | SHYID "MENU" ->
      scansymb ();
      Some (parseMenu report query true)
  | SHYID "MIDFIX" ->
      scansymb ();
      processLeftMidfix (fun v -> MIDFIX v);
      more ()
  | SHYID "NUMBER" ->
      scansymb ();
      processClassDecl report query NumberClass;
      more ()
  | SHYID "OUTFIX" ->
      scansymb ();
      processOutfix ();
  ...
  | SHYID "POSTFIX" ->
      scansymb ();
      processUnifix (fun v -> POSTFIX v);
      more ()
  | SHYID "PREFIX" ->
      scansymb ();
      processUnifix (fun v -> PREFIX v);
      more ()
  | SHYID "PROOF" ->
      scansymb ();
      Some (parseProof report Complete)
  ...
  | SHYID "RIGHTWEAKEN" ->
      scansymb ();
      Some (parseStructure "RIGHTWEAKEN")
  | SHYID "RULE" ->
      scansymb ();
      Some (RuleDef (parseRule report true))
  | SHYID "RULES" ->
      scansymb ();
      Some (parseRules report true)
  | SHYID "SCOPEHYP" ->
      scansymb ();
      processScopeHyp ();
      more ()
  ...
  | SHYID "STRING" ->
      scansymb ();
      processClassDecl report query StringClass;
      more ()
  | SHYID "STRUCTURERULE" ->
      scansymb ();
      Some (parseStructureRule ())
  | SHYID "SUBSTFIX" ->
      scansymb ();
      processSubstfix report query;
      more ()
  | SHYID "TACTIC" ->
      let h =
        scansymb ();
  ...
  | SHYID "TACTICPANEL" ->
      scansymb ();
      Some (parseTacticPanel report query)
  | SHYID "THEOREM" ->
      scansymb ();
      Some (parseTheorem report)
  | SHYID "THEOREMS" ->
      scansymb ();
      Some (parseTheorems report)
  | SHYID "THEORY" ->
      scansymb ();
      Some (parseTheory report query)
  | SHYID "TRANSITIVE" ->
      scansymb ();
      Some (parseStructure "TRANSITIVE")
  | SHYID "UMENU" ->
      scansymb ();
      Some (parseMenu report query false)
  | SHYID "USE" ->
      scansymb ();
      Some
        (let r = parseUse report query in
         scansymb ();
         r)
  | SHYID "VARIABLE" ->
      scansymb ();
      processClassDecl report query VariableClass;
      more ()
  | SHYID "WEAKEN" ->
      scansymb ();
      Some (parseStructure "WEAKEN")
  | _ -> None
...
and processSequentSpec () =
  ...
  in
  ignore _ISWORD;
  describeSeqs (parseList (fun _ -> true) f (SHYID "AND"))
...
and parseRule report axiom =
  ...
  then parseUnnamedRule report None axiom
  else
...
and parseUnnamedRule report nopt axiom =
  ...
  let _ =
    checkvalidruleheading report "rule" "body of the rule" heading antes conseq
      (fun _ -> [])
  in
  (heading, antes, conseq, axiom)
...
and parseMenu report query mproof =
  ...
  let rec mkentry canstart getitem prefix toitem =
    ...
    let item =
      ...
      | SHYID "IS" ->
          scansymb ();
          getitem ()
      | _ -> if canstart (currsymb ()) then getitem () else toitem itemname
    in
  ...
  and parsesimpleentry () =
    ...
    | SHYID "ENTRY" ->
        scansymb ();
        mkentry canstartTerm
          (fun () -> parseTacticEntry report)
          "apply " parseablestring_of_name
    | SHYID "BUTTON" ->
        scansymb ();
        mkentry canstartCommand parseCommand "" string_of_name
    | SHYID "RADIOBUTTON" ->
        scansymb ();
        parseRadioButton report query (fun v -> Mradiobutton v)
    | SHYID "CHECKBOX" ->
        scansymb ();
        parseCheckBox report query (fun v -> Mcheckbox v)
    | SHYID "SEPARATOR" ->
        scansymb ();
        Mseparator
    | s ->
...
and parseConjectureEntry report =
  try
    match name_of_stringsymbol (currsymb ()) with
    | Some _ -> argstring_of_tactic (transTactic (asTactic parseTerm EOF))
    | None ->
        raise
          (ParseError_
             [
               "conjecture name expected in ENTRY; found ";
               string_of_symbol (currsymb ());
             ])
  with ParseError_ ss ->
    showInputError report ss;
    raise Use_
...
and parseTacticEntry report =
  try argstring_of_tactic (transTactic (asTactic parseTerm EOF))
  with ParseError_ ss ->
    showInputError report ss;
    raise Use_
...
and parsePanel report query panelkind parseEntry entrystarters parastarters =
    ...
    if member (currsymb (), parasymbs) then
      ...
      | SHYID "BUTTON" ->
          ...
          let rec getcmd () =
            ...
            | SHYID "COMMAND" ->
                scansymb ();
                CommandInsert :: getcmd ()
            | sy ->
                if canstartCommand sy then
                  let v = StringInsert (parseCommand ()) in
                  v :: getcmd ()
                else []
          in
          let itemcmd =
      ...
      *)
      | sy ->
          raise
            (Catastrophe_
               [ "internal error in parsePanel -- "; string_of_symbol sy ])
  in
  ...
  | s ->
      showInputError report
        ( ( [
              "error in panel description: found ";
              debugstring_of_symbol s;
              ", expecting one of ";
            ]
          @ interpolate ", " starters )
...
and parseCommand () =
  ...
  try
    let f =
      match currsymb () with STRING s -> enQuote s | sy -> string_of_symbol sy
    in
    scansymb ();
    if canstartCommand (currsymb ()) then f ^ " " ^ parseCommand () else f
  with None_ ->
    raise
      (Catastrophe_
         [
           "name_of_stringsymbol failed in parseCommand, looking at ";
           string_of_symbol (currsymb ());
         ])
...
and parseInitialise () =
  let name = name_of_currsymb () in
  let value = asTactic parseTerm EOF in
  InitVar (name, value)

and parseAutoRule sense =
  AutoRule
    (sense, transTactic <* parseUnsepList canstartTerm (asTactic parseTerm))
...
and parseUse report query =
  ...
  | s ->
      raise
        (ParseError_
           [ "USE expects a string; found: "; debugstring_of_symbol s ])
...
and parseTheorems report =
  ...
  let t =
    Theory (p, parseList (fun _ -> true) (fun _ -> parseThm ()) (SHYID "AND"))
  in
  match currsymb () with
  | SHYID "END" ->
      scansymb ();
  ...
  | s ->
      showInputError report
        [
          "error in THEOREMS description: found ";
          debugstring_of_symbol s;
...
and parseTheorem report =
  if member (currsymb (), [ BRA "("; SHYID "WHERE"; SHYID "IS"; SHYID "INFER" ])
  then parseUnnamedTheorem report None
  else
    let heading = parseRuleHeading true _ISWORD in
...
and parseUnnamedTheorem report nopt =
  ...
  in
  let s = parseSeq () in
  let n = match nopt with Some n -> n | None -> Name (string_of_seq s) in
  let heading = RuleHeading (n, params, provisos) in
  let _ =
    checkvalidruleheading report "theorem" "body of the theorem" heading [] s
      (fun _ -> [])
  in
...
and parseProof report stage =
  try
    ...
    consolereport
      [ "reading "; word_of_proofstage stage; " "; string_of_name n ];
    let fs =
      match currsymb () with
      | SHYID "FORMULAE" ->
          scansymb ();
          let fs =
            parseList
              ...
              (fun _ ->
                scansymb ();
                parseTerm commasymbol)
              commasymbol
          in
          readintasarg := Some (Array.of_list fs);
          Some fs
      | _ -> None
    in
...
and parseRules report axiom =
  ...
  in
  let rec nextname () =
    let r = (string_of_name name ^ "'") ^ string_of_int !n in
    incr n;
    r
  in
  let rec parseRle () =
  ...
  in
  let t =
    RuleGroup (p, parseList (fun _ -> true) (fun _ -> parseRle ()) (SHYID "AND"))
  in
  match currsymb () with
  | SHYID "END" ->
      scansymb ();
  ...
  | s ->
      showInputError report
        [
          "error in RULES description: found ";
          string_of_symbol s;
          ", expecting AND or END";
        ];
      raise Use_
...
and parseDerived report =
  ...
  | SHYID "RULE" ->
      scansymb ();
      RuleDef (parseRule report false)
  | SHYID "RULES" ->
      scansymb ();
      parseRules report false
  | sy ->
      showInputError report
...
and parseTheory report query =
  ...
  | s ->
      showInputError report
        [
          "error in THEORY description: found ";
          string_of_symbol s;
          ", expecting one of ";
          string_of_list string_of_symbol ", " parastarters;
          " or END";
        ];
      raise Use_
...
and parseForceDefSpec () =
  ...
  let _ = ignore _ISWORD in
  let d = parseForceDef () in
  ForceDef (t, d)

and parseStructureRule () =
  let rec bang s =
    raise (ParseError_ [ "rule type expected after STRUCTURERULE, found "; s ])
  in
  match currsymb () with
  | SHYID s ->
...
and parseStructure s =
  ...
  ignore (SHYID "RULE");
  ignore _ISWORD;
  if member (s, structurerulestrings) then StructureRule (s, name_of_currsymb ())
...
and paragraphs_of_file report query s =
  ...
  let rec cleanup () =
    poplex st;
    consolereport [ "[CLOSING \""; Usefile.normalizePath s; "\"]" ];
    stopusing ();
    close_in ic
  in
  let error_cleanup () =
    ...
    | MalformedUTF_ ss ->
        showInputError report
          [
            ...
            string_of_list idf "" ss;
            ".\n\n\
             Jape now works only with unicode (UTF-8/16/32) files.\n\n\
             (Are you perhaps reading a old unconverted non-Unicode file? If \
             so, try using the file converter that comes with Jape.)\n\n\
             (Or, if this is the first time you've run the new version of \
             Jape, check that it isn't picking up an unconverted file by \
             default. The file dialogue starts in the last directory where you \
             opened a file: it may be prompting you to pick up the wrong \
             file.)";
          ];
        error_cleanup ();
        raise Use_
...
and catelim_string_of_rulebody linesep provisos antes conseq ss =
  let body =
    "INFER "
    :: catelim_separatedstring_of_seq
         (if linesep = "\n" then "\n     " else linesep)
         conseq ss
  in
  let antes_body =
    if null antes then body
      ...
      :: catelim_string_of_list
           (catelim_separatedstring_of_seq
              (if linesep = "\n" then "\n     " else linesep))
           (linesep ^ "AND ") antes (linesep :: body)
  in
  if null provisos then antes_body
  else
    "WHERE "
    :: catelim_string_of_list catelim_string_of_proviso (linesep ^ "AND ")
         provisos (linesep :: antes_body)
