...
let rec dosubst facts =
  option_mapterm (decodeSubst &~ fun (_, _P, vts) -> simplifySubst facts vts _P)

let rec patvar t =
  (isleaf t && isId t) && isextensibleID (string_of_vid (vid_of_var t))
...
let rec isBagClass t =
  match collectionkind t with Some (BagClass _) -> true | _ -> false
...
let intofdisproofemphasis e =
  match e with Irrelevant -> 0 | Unforced -> 1 | Forced -> 2
...
let catelim_string_of_world =
  catelim_string_of_triple catelim_string_of_emph
...
let rec list_of_universe u =
  List.map (fun (c, (e, ts, cs)) -> (c, e, ts, cs)) (aslist u)

let rec simplestuniverse () = (0, 0) |-> (Unforced, [], [])

let rec issimplestuniverse u =
  match ran u with [ (_, [], []) ] -> true | _ -> false
...
let rec addworldlabel u c t =
  ...
  | Some (_, pts, pcs) ->
      ...
      if label_member (l, pts) then None
        ...
        Some (foldl (domono [ l ]) (u ++ (c |-> (Unforced, l :: pts, pcs))) pcs)
  ...
  | Some (_, pts, pcs) ->
      ...
      if not (label_member (l, pts)) then None
        ...
        let rec doparents u c =
          foldl doparents (doself u c) (islabelledparent u c <| dom u)
        in
        Some (doparents u c)
...
let string_of_definition =
  Stringfuns.string_of_quadruple string_of_term
    ...
    string_of_bool string_of_forcedef ","
...
let rec newoccurrence v =
  match (function t, [ _ ] -> true | _ -> false) <| !occurrences with
  | (occ, [ occv ]) :: _ -> _The (option_remapterm (occv |-> v) occ)
  | _ ->
      raise
        (Catastrophe_
...
let rec newocc t vs os =
  ...
  let t, os' = nodups t vs patf os in
  (t, vs) :: os'
...
let rec addforcedef (t, fd) =
  try
    ...
    let pbs = discardzeroarities (foldterm (findpredicates notabst []) [] t) in
    let rec pvars (_P, abss) =
  ...
  with Predicate_ ss -> raise (ParseError_ ss)
     ...
     if !disproofdebug then
       consolereport
         [
           ...
           string_of_option
             (string_of_triple
                (string_of_mapping string_of_term string_of_term)
                string_of_bool string_of_forcedef ",")
             r;
           " with forcedefs ";
           bracketed_string_of_list string_of_definition ";" !forcedefs;
         ];
     r)
    &~~ fun (env, hassubst, fd) ->
  ...
  if !disproofdebug then
    consolereport
      [
        ...
        string_of_term t;
        " = ";
        string_of_option string_of_forcedef r;
        " with forcedefs ";
        bracketed_string_of_list string_of_definition ";" !forcedefs;
      ];
  r
  ...
  let rec newoccenv ocvs =
    ...
    if !disproofdebug then
      consolereport
        [
          "newoccenv ";
          bracketed_string_of_list string_of_term ";" ocvs;
          " = ";
          string_of_mapping string_of_term string_of_term r;
        ];
    r
  in
  let rec doit ocvs = mapforcedefterms (option_remapterm (newoccenv ocvs)) in
  let rec stopcap fd =
    match fd with
...
let rec semanticfringe facts ts t =
  ...
  *)
  let rec tf ts t =
    ...
    | None -> (
        match decodeBracketed t with Some t' -> tf ts t' | None -> t :: ts )
    | Some fd -> ff ts fd
  and ff ts fd =
    match fd with
    | ForceAlways -> ts
  ...
  *)
  ...
  if !disproofdebug then (
    consolereport
      [
        "indiv_fd ";
        string_of_triple string_of_term
          (bracketed_string_of_list string_of_term ",")
    ...
    | Some env ->
        consolereport
          [
            ...
            string_of_forcedef (mapforcedefterms (option_remapterm env) fd);
          ];
        consolereport
          [
            "mapforcedefterms (option_remapterm env  &~ (somef (dosubst \
             facts))) fd => ";
            string_of_forcedef
              (mapforcedefterms
...
type disproofstaterec = {
  seq : seq;
  selections :
    pos list (* element selection coordinates *) * (pos * string list) list;
  (* text selection coordinates and description *)
...
let rec catelim_string_of_disproofstate
    (Disproofstate
      { seq; universe; selected; tiles; forcemap; conclusive; countermodel }) ss
    =
...
let rec string_of_disproofstate d =
  implode (catelim_string_of_disproofstate d [])

let rec disproofstate_seq (Disproofstate { seq }) = seq

let rec disproofstate_selections (Disproofstate { selections }) = selections

let rec disproofstate_universe (Disproofstate { universe }) = universe

let rec disproofstate_selected (Disproofstate { selected }) = selected

let rec disproofstate_conclusive (Disproofstate { conclusive }) = conclusive

let rec disproofstate_countermodel (Disproofstate { countermodel }) =
  countermodel

(* because of the need for facts, these don't evaluate! *)

let rec withdisproofselections (Disproofstate s) selections =
  Disproofstate { s with selections }

let rec withdisproofuniverse (Disproofstate s) universe =
  Disproofstate { s with universe }

let rec withdisproofselected (Disproofstate s) selected =
  Disproofstate { s with selected }

let rec withdisprooftiles (Disproofstate s) tiles =
  Disproofstate { s with tiles }

let rec disproof_minimal = function
  | None -> true
  | Some (Disproofstate { universe }) -> issimplestuniverse universe

let newforcemap () = Hashtbl.create 17

let newforcefun forcemap facts universe =
  Fix.memofix forcemap (unfixedforced facts universe)

let rec evaldisproofstate facts tree
    (Disproofstate { seq; selections; universe; selected; tiles }) =
  ...
  let countermodel =
    if null selected then false
      ...
      all idf results
  in
  let rec realterm t = match binding_of_term t with Some t' -> t' | _ -> t in
  let rec ivb t =
    let t = realterm t in
    ...
    *)
    try
      let results =
        List.map (fun root -> Hashtbl.find forcemap (root, t)) selected
      in
      fst (if all fst results then onbraket else offbraket)
      ^ if List.exists snd results then fst lockbraket else ""
  ...
  let rec ivk t =
    ...
    try
      let results =
        List.map (fun root -> Hashtbl.find forcemap (root, t)) selected
      in
      (if List.exists snd results then snd lockbraket else "")
      ^ snd (if all fst results then onbraket else offbraket)
    with Not_found -> snd outbraket
  in
  let seqplan =
    makeseqplan 0
      (chooseinvisbracketedstring_of_element ivb ivk)
      true origin seq
  in
  Disproofstate
    {
      ...
      (fun p ts ->
        ...
        | _ ->
            raise
              (Catastrophe_
                 [
                   "Disproof.tint_universe can't find element ";
                   string_of_pos p;
                   " in ";
                   bracketed_string_of_list
                     (debugstring_of_plan string_of_planclass)
                     ";" plan;
                 ]))
      (foldr
         (fun (_, ss) ts ->
           let rec tsel sels =
             ...
             | _ :: t :: s :: sels ->
                 Termparse.term_of_string t :: tsel (s :: sels)
             | [] -> ts
             | _ ->
                 raise
                   (Catastrophe_
                      [
                        "Disproof.tint_universe can't understand text \
                         selection ";
    ...
    optionfilter
      (fun t ->
        let rec isbinder fd =
          ...
          | ForceEverywhere fd' ->
              isbinder fd'
              (* this happens to work for I2L_disproof.j All x.P(x), which is what I want *)
          | ForceNowhere fd' ->
              isbinder fd'
              (* this happens not to work for I2L_disproof.j not(Exists x.P(x)), which is what I want *)
...
and tint_label facts forced bindersels c (_, t) =
  ( ( if
      all1
        (fun bsel ->
          match indiv_fd facts bsel t &~~ term_of_forcedef with
...
let rec alphasort render sel ts =
  List.map sel
    (sortunique
       (fun (s1, _) (s2, _) -> lowercase s1 < lowercase s2)
...
let tiles_of_seq facts seq =
  ...
  let rec iterelement f e =
    match term_of_element e with None -> () | Some t -> iterterm f t
  in
  List.iter
    (fun e ->
      if issegvar e then
        raise
          (Disproof_
             [
               "Forcing semantics can only deal with actual formulae in \
                sequents.";
            ...
            raise
              (Disproof_
                 [
                   ...
                   " in sequent ";
                   string_of_seq seq;
                   ") make disproof somewhat tricky.  Try again when you have \
                    resolved the ";
                   " unknown formulae.";
                 ]))
        e;
              ...
              raise
                (Disproof_
                   [
                     ...
                     " in sequent ";
                     string_of_seq seq;
                     ") aren't part of the forcing semantics.  Try again when \
                      you have ";
  ...
  let ts =
    ( if
      List.exists (bool_of_opt <.> decodeBinding) hypterms
      || List.exists (bool_of_opt <.> decodeBinding) concterms
    then
      List.map fst
        ( (fun (occ, vs) ->
            not (List.exists (bool_of_opt <.> matchit occ vs) ts))
        <| !occurrences )
    else [] )
    @ ts
  in
...
let rec model_of_disproofstate = function
  | None -> None
  | Some (Disproofstate { seq; universe }) ->
      Some
        ( seq,
          Model
            (List.map
               (fun (c, (_, ts, chs)) ->
                 World
                   (Coord c, List.map (fun v -> Coord v) chs, List.map snd ts))
               (aslist universe)) )
...
let rec disproofstate_of_model facts tree disproofopt =
  ...
  | Some (seq, model) ->
      ...
      let utiles =
        nj_fold
          (fun (x, y) -> x @ y)
          (List.map (List.map snd <.> labelsofworld <.> snd) ulist)
          []
      in
      let uvars = nj_fold (uncurry2 tmerge) (List.map variables utiles) [] in
      let tiles =
        List.map newoccurrence uvars @ tiles_of_seq facts seq @ utiles
      in
      let minc =
        foldr
          ...
          List.iter
            (fun ((_, chy) as ch) ->
              if chy <= cy then
                ...
                match u <@> ch with
                | Some (_, chts, _) ->
                    if not (null (listsub (fun (x, y) -> x = y) ts chts)) then
                      raise
                        (ParseError_
                           [
           ...
           (Disproofstate
              {
                ...
                selections = ([], []);
                seqplan = None;
                universe = mkmap ulist;
                tiles = tilesort tiles;
                selected = [ minc ];
                conclusive = false;
                forcemap = newforcemap ();
                countermodel = false;
              }))

let rec checkdisproof facts tree disproofopt =
  match disproofstate_of_model facts tree disproofopt with
  | Some (Disproofstate { countermodel }) -> countermodel
  | _ -> false
...
let disproof_start facts tree pathopt hyps =
  ...
  | None -> (
      ...
      match hyps with [] -> res s | hs -> res (replace_hyps s hs) )
  | Some path ->
      let tree' = subtree tree path in
      let s' = base_sequent tree' in
      res (if null hyps then s' else replace_hyps s' hyps)

let rec newtile (Disproofstate { tiles } as d) t =
  let indivs t =
    let varstrings = List.map string_of_term (variables t) in
    let longest =
      foldl
        (fun long v -> if String.length v > String.length long then v else long)
        "" varstrings
    in
    let appendix = ref "" in
    let template =
  ...
  in
  let rec newtile v v' =
    option_remapterm (v |-> v') t &~~ fun t' ->
    if member (t', tiles) then None else Some t'
  in
  let rec newoccurrence v =
  ...
  if null tvs then None
    ...
    ( if member (t, occs) then
        ...
        | (xs : term list), n ->
            ( let ys : term list list = nlists xs (n - 1) in
              let rec insert a1 a2 =
                ...
                | i, [] -> [ [ i ] ]
                | i, j :: js ->
                    (i :: j :: js) :: List.map (fun js -> j :: js) (insert i js)
              in
              nj_fold
                (fun (x, y) -> x @ y)
                (nj_fold
      ...
      let possibles =
        ...
        <| List.map
             (fun vs -> _The (option_remapterm (mkmap (tvs ||| vs)) template))
             args
      in
      ...
      | _ ->
          ...
          askChoice
            ( "Choose your new tile",
              List.map (fun t -> [ t ]) (List.map snd pairs) )
          &~~ fun i ->
          Some
            ( try fst (Listfuns.guardednth pairs i)
...
let rec addlink u ((_, fromy) as from) ((_, to__y) as to__) =
  ...
  if to__y <= fromy then u
  else if member (to__, cs) then u
  else domono ts (u ++ (from |-> (Unforced, ts, to__ :: cs))) to__
...
let addchildtolink u parent child lfrom lto__ =
  addchild u parent child &~~ fun u ->
  Some (spliceworldtolink u child lfrom lto__)

let deleteworld (Disproofstate { universe; selected } as state) c =
...
let rec worldselect (Disproofstate { selected } as d) cs =
  if
    null (listsub (fun (x, y) -> x = y) selected cs)
...
let moveworld (Disproofstate { universe = u; selected = sels } as d) from
    ((_, toy) as to__) =
  if from = to__ then None
    ...
    *)
    ...
    let fromcs =
      (fun (_, cy) -> toy < cy) <| listsub (fun (x, y) -> x = y) fromcs [ to__ ]
    in
    ...
    let swing ws =
      if member (to__, ws) then listsub (fun (x, y) -> x = y) ws [ from ]
      else List.map switch ws
    in
    let clean ((_, wy) as w) =
      ...
      ( w,
        ( Unforced,
          ts,
          if wy < toy then swing cs
          else listsub (fun (x, y) -> x = y) cs [ from ] ) )
    in
    let u =
      mkmap (List.map clean (listsub (fun (x, y) -> x = y) (dom u) [ from ]))
    in
    let u =
      match u <@> to__ with
      | None ->
          (* easy, no world at this position *)
          u ++ (to__ |-> (Unforced, fromts, fromcs))
      | Some (_, to__ts, to__cs) ->
          (* harder! unite the worlds *)
...
let moveworldtolink d from to__ lfrom lto__ =
  moveworld d from to__ &~~ fun (Disproofstate { universe = u } as d) ->
  Some (withdisproofuniverse d (spliceworldtolink u to__ lfrom lto__))

let showdisproof
    (Disproofstate
      { seq; selections; seqplan = plan; universe; tiles; selected; forcemap })
    =
          ...
          | ElementClass (el, _) ->
              let emph =
                ...
                | Some t -> (
                    try
                      all
                        (fst <.> fun root -> Hashtbl.find forcemap (root, t))
                        selected
        ...
        (List.map
           (fun (c, e, labels, ws) ->
             ( c,
               e = Forced,
               List.map
                 (fun (e, t) -> (e = Forced, string_of_term t))
                 (labelsort labels),
               ws ))
