...
module type Term =
  sig
    ...
    val NJfoldterm : (term * 'a -> 'a option) -> term * 'a -> 'a
...
module
  ...
  struct
    ...
    let rec catelim_resnumstring r tail =
      match r with
        Nonum -> "Nonum" :: tail
      | Resnum r -> "Resnum " :: makestring r :: tail
      | ResUnknown r -> "ResUnknown " :: makestring r :: tail
    ...
    let rec catelim_smltermstring t tail =
      match t with
        Id (h, v, c) ->
          "Id(" ::
            catelim_optionstring (stringfn2catelim makestring) h
    ...
    let rec remake mapterm (_, (bs, ss, us), env, pat as b) =
      try
        ...
        let rec f t =
          match at (emap, t) with
            Some (1, i) -> Some (nth (bs, i))
          | Some (2, i) -> Some (nth (ss, i))
          | Some (3, i) -> Some (nth (us, i))
          | Some (k, i) ->
              raise
                (Catastrophe_
                   ["Some("; makestring k; ","; makestring i; ") in remake ";
      ...
      with
        Nth ->
          raise (Catastrophe_ ["Nth in remake "; smltermstring (Binding b)])
     ...
     * Because T is now properly cat-eliminated, we can't fold quadcolon into the result as we used to.
     ...
     * RB 25/6/2001
     *)
    let rec T ivb ivk n a t s =
      (* a isn't associativity, it's mustbracket if equal priority *)
      let rec mustbracket b a m = if n > m || n = m && a then b else "" in
      let OB = mustbracket "(" a in
      let CB = mustbracket ")" a in
      let OBprefix = mustbracket "(" false in
      let CBprefix = mustbracket ")" false in
      let rec TAP f arg s =
        quadcolon
          (OB !appfix,
           T (ivb, ivk, !appfix, false, f,
              T (ivb, ivk, !appfix, true, arg, quadcolon (CB !appfix, s))))
      in
      let rec TIP n' assoc arg1 f arg2 s =
        quadcolon
          (OB n',
           (let afterf =
              T (ivb, ivk, n', (assoc <> RightAssoc), arg2,
                 quadcolon (CB n', s))
            in
            T (ivb, ivk, n', (assoc <> LeftAssoc), arg1,
               tricolon (f, afterf))))
      in
      let rec TFA sy f t s =
        match sy with
          PREFIX (m, name) ->
            quadcolon
              (OBprefix m,
               quadcolon
                 (name, T (ivb, ivk, m, false, t, quadcolon (CBprefix m, s))))
        | POSTFIX (m, name) ->
            quadcolon
              (OB m,
               T (ivb, ivk, m, false, t,
                  quadcolon (name, quadcolon (CB m, s))))
        | _ -> TAP (f, t, s)
      in
      let rec TT n b sep ts s =
        let rec TT' =
          function
            [] -> s
          | [t] -> T (ivb, ivk, n, b, t, s)
          | t :: ts -> T (ivb, ivk, n, b, t, tricolon (sep, TT' ts))
        in
        TT' ts
      in
      match t with
              ...
              (opname f,
               (fun name ->
                  match lookup name with
                    INFIXC (m, a, _) ->
                      Some
                        (ivb t :: TIP (m, a, arg1, name, arg2, (ivk t :: s)))
                  | _ -> None))
          with
            Some r -> r
          | _ -> ivb t :: TAP (l, arg2, (ivk t :: s))
          end
      | App (_, f, arg) ->
          begin match
            andthenr
              (opname f,
               (fun name ->
                    ...
                    INFIX (m, a, _) ->
                      ...
                      with
                        Tup (_, ",", [arg1; arg2]), _, _ ->
                          Some
                            (ivb t ::
                               TIP (m, a, arg1, name, arg2, (ivk t :: s)))
                  ...
                  | sy -> Some (ivb t :: TFA (sy, f, arg, (ivk t :: s)))))
          with
            Some r -> r
          | None -> ivb t :: TAP (f, arg, (ivk t :: s))
          end
      | Tup (_, sep, ts) ->
          let n' =
          ...
          ivb t ::
            quadcolon
              (OB n', TT (n', true, sep, ts, quadcolon (CB n', ivk t :: s)))
      | Literal (_, Number k) ->(* take in the brackets as well ... *)
         ivb t :: quadcolon (makestring k, ivk t :: s)
      | Literal (_, String k) ->
      ...
      | Fixapp (_, ss, ts) ->
          begin match lookup (List.hd ss) with
            BRA _ ->
              ivb t ::
                quadcolon
                  (List.hd ss, TS1 (ivb, ivk, 0, List.tl ss, false, ts, (ivk t :: s)))
          | LEFTFIX (m, _) ->
              ivb t ::
                quadcolon
                  (OBprefix m,
                     ...
                     (List.hd ss,
                      TS2
                        (ivb, ivk, m, List.tl ss, true, ts,
                         quadcolon (CBprefix m, ivk t :: s))))
          | MIDFIX (m, _) ->
              ivb t ::
                quadcolon
                  (OB m,
                   TS2
                     (ivb, ivk, m, ss, true, ts,
                      quadcolon (CB m, ivk t :: s)))
          | RIGHTFIX (m, _) ->
              ivb t ::
                quadcolon
                  (OB m,
      ...
      | Subst (_, _, t, m) ->
          ivb t ::
            T (ivb, ivk, !substfix, false, t,
               quadcolon
                 (symbolstring SUBSTBRA,
                  TM (ivb, ivk, m,
                      quadcolon (symbolstring SUBSTKET, ivk t :: s))))
      | Binding stuff -> T (ivb, ivk, n, a, remake mapterm stuff, s)
      | Collection (_, c, es) ->
          ivb t ::
            quadcolon
              (OB 0,
    ...
    and TS ivb ivk ts r =
      match ts with
        [] -> r
      | [t] -> T (ivb, ivk, 0, true, t, r)
      | t :: ts ->
          T (ivb, ivk, 0, true, t, quadcolon (",", TS (ivb, ivk, ts, r)))
    and TS1 ivb ivk m seps b ts s =
      match seps, ts with
        [ket], [] -> quadcolon (ket, s)
      ...
      | sep :: seps, t :: ts ->
          ...
          T (ivb, ivk, m, b, t,
             quadcolon (sep, TS1 (ivb, ivk, m, seps, b, ts, s)))
      | _ -> quadcolon ("???TS1???", s)
    and TS2 ivb ivk m seps b ts s =
      match seps, ts with
        [], [t] -> T (ivb, ivk, m, b, t, s)
      | sep :: seps, t :: ts ->
          T (ivb, ivk, m, b, t,
             quadcolon (sep, TS2 (ivb, ivk, m, seps, b, ts, s)))
      | _ -> quadcolon ("???TS2???", s)
    and TM ivb ivk vts s =
    ...
    let catelim_termstring_invisbracketed =
      T ((fun _ -> invisbra), (fun _ -> invisket), 0, false)
    let termstring_invisbracketed =
      catelim2stringfn catelim_termstring_invisbracketed
    let rec catelim_termstring_invischoose ivb ivk = T (ivb, ivk, 0, false)
    ...
    let rec catelim_vtsstring vts ss =
      quadcolon
        (symbolstring SUBSTBRA,
         TM (nobra, nobra, vts, quadcolon (symbolstring SUBSTKET, ss)))
    ...
    let rec catelim_argstring t ss =
      let rec mustbracket t =
        ...
        | Subst (_, _, P, _) -> !substfix <= !appfix || mustbracket P
        | _ -> true
      in
    ...
    let
      ...
      let (cacheterm, resettermstore) =
        simplecache "termstore"
          (fun (t, h, (t' : term)) ->
             pairstring (makestring : int -> string) termstring ", " (h, t))
          127(* why not? It can only grow :-) *)
           (fun t -> t)
      and (cacheelement, resetelementstore) =
        simplecache "elementstore"
          (fun (e, h, (e' : element)) ->
             pairstring (makestring : int -> string) elementstring ", "
               (h, e))
          127(* why not? It can only grow :-) *)
           (fun e -> e)
      in
      ...
      in
      let resettermstore () = resettermstore (); resetelementstore () in
      hashcombine, hash_tepair, hash_telist, registerId, registerUnknown,
      registerApp, registerTup, registerLiteral, registerFixapp,
      registerSubst, registerBinding, registerCollection, registerElement,
      registerSegvar, resettermstore
    ...
    *)
            ...
            | Subst (_, r, P, vts) ->
                andthenr
                  (option_rewrite2 mtff
                     ...
                     (P, vts),
                   (fun (P', vts') -> Some (registerSubst (r, P', vts'))))
    ...
    let rec foldterm f z t =
      ...
      | None ->
          ...
          | _ -> z
    and NJfoldterm f (t, z) = foldterm f z t
        ...
        match g h t with
          None ->
            ...
            | Subst (_, r, P, vts) ->
                ortryr
                  (fh (fun ooo ->
                         h ((fun P -> registerSubst (r, P, vts)) ooo))
                      P,
                   (fun _ ->
                      fhs (fun (v, t) -> t) (fun (v, _) t -> v, t)
                        (fun ooo ->
                           h ((fun vts -> registerSubst (r, P, vts)) ooo))
                        vts))
            | Collection (_, k, es) ->
                let rec sele =
                  function
    ...
    let rec uniqueVID class__ sortedVIDs extraVIDs vid =
      ...
      let rec nextVID n = rootVID ^ makestring (n + 1) in
      let rec E vid n =
        if member (vid, extraVIDs) || symclass vid <> class__ then
          U (nextVID n, (n + 1), sortedVIDs)
        else vid
      and U a1 a2 a3 =
        match a1, a2, a3 with
          vid, n, [] -> E (vid, n)
        | vid, n, vid1 :: vids ->
            if vid < vid1 then E (vid, n)
            else if vid = vid1 then
              let vid' = nextVID n in
              if vid1 < vid' then U (vid', (n + 1), vids)
              else U (vid', (n + 1), sortedVIDs)
            else U (vid, n, vids)
      in
      if isextensibleID vid then
        ...
        else U (vid, (if extVID = "" then 0 else atoi extVID), sortedVIDs)
      else
        raise
          (Catastrophe_ ["uniqueVID "; idclassstring class__; " ... "; vid])
    let rec mergeVIDs xs ys =
      sortedmerge (fun (x, y) -> x < y : vid * vid -> bool) (xs, ys)
    ...
    let rec eqterms (t1, t2) =
      let rec EQs =
        function
          t1 :: t1s, t2 :: t2s -> eqterms (t1, t2) && EQs (t1s, t2s)
        | [], [] -> true
        | _ -> false
      in
      match debracket t1, debracket t2 with
      ...
      | Tup (_, s1, t1s), Tup (_, s2, t2s) -> s1 = s2 && EQs (t1s, t2s)
          ...
          let rec EQvts =
            function
              (v1, t1) :: vts1, (v2, t2) :: vts2 ->
                (eqterms (v1, v2) && eqterms (t1, t2)) && EQvts (vts1, vts2)
            | [], [] -> true
            | _ -> false
          in
          eqterms (P1, P2) && EQvts (vts1, vts2)
      | Binding (_, (bs, ss, us), _, pat),
        Binding (_, (bs', ss', us'), _, pat') ->
          ((pat = pat' && EQs (sort earliervar bs, sort earliervar bs')) &&
           EQs (ss, ss')) &&
          EQs (us, us')
    ...
    let rec eqalphaterms (t1, t2) =
      let count = ref 0 in
      let rec nxb _ = inc count; !count in
      ...
      let rec eq t1bs t2bs (t1, t2) =
        let EQ = eq t1bs t2bs in
        let rec EQs =
          function
            t1 :: t1s, t2 :: t2s -> EQ (t1, t2) && EQs (t1s, t2s)
          | [], [] -> true
          | _ -> false
        in
        let rec doublev () =
        ...
        | Unknown _, _ -> doublev ()
        | _, Unknown _ -> doublev ()
        | App (_, f1, a1), App (_, f2, a2) -> EQ (f1, f2) && EQ (a1, a2)
        | Literal (_, k1), Literal (_, k2) -> k1 = k2
        | Tup (_, s1, t1s), Tup (_, s2, t2s) -> s1 = s2 && EQs (t1s, t2s)
        | Fixapp (_, ss1, t1s), Fixapp (_, ss2, t2s) ->
            ss1 = ss2 && EQs (t1s, t2s)
        | Subst (_, _, P1, vts1), Subst (_, _, P2, vts2) ->
            ...
            let rec EQvts =
              function
                (v1, t1) :: vts1, (v2, t2) :: vts2 ->
                  (EQ (v1, v2) && EQ (t1, t2)) && EQvts (vts1, vts2)
              | [], [] -> true
              | _ -> false
            in
        ...
        | Collection (_, BagClass k1, es1),
          Collection (_, BagClass k2, es2) ->
            ...
            k1 = k2 && eqbags (eqelements EQ) (es1, es2)
        | Collection (_, ListClass k1, es1),
          Collection (_, ListClass k2, es2) ->
            k1 = k2 && eqlists (eqelements EQ) (es1, es2)
        | _ -> false
      in
      let r = t1 = t2 || eq [] [] (t1, t2) in(* if !eqalphadebug then
          consolereport["eqalphaterms (", termstring t1, ",",
                        termstring t2, ") => ", makestring r
                       ]
         else ();
       *)
       r
     ...
     * RB 20/i/93
     *)
    let rec termoccursin t = fun P -> existsterm (curry2 eqterms t) P
