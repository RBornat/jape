...
let leftmidfixtree : (symbol, unit) searchtree ref = ref (emptysearchtree mkalt)
...
let pushSyntax name =
  syntaxes :=
    (name, !outrightfixtree, !leftmidfixtree, !outrightfixes, !leftmidfixes)
    :: !syntaxes;
...
let popSyntax () =
  ...
  | (s, orT, lmT, ors, lms) :: sys ->
      ...
      leftmidfixes := lms;
      syntaxes := sys
  | _ -> raise (ParseError_ [ "termparse popSyntax stack empty" ])
...
let resettermparse () =
  ...
  (* oh dear this was an sml bug -- what next? *)
  ()
...
let rec parseUnsepList start f =
  if start (currsymb ()) then
    let v = f EOF in
    v :: parseUnsepList start f
  else []

let parseList start f sep =
  if start (currsymb ()) then
    let rec more () =
      if currsymb () = sep then (
        scansymb ();
        one () )
      else []
    and one () =
...
let canstartTerm sy =
  ...
  | LEFTFIX _ -> true
  | _ ->
      (* temporary hack to allow Collection formulae *)
...
let rec parseAtom () =
  ...
  | UNKNOWN (s, Some c) ->
      checkclass ("unknown", registerUnknown (vid_of_string s, c))
  | UNKNOWN (s, None) ->
      !undecl "unknown" (registerUnknown (vid_of_string s, NoClass))
  | NUM s -> (
      try registerLiteral (Number (atoi s))
      with _ ->
        raise (Catastrophe_ [ "parseAtom can't convert "; s; " to an integer" ])
      )
...
and parseOutfix bra =
  ...
  match
    fsmpos (rootfsm outrightfixtree) [ bra ] &~~ fun t ->
    match currsymb () with
    | KET _ as ket -> (
  ...
  | Some t -> t
  | None ->
      putbacksymb bra (* can this be right? *);
      snd (parseOutRightfixTail 0 [])

and parseRightfix m t =
  let ket, t = parseOutRightfixTail m [ t ] in
  checkAfterKet ket m;
  t

and parseOutRightfixTail m ts =
  match
    scanstatefsm treecurr (treenext m false) (rootfsm outrightfixtree) ([], ts)
  with
  | Found (ket, (ss, ts)) ->
      check ket;
  ...
  | NotFound (ss, _) ->
      raise
        (ParseError_
           [
             "expected ";
             string_of_list string_of_symbol " or " ss;
             "; found ";
             string_of_symbol (currsymb ());
           ])

and parseLeftfix m bra =
  putbacksymb bra;
  parseLeftMidfixTail m []

and parseMidfix m t = parseLeftMidfixTail m [ t ]

and parseLeftMidfixTail m ts =
  match
    scanstatefsm treecurr (treenext m true) (rootfsm leftmidfixtree) ([], ts)
  with
  | Found (_, (ss, ts)) -> registerFixapp (string_of_symbol <* rev ss, rev ts)
  | NotFound (ss, _) ->
      raise
        (ParseError_
           [
...
and parseExpr n a =
  ...
  let rec pe t =
    ...
    let pq n' a' s down =
      ...
      if n' >> n then
        pe
          ( if a' = TupleAssoc then registerTup (s, t :: pts ())
          else down (s, nextt (a' = LeftAssoc)) )
      else t
    in
    let pr n' down = if n' >> n then pe (down n' t) else t in
    match sy with
    | INFIX s ->
        pq (prio sy) (assoc sy) s (fun (s, t') ->
            registerApp
    ...
    | SUBSTBRA ->
        if !substfix >> n then (
          ...
          checkAfterKet SUBSTKET !substfix;
          pe t )
        else t
    | _ ->
...
and parseSubststuff () =
  ...
  if length xs = length ys then if !substsense then xs ||| ys else ys ||| xs
  else
    (* Zip_ can't happen *)
    raise
      (ParseError_
         [
           ...
           string_of_list string_of_term "," xs;
           string_of_symbol SUBSTSEP;
           string_of_list string_of_term "," ys;
           string_of_symbol SUBSTKET;
           " is unbalanced";
         ])
...
and parseterm fsy =
  ...
  if canstartCollectionidclass (currsymb ()) then
    ...
    let _, els = parseElementList canstartTerm parseterm commasymbol (Some c) in
...
and parseElementList starter parser__ sep k =
  ...
  let iscoll c =
    match c with BagClass _ -> true | ListClass _ -> true | _ -> false
  in
  let parseElement sep =
    let rec getSegvar () =
      ...
      | PREFIX sp as sy -> (
          ...
          | Some (ps, v) ->
              Some (registerId (vid_of_string sp, OperatorClass) :: ps, v)
          | None ->
              putbacksymb sy;
              None )
  ...
  in
  let elements = parseList starter parseElement sep in
  (!kind, elements)
...
and parseTerm sy =
  ...
  and findbinding t =
    ...
    | Some ((bs, us, ss), env, pat) ->
        Some (registerBinding ((bs, doit <* us, doit <* ss), env, pat))
    | None -> None
  in
  doit t
...
let parseCollection k =
  let _, els = parseElementList canstartTerm parseTerm commasymbol (Some k) in
...
let try_negnum s =
  if
    string_of_symbol s = "-"
    && match currsymb () with NUM s -> true | _ -> false
  then
    registerApp
      (registerId (vid_of_string (string_of_symbol s), NoClass), parseAtom ())
  else badstart s
...
let checkTacticTerm t =
  let bang reason t = raise (Tacastrophe_ [ reason; " "; string_of_term t ]) in
  let badvar = function
    | Id (_, _, class__) as t ->
        if class__ <> VariableClass then bang "non-variable in substitution" t
        else None
