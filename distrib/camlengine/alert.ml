...
let rec alertpatched s =
  ...
  let rec patched (h, a) =
    let r =
      if
        String.length h <= String.length s
        && String.sub s 0 (String.length h) = h
      then Some a
      else None
    in
    if !patchalertdebug then
      consolereport
        [
          "    checked ";
          Stringfuns.enQuote h;
          " => ";
          string_of_bool (bool_of_opt r);
        ];
    r
  in
...
let rec patchalert (h, a) =
  if !patchalertdebug then consolereport [ "patchalert patching "; h ];
  let rec f = function
    | ((h', a') as p) :: ps ->
...
let rec ask code m (bs : (string * 'a) list) def =
  if null bs then raise (Catastrophe_ [ "ask no buttons \""; m; "\"" ])
    ...
    if i < 0 then
      ...
      try snd (Listfuns.guardednth bs i)
...
let rec askCancel code m (bs : (string * 'a) list) c def =
  if null bs then raise (Catastrophe_ [ "askCancel no buttons \""; m; "\"" ])
  else if def < 0 || def > List.length bs then
    raise
      (Catastrophe_
         [
           "askCancel bad default \"";
           m;
           "\"";
           bracketed_string_of_list fst "," bs;
           " ";
    ...
    match askCancel_unpatched (intseverity code) m (List.map fst bs) def with
    | Some i ->
        if i < 0 then c (* Java close window is reported as -1, means Cancel *)
        else
          snd
            ( try Listfuns.guardednth bs i
              with Listfuns.Bad_nth ->
                raise
                  (Catastrophe_
                     [
...
let rec showAlert code s =
  ...
  in
  match alertpatched s with None -> display code s | aopt -> patch code aopt
...
and showHowTo s =
  ...
  | "" ->
      raise
        (Tacastrophe_
           [ "Alert.showHowTo doesn't understand"; Stringfuns.enQuote s ])
  | s' -> showAlert Info s'
