...
module type Style = sig
  val style : Displaystate.displaystate
end
...
module F (AAA : sig
  ...
  let rec tranvpath state vpath =
    state &~~ fun (proof, _, _, _, _, showall, _) -> fmtpath showall proof vpath

  let rec tranfpath state fpath =
    state &~~ fun (proof, _, _, _, _, showall, _) -> vispath showall proof fpath
  ...
  let rec locateHit state pos classopt hitkind =
    optf
      (fmtpath_of_ints tranhitpath state)
  ...
  let rec saveanddraw proof pos vgoal vproof plan =
    ...
    screendrawstate
      (Some
         (proof, pos, vgoal, vproof, plan, !showallproofsteps, !hideuselesscuts))

  and refresh proof viewport vgoal vproof plan =
    Draw.clearView ();
    saveanddraw proof (defaultpos viewport plan) vgoal vproof plan
    ...
    *)
    ...
    if !screenpositiondebug then
      consolereport
        [
          "looking for ";
          string_of_option Prooftree.Tree.Fmttree.string_of_path goal;
          " (in visible proof that's ";
          string_of_option string_of_path vgoal;
          ")";
          " target=";
    ...
    | Some (oldproof, oldpos, _, oldvproof, oldplan, oldshowall, oldhideuseless)
      ->
          ...
          | (Some _ as oldpath), (Some _ as path) -> (
              if !screenpositiondebug then
                consolereport
                  [
                    "found oldpath=";
                    string_of_option Prooftree.Tree.Vistree.string_of_path
                      oldpath;
                    "; path=";
                    string_of_option Prooftree.Tree.Vistree.string_of_path
                      oldpath;
              ...
              | Some oldbox, Some box ->
                  if
                    intersects (box_of_textbox oldbox) viewport
                    (* it was visible *)
                  then doit (tbP oldbox) (tbP box)
                  else handleinvis ()
              | _ -> retry target )
          | _ ->
              (* one of the boxes wasn't there *)
              retry target
        and retry = function
          | Some targetpath -> (
              match try Some (parentPath proof targetpath) with _ -> None with
        ...
        and handleinvis () =
          ...
          let acceptable f ts =
            (fun (tbox, _) -> f (box_of_textbox tbox) viewport) <| ts
          in
          ...
          let bad () =
            consolereport
              [
                "We have a problem: Displaystyle.handleinvis can't find a \
                 visible path";
              ];
            consolereport
              [
                ...
                bracketed_string_of_list
                  (string_of_pair string_of_textbox string_of_hit ",")
                  ";" oldts;
                "; viewport=";
                string_of_box viewport;
              ];
            Japeserver.dropdead ();
            (* doesn't return *)
            default ()
            (* would cause an infinite recursion, if it happened *)
          in
          let process ts =
            (* bloody hell, N^2. Oh well. *)
            ...
            findfirst
              (fun (oldbox, hit) ->
                findfirst
                  (fun (box, hit') ->
                    if hit = hit' then Some (oldbox, box) else None)
                  newts)
              ts
          in
          ...
          | grazers -> (
              match
                match acceptable entirelywithin grazers with
                | [] -> process grazers
  ...
  and printProof str proof target goal =
    let viewport = Draw.viewBox () in
    (* daft, but that's what it used to do *)
    let vproof = visproof !showallproofsteps !hideuselesscuts proof in
    let vgoal = vpath !showallproofsteps proof goal in
    let plan = layout viewport (abstracttree vproof) in
  ...
  and showFocussedProof proof viewport goal =
    ...
    *)
    ...
    let vgoal = vpath !showallproofsteps proof goal in
    let plan = layout viewport (abstracttree vproof) in
    match targetbox origin (optf deVis vgoal) plan with
    (* DANGER: but it used to be origin before *)
    | Some box ->
        saveanddraw proof
          (postoinclude viewport (box_of_textbox box) plan)
          vgoal vproof plan
  ...
  and storedProof state () = optf (fun (proof, _, _, _, _, _, _) -> proof) state

  and screendrawstate state =
    DisplayState
      {
        showProof = showProof state;
        showFocussedProof;
        refreshProof = refreshProof state;
        printProof;
        locateHit = locateHit state;
        locateElement = locateElement state;
