...
let baseseqsides cxt =
  ...
  | Exterior ((_, Seq (_, left, right)), _, fvi) -> (
      ( left,
        right,
        match fvi with
        | Some { bhfvs; bcfvs } -> Some (bhfvs, bcfvs)
        | None -> None ) )
...
let rec simplifyProviso facts (p, cats) =
  ...
  let rec np v (t, cats) =
    ...
    | Subst (_, r, _P, vts) ->
        ...
        if List.exists qDEF rs then Some (nj_fold fnp (snd <* vts) cats)
        else if not (List.exists qUNSURE rs) then
          match vts with
          | [ (v', t') ] -> (
    ...
    | _ -> None
  in
  let rec nop ((vs, pat, _C) as n) =
    let rec expand env (v, cats) =
      match env <@> v with
      | Some v' -> simplifyProviso facts (new__ (NotinProviso (v, v')), cats)
    ...
    | Collection (_, class__, es) ->
        ...
        if null defers then cats'
          ...
          new__
            (NotoneofProviso (vs, pat, registerCollection (class__, defers)))
          :: cats'
    | t -> (
        match simp t cats with
        | Some cats' -> cats'
...
let rec deferrable cxt (t1, t2) =
  ...
  | Subst (_, _, (Unknown _ as _P1), vts), _ ->
      ...
      || specialisesto (idclass _P1, VariableClass)
         && List.exists (fun t1' -> deferrable cxt (t1', t2)) (snd <* vts)
...
let rec _PROVISOq facts p =
  ...
  | DistinctProviso vs ->
      let rec dp = function
        | [] -> Yes
        | v :: vs ->
            andalsoq (notq (existsq (varoccursinq facts v) vs)) (fun _ -> dp vs)
      in
      dp vs
  ...
  | UnifiesProviso (_P1, _P2) ->
      if eqterms (_P1, _P2) then Yes
        ...
        (* if termoccursin (debracket _P2) _P1
...
let relevantprovisos seq ps =
  ...
  (fun p ->
    Listfuns.sorteddiff earliervar (provisovars termvars tmerge p) lemmavars
    = [])
  <| ps
...
let rec checker cxt ( -- ) ps qs =
  let rec ch a1 a2 =
    ...
    | p :: ps, qs -> (
        ...
        let qs' = qs -- p in
        (* check p against the _other_ provisos,
           when (--) is (--). Ok, but why would you _not_ do this?.
        *)
        if List.exists (fun q' -> pp = provisoactual q') qs' then ch ps qs'
          (* discard p if there is a proviso in qs' which differs only in visibility *)
        else
          ...
          | Maybe -> p :: ch ps qs )
    (* qs not qs', cos qs is the database you are verifying against *)
  in
  ch ps qs
...
let rec remalldups eq ps =
  let rec rad ps =
    ...
    | [] -> None
    | p :: ps ->
        if List.exists (fun p' -> eq p p') ps then Some (anyway rad ps)
...
let rec verifyprovisos cxt =
  try
    ...
    let pros =
      ...
      let simpleps =
        remalldups
          (fun p p' -> provisoactual p = provisoactual p')
          (nj_fold (simplifyProviso (facts ps cxt)) ps [])
      in
      let _ =
    ...
    let ps =
      let rec dofresh ((((h, g, r, v) as f), ns), pros) =
        ...
        let rec push f ps =
          if
            List.exists
              ( (function FreshProviso (_, _, _, v') -> v = v' | _ -> false)
              <.> provisoactual )
              ps
          then
            (fun vp ->
              ...
              | FreshProviso (h', g', r', v') ->
                  if v = v' then
                    mkvisproviso
                      (true, FreshProviso (h || h', g || g', r && r', v))
                  else vp
              | _ -> vp)
            <* ps
...
let rec checkprovisos cxt =
  try Some (verifyprovisos cxt) with Verifyproviso_ _ -> None
...
let draganddropmapping ps =
  let istarget = function Segvar (_, _, Unknown _) -> true | _ -> false in
  let getsts = function
    | ( UnifiesProviso
          ( Collection (_, BagClass FormulaClass, xs),
   ...
   *)
  let ists =
    not <.> null <.> fst <| ((fun (xs, ys) -> (istarget <| xs, ys)) <* sts)
  in
  let ists =
    nj_fold
      (fun ((ss, ts), rs) -> nj_fold (fun (s, rs) -> (s, ts) :: rs) ss rs)
      ists []
  in
  let sts =
    nj_fold
