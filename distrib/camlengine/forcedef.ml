...
let term_of_forcedef fd = match fd with ForcePrim t -> Some t | _ -> None
...
let rec catelim_string_of_forcedef f ss =
  ...
  | ForceBoth (f1, f2) ->
      "BOTH ("
      :: catelim_string_of_forcedef f1
           (") (" :: catelim_string_of_forcedef f2 (")" :: ss))
  | ForceEither (f1, f2) ->
      "EITHER ("
      :: catelim_string_of_forcedef f1
           (") (" :: catelim_string_of_forcedef f2 (")" :: ss))
  | ForceIf (f1, f2) ->
      ...
      :: catelim_string_of_forcedef f1
           (") (" :: catelim_string_of_forcedef f2 (")" :: ss))
  | ForceEverywhere f ->
      "EVERYWHERE (" :: catelim_string_of_forcedef f (")" :: ss)
  | ForceNowhere f -> "NOWHERE (" :: catelim_string_of_forcedef f (")" :: ss)
  | ForceAll (t, vs, f) ->
      "ALL ("
      :: catelim_string_of_term t
           (") (" :: catelim_string_of_forcedef f (")" :: ss))
  | ForceSome (t, vs, f) ->
      "SOME ("
      :: catelim_string_of_term t
           (") (" :: catelim_string_of_forcedef f (")" :: ss))
...
let rec option_mapforcedef f fd =
  ...
  | None -> (
      match fd with
      | ForceAlways | ForceNever | ForcePrim _ -> None
...
let rec findinforcedef f fd =
  ...
  | ForceAll (t, _, fd) -> findterm f t |~~ fun _ -> findinforcedef f fd
  | ForceSome (t, _, fd) -> findterm f t |~~ fun _ -> findinforcedef f fd
...
let rec parseForceDef () =
  ...
  let decodeApp t =
    match explodeApp false t with f, args -> (string_of_term f, args)
  in
  let rec tranForceDef t =
    ...
    | _ ->
        raise
          (ParseError_
             [
               "FORCE t, EVERYWHERE f, NOWHERE f, ALL pat f, SOME pat f, BOTH \
                f f, EITHER f f ";
               "or IF f f expected in FORCEDEF; found ";
               string_of_term t;
             ])
  and tranPrim t =
    try
  ...
  and tranForceDefBinder pat f =
    ...
    if List.exists (not <.> isextensibleId) vs then
      raise
        (ParseError_
           [
             "ALL and SOME must use CLASS VARIABLE identifiers to describe \
              individuals";
           ])
    ...
    | BRA "(" -> (
        ...
        let rec parseInt () =
          ...
          | sy -> (
              ...
              | s -> bang [ s ] )
        and bang ss =
          raise (ParseError_ ("number expected in coordinate; found " :: ss))
  ...
  let rec parseWorlds () =
    ...
    | SHYID "WORLD" ->
        ...
        let ts =
          ...
          | SHYID "LABELS" ->
              scansymb ();
              parseList canstartTerm parseTerm commasymbol
          | _ -> []
        in
...
let rec catelim_string_of_model a1 a2 =
  ...
  | Some (seq, Model worlds), ss ->
      ...
      in
      let rec catelim_string_of_coord (Coord c) =
        catelim_string_of_pair catelim_string_of_int catelim_string_of_int "," c
      in
      let rec catelim_string_of_world (World (c, chs, ts)) ss =
        let sep2 = sep ^ "  " in
